...
let RV32_MLDSA_INTT_PHASE1_OUTER_SETUP = prove
 (`!a zetas:int32. !g pc.
  ...
  ASSUME_TAC THENL
   [MATCH_MP_TAC
     (ARITH_RULE
       `g < 64 ==> 2040 - 24 * g = 4 * (510 - 6 * g)`) THEN
    ASM_REWRITE_TAC[];
    ALL_TAC] THEN
  SUBGOAL_THEN
   `2040 - 24 * (g + 1) = 4 * (504 - 6 * g)`
  ASSUME_TAC THENL
   [MATCH_MP_TAC
     (ARITH_RULE
       `g < 64
        ==> 2040 - 24 * (g + 1) = 4 * (504 - 6 * g)`) THEN
    ASM_REWRITE_TAC[];
    ALL_TAC] THEN
  SUBGOAL_THEN
   `aligned 4
      (word_add zetas (word(2040 - 24 * g)):int32)`
  ASSUME_TAC THENL
   [ASM_REWRITE_TAC[] THEN
  ...
  SUBGOAL_THEN
   `aligned 4
      (word_add zetas (word(4 * (510 - 6 * g))):int32)`
  ASSUME_TAC THENL
   [ASM_SIMP_TAC[ALIGNED_WORD_ADD_EQ; ALIGNED_WORD; DIMINDEX_32] THEN
    CONJ_TAC THENL
     [CONV_TAC NUM_DIVIDES_CONV;
  ...
  SUBGOAL_THEN
   `aligned 4
      (word_add zetas (word(2040 - 24 * (g + 1))):int32)`
  ASSUME_TAC THENL
   [ASM_REWRITE_TAC[] THEN
    ASM_SIMP_TAC[ALIGNED_WORD_ADD_EQ; ALIGNED_WORD; DIMINDEX_32] THEN
    CONJ_TAC THENL
  ...
  SUBGOAL_THEN
   `word_add
      (word_add zetas (word(4 * (510 - 6 * g))))
      (word 4294967272):int32 =
    word_add zetas (word(2040 - 24 * (g + 1)))`
  (LABEL_TAC "post_pointer") THENL
   [SUBGOAL_THEN
     `4 * (510 - 6 * g) + 4294967272 =
      4294967296 + (2040 - 24 * (g + 1))`
    ASSUME_TAC THENL
     [MATCH_MP_TAC
       (ARITH_RULE
         `g < 64
          ==> 4 * (510 - 6 * g) + 4294967272 =
              4294967296 + (2040 - 24 * (g + 1))`) THEN
      ASM_REWRITE_TAC[];
      ASM_REWRITE_TAC
       [WORD_RULE
   ...
   (fun n ->
      RISCV_STEP_TAC MLDSA_INTT_EXEC []
       ...
       (fun th ->
          ...
          STRIP_TAC))
   (2--7) THEN
  SUBGOAL_THEN `read memory s7 = read memory s0` ASSUME_TAC THENL
   [MAP_EVERY
     (fun n ->
    ...
    ALL_TAC] THEN
  SUBGOAL_THEN
   `wordlist_from_memory(zetas,510) s7:int32 list =
    MAP iword rv32_mldsa_ntt_zetas`
  ASSUME_TAC THENL
   [REWRITE_TAC[wordlist_from_memory; READ_COMPONENT_COMPOSE] THEN
    ASM_REWRITE_TAC[] THEN
  ...
  SUBGOAL_THEN
   `2 * (252 - 3 * g) = 504 - 6 * g /\
    2 * (252 - 3 * g) + 1 = 504 - 6 * g + 1 /\
    2 * (253 - 3 * g) = 504 - 6 * g + 2 /\
    2 * (253 - 3 * g) + 1 = 504 - 6 * g + 3 /\
    2 * (254 - 3 * g) = 504 - 6 * g + 4 /\
    2 * (254 - 3 * g) + 1 = 504 - 6 * g + 5`
...
let RV32_MLDSA_INTT_PHASE2_OUTER_SETUP = prove
 (`!a zetas:int32. !g pc.
  ...
  ASSUME_TAC THENL
   [MATCH_MP_TAC
     (ARITH_RULE
       `g < 16 ==> 504 - 24 * g = 4 * (126 - 6 * g)`) THEN
    ASM_REWRITE_TAC[];
    ALL_TAC] THEN
  SUBGOAL_THEN
   `504 - 24 * (g + 1) = 4 * (120 - 6 * g)`
  ASSUME_TAC THENL
   [MATCH_MP_TAC
     (ARITH_RULE
       `g < 16
        ==> 504 - 24 * (g + 1) = 4 * (120 - 6 * g)`) THEN
    ASM_REWRITE_TAC[];
    ALL_TAC] THEN
  SUBGOAL_THEN
   `aligned 4
  ...
  SUBGOAL_THEN
   `aligned 4
      (word_add zetas (word(4 * (126 - 6 * g))):int32)`
  ASSUME_TAC THENL
   [ASM_SIMP_TAC[ALIGNED_WORD_ADD_EQ; ALIGNED_WORD; DIMINDEX_32] THEN
    CONJ_TAC THENL
     [CONV_TAC NUM_DIVIDES_CONV;
  ...
  SUBGOAL_THEN
   `word_add
      (word_add zetas (word(4 * (126 - 6 * g))))
      (word 4294967272):int32 =
    word_add zetas (word(504 - 24 * (g + 1)))`
  (LABEL_TAC "post_pointer") THENL
   [SUBGOAL_THEN
     `4 * (126 - 6 * g) + 4294967272 =
      4294967296 + (504 - 24 * (g + 1))`
    ASSUME_TAC THENL
     [MATCH_MP_TAC
       (ARITH_RULE
         `g < 16
          ==> 4 * (126 - 6 * g) + 4294967272 =
              4294967296 + (504 - 24 * (g + 1))`) THEN
      ASM_REWRITE_TAC[];
      ASM_REWRITE_TAC
       [WORD_RULE
         `word_add (word_add z (word x)) (word y):int32 =
   ...
   (fun n ->
      RISCV_STEP_TAC MLDSA_INTT_EXEC []
       ...
       (fun th ->
          ...
          STRIP_TAC))
   (2--8) THEN
  SUBGOAL_THEN `read memory s8 = read memory s0` ASSUME_TAC THENL
   [MAP_EVERY
     (fun n ->
        USE_THEN ("setup" ^ string_of_int n)
         (fun th -> ONCE_REWRITE_TAC[GSYM th]) THEN
    ...
    ALL_TAC] THEN
  SUBGOAL_THEN
   `wordlist_from_memory(zetas,510) s8:int32 list =
    MAP iword rv32_mldsa_ntt_zetas`
  ASSUME_TAC THENL
   [TRANS_TAC EQ_TRANS
     `wordlist_from_memory(zetas,510) s0:int32 list` THEN
  ...
  SUBGOAL_THEN
   `2 * (60 - 3 * g) = 120 - 6 * g /\
    2 * (60 - 3 * g) + 1 = 120 - 6 * g + 1 /\
    2 * (61 - 3 * g) = 120 - 6 * g + 2 /\
    2 * (61 - 3 * g) + 1 = 120 - 6 * g + 3 /\
    2 * (62 - 3 * g) = 120 - 6 * g + 4 /\
    2 * (62 - 3 * g) + 1 = 120 - 6 * g + 5`
  STRIP_ASSUME_TAC THENL
   [MATCH_MP_TAC
     (ARITH_RULE
       `g < 16
...
let RV32_MLDSA_INTT_PHASE3_OUTER_SETUP = prove
 (`!a zetas:int32. !g pc.
  ...
  SUBGOAL_THEN
   `aligned 4
      (word_add zetas (word(4 * (30 - 6 * g))):int32)`
  ASSUME_TAC THENL
   [ASM_SIMP_TAC[ALIGNED_WORD_ADD_EQ; ALIGNED_WORD; DIMINDEX_32] THEN
    CONJ_TAC THENL
     [CONV_TAC NUM_DIVIDES_CONV;
  ...
  SUBGOAL_THEN
   `word_add
      (word_add zetas (word(4 * (30 - 6 * g))))
      (word 4294967272):int32 =
    word_add zetas (word(120 - 24 * (g + 1)))`
  (LABEL_TAC "post_pointer") THENL
   [SUBGOAL_THEN
     `4 * (30 - 6 * g) + 4294967272 =
      4294967296 + (120 - 24 * (g + 1))`
    ASSUME_TAC THENL
     [MATCH_MP_TAC
       (ARITH_RULE
         `g < 4
          ==> 4 * (30 - 6 * g) + 4294967272 =
              4294967296 + (120 - 24 * (g + 1))`) THEN
      ASM_REWRITE_TAC[];
      ASM_REWRITE_TAC
       [WORD_RULE
         `word_add (word_add z (word x)) (word y):int32 =
