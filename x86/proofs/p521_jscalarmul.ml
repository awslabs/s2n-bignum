...
let LOCAL_JDOUBLE_SAFE = time prove
 (`exists f_events.
        forall e p3 p1 pc stackpointer.
            ALL (nonoverlapping (stackpointer,592))
            [word pc,16138; p1,216; p3,216] /\
            nonoverlapping (p3,216) (word pc,16138)
            ==> ensures x86
                (\s.
                     bytes_loaded s (word pc) p521_jscalarmul_tmc /\
                     read RIP s = word (pc + 10470) /\
                     read RSP s = word_add stackpointer (word 72) /\
                     C_ARGUMENTS [p3; p1] s /\
                     read events s = e)
                (\s.
                     read RIP s = word (pc + 13208) /\
                     read RSP s = word_add stackpointer (word 72) /\
                     (exists e2.
                        read events s = APPEND e2 e /\
                          e2 = f_events p1 p3 pc stackpointer /\
                          memaccess_inbounds e2
                            [p1,216; p3,216; stackpointer,592]
                            [p3,216; stackpointer,592]))
                (MAYCHANGE
                 ...
                 [memory :> bytes (p3,216);
                  memory :> bytes (stackpointer,592)])`,
  PROVE_SAFETY_SPEC_TAC P521_JSCALARMUL_EXEC);;
...
let LOCAL_JDOUBLE_SUBR_SAFE = time prove
 (`exists f_events.
        ...
        ==> ensures x86
                  ...
                  (exists e2.
                    read events s = APPEND e2 e /\
                      e2 = f_events p1 p3 pc
                        (word_sub stackpointer (word 640))
                        returnaddress /\
                      memaccess_inbounds e2
                        [p1,216; p3,216; (word_sub stackpointer (word 640)),648]
                        [p3,216; (word_sub stackpointer (word 640)),640]))
              (MAYCHANGE [RSP] ,,
              MAYCHANGE [RIP] ,,
              MAYCHANGE [RAX; RCX; RDX; RSI; RDI; R8; R9; R10; R11] ,,
              MAYCHANGE [CF; PF; AF; ZF; SF; OF] ,, MAYCHANGE [events] ,,
              MAYCHANGE [memory :> bytes(p3,216);
                          memory :> bytes(word_sub stackpointer (word 640),640)])`,
  X86_ADD_RETURN_STACK_TAC P521_JSCALARMUL_EXEC LOCAL_JDOUBLE_SAFE
    `[RBX; RBP; R12; R13; R14; R15]` 640 THEN
...
let LOCAL_JADD_SAFE = time prove
 (`exists f_events.
        forall e p3 p1 p2 pc stackpointer.
            ALL (nonoverlapping (stackpointer,600))
            [word pc,16138; p1,216; p2,216; p3,216] /\
            nonoverlapping (p3,216) (word pc,16138)
            ==> ensures x86
                (\s.
                     bytes_loaded s (word pc) p521_jscalarmul_tmc /\
                     read RIP s = word (pc + 7160) /\
                     read RSP s = word_add stackpointer (word 72) /\
                     C_ARGUMENTS [p3; p1; p2] s /\
                     read events s = e)
                (\s.
                     read RIP s = word (pc + 10435) /\
                     read RSP s = word_add stackpointer (word 72) /\
                     (exists e2.
                          read events s = APPEND e2 e /\
                          e2 = f_events p1 p2 p3 pc stackpointer /\
                          memaccess_inbounds e2
                          [p1,216; p2,216; p3,216; stackpointer,600]
        ...
        forall e p3 p1 p2 pc stackpointer returnaddress.
            ...
            ==> ensures x86
                ...
                (MAYCHANGE [RSP] ,,
                 ...
                 MAYCHANGE [memory :> bytes(p3,216);
                      memory :> bytes(word_sub stackpointer (word 648),648)])`,
  X86_ADD_RETURN_STACK_TAC P521_JSCALARMUL_EXEC LOCAL_JADD_SAFE
...
let LOCAL_MOD_N521_SAFETY_TAC (assump_name:string) (n:int) =
  REMOVE_THEN assump_name (fun safety_th ->
    X86_SUBROUTINE_SIM_TAC ~is_safety_thm:true
      (p521_jscalarmul_tmc,P521_JSCALARMUL_EXEC,
        0x1ac3,bignum_mod_n521_9_tmc,safety_th)
...
let LOCAL_JADD_SAFETY_TAC (assump_name:string) (n:int) =
  REMOVE_THEN assump_name (fun safety_th ->
    X86_SUBROUTINE_SIM_TAC ~is_safety_thm:true
      (p521_jscalarmul_tmc,P521_JSCALARMUL_EXEC,
        0x0,p521_jscalarmul_tmc,safety_th)
...
let full_spec,public_vars = mk_safety_spec
    ~keep_maychanges:true
    (assoc "p521_jscalarmul" subroutine_signatures)
    P521_JSCALARMUL_CORRECT
    P521_JSCALARMUL_EXEC;;

let P521_JSCALARMUL_SAFE = time prove
 (`exists f_events.
          ...
          ==> ensures x86
               ...
               (\s. read RIP s = word (pc + 0x1a09) /\
                    (exists e2.
                         ...
                         memaccess_inbounds e2
                            [scalar,72; point,216; res,216; stackpointer,4688]
                            [res,216; stackpointer,4688]))
               (MAYCHANGE [RIP] ,,
                MAYCHANGE [RAX; RCX; RDX; RSI; RDI; R8; R9; R10; R11] ,,
                MAYCHANGE [CF; PF; AF; ZF; SF; OF] ,, MAYCHANGE [events] ,,
                MAYCHANGE [RBX; RBP; R12; R13; R14; R15] ,,
  ...
  ASSUME_CALLEE_SAFETY_TAILED_TAC
    (X86_SIMD_SHARPEN_RULE BIGNUM_MOD_N521_9_NOIBT_SUBROUTINE_SAFE
      (X86_PROMOTE_RETURN_NOSTACK_TAC
        bignum_mod_n521_9_tmc BIGNUM_MOD_N521_9_SAFE THEN
      DISCHARGE_SAFETY_PROPERTY_TAC))
  ...
  REPEAT CONJ_TAC THENL [
    ...
    X86_STEPS_TAC P521_JSCALARMUL_EXEC (18--100) THEN
    ...
    X86_STEPS_TAC P521_JSCALARMUL_EXEC (165--293) THEN
    ...
    X86_STEPS_TAC P521_JSCALARMUL_EXEC (1--55) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    DISCHARGE_SAFETY_PROPERTY_TAC
  ] THEN
  ...
  X86_STEPS_TAC P521_JSCALARMUL_EXEC (22--54) THEN
  X86_STEPS_TAC P521_JSCALARMUL_EXEC (55--579) THEN
  X86_STEPS_TAC P521_JSCALARMUL_EXEC (580--615) THEN
  LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 616 THEN
  X86_STEPS_TAC P521_JSCALARMUL_EXEC (617--618) THEN
...
let P521_JSCALARMUL_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 4736),4736))
        [word pc,LENGTH p521_jscalarmul_tmc; scalar,72; point,216] /\
        ALL (nonoverlapping (res,216))
        [word pc,LENGTH p521_jscalarmul_tmc;
         word_sub stackpointer (word 4736),4744]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p521_jscalarmul_tmc /\
                 read RIP s = word pc /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      (word_sub stackpointer (word 4736))
                      returnaddress /\
                      memaccess_inbounds e2
                      [scalar,72; point,216; res,216;
                       word_sub stackpointer (word 4736),4744]
                      [res,216; word_sub stackpointer (word 4736),4736]))
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (res,216);
              memory :> bytes (word_sub stackpointer (word 4736),4736)])`,
  X86_ADD_RETURN_STACK_TAC P521_JSCALARMUL_EXEC
    P521_JSCALARMUL_SAFE `[RBX; RBP; R12; R13; R14; R15]` 4736
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let P521_JSCALARMUL_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 4736),4736))
        [word pc,LENGTH p521_jscalarmul_mc; scalar,72; point,216] /\
        ALL (nonoverlapping (res,216))
        [word pc,LENGTH p521_jscalarmul_mc;
         word_sub stackpointer (word 4736),4744]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p521_jscalarmul_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [res; scalar; point] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (res,216);
              memory :> bytes (word_sub stackpointer (word 4736),4736)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P521_JSCALARMUL_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let P521_JSCALARMUL_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 4760),4760))
        [word pc,LENGTH p521_jscalarmul_windows_tmc; scalar,72; point,216] /\
        ALL (nonoverlapping (res,216))
        [word pc,LENGTH p521_jscalarmul_windows_tmc;
         word_sub stackpointer (word 4760),4768]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p521_jscalarmul_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [res; scalar; point] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      e2 =
                      f_events scalar point res pc
                      (word_sub stackpointer (word 4760))
                      returnaddress /\
                      memaccess_inbounds e2
                      [scalar,72; point,216; res,216;
                       word_sub stackpointer (word 4760),4768]
                      [res,216; word_sub stackpointer (word 4760),4760]))
            (MAYCHANGE [RSP] ,,
  ...
  let WINDOWS_P521_JSCALARMUL_EXEC =
    X86_MK_EXEC_RULE p521_jscalarmul_windows_tmc
  and baseth =
    X86_SIMD_SHARPEN_RULE P521_JSCALARMUL_NOIBT_SUBROUTINE_SAFE
    (X86_ADD_RETURN_STACK_TAC P521_JSCALARMUL_EXEC
     P521_JSCALARMUL_SAFE `[RBX; RBP; R12; R13; R14; R15]` 4736 THEN
  ...
  META_EXISTS_TAC THEN STRIP_TAC THEN (* e *)

  REWRITE_TAC[fst WINDOWS_P521_JSCALARMUL_EXEC] THEN
  REPLICATE_TAC 4 GEN_TAC THEN WORD_FORALL_OFFSET_TAC 4760 THEN
  REWRITE_TAC[ALL; WINDOWS_C_ARGUMENTS; SOME_FLAGS;
              WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI] THEN
  REWRITE_TAC[NONOVERLAPPING_CLAUSES] THEN REPEAT STRIP_TAC THEN
  ENSURES_PRESERVED_TAC "rsi_init" `RSI` THEN
...
let P521_JSCALARMUL_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 4760),4760))
        [word pc,LENGTH p521_jscalarmul_windows_mc; scalar,72; point,216] /\
        ALL (nonoverlapping (res,216))
        [word pc,LENGTH p521_jscalarmul_windows_mc;
         word_sub stackpointer (word 4760),4768]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p521_jscalarmul_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [res; scalar; point] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (res,216);
              memory :> bytes (word_sub stackpointer (word 4760),4760)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P521_JSCALARMUL_NOIBT_WINDOWS_SUBROUTINE_SAFE));;