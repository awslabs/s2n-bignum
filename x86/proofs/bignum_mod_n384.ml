...
let bignum_mod_n384_cmc =
  define_assert_from_elf "bignum_mod_n384_cmc" "x86/p384/bignum_mod_n384.o"
[
...
let bignum_mod_n384_mc = define_trimmed "bignum_mod_n384_mc" bignum_mod_n384_cmc;;
...
let BIGNUM_MOD_N384_IBT_SUBROUTINE_CORRECT = time prove
 (`!z k x n pc stackpointer returnaddress.
      nonoverlapping (word_sub stackpointer (word 32),40) (z,48) /\
      ALL (nonoverlapping (word_sub stackpointer (word 32),32))
          [(word pc,LENGTH bignum_mod_n384_cmc); (x, 8 * val k)] /\
      nonoverlapping (word pc,LENGTH bignum_mod_n384_cmc) (z,48)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) bignum_mod_n384_cmc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                C_ARGUMENTS [z; k; x] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,6);
                memory :> bytes(word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MOD_N384_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_mod_n384_cmc = define_from_elf
   "windows_bignum_mod_n384_cmc" "x86/p384/bignum_mod_n384.obj";;

let windows_bignum_mod_n384_mc = define_trimmed "windows_bignum_mod_n384_mc" windows_bignum_mod_n384_cmc;;
...
let WINDOWS_BIGNUM_MOD_N384_SUBROUTINE_CORRECT = time prove
 (`!z k x n pc stackpointer returnaddress.
      ...
      ALL (nonoverlapping (word_sub stackpointer (word 48),48))
          [(word pc,LENGTH windows_bignum_mod_n384_mc); (x, 8 * val k)] /\
      nonoverlapping (word pc,LENGTH windows_bignum_mod_n384_mc) (z,48)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) windows_bignum_mod_n384_mc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
...
let WINDOWS_BIGNUM_MOD_N384_IBT_SUBROUTINE_CORRECT = time prove
 (`!z k x n pc stackpointer returnaddress.
      nonoverlapping (word_sub stackpointer (word 48),56) (z,48) /\
      ALL (nonoverlapping (word_sub stackpointer (word 48),48))
          [(word pc,LENGTH windows_bignum_mod_n384_cmc); (x, 8 * val k)] /\
      nonoverlapping (word pc,LENGTH windows_bignum_mod_n384_cmc) (z,48)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) windows_bignum_mod_n384_cmc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                WINDOWS_C_ARGUMENTS [z; k; x] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,6);
                memory :> bytes(word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_MOD_N384_SUBROUTINE_CORRECT));;
