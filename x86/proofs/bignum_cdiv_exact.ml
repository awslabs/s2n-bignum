...
let bignum_cdiv_exact_cmc =
  define_assert_from_elf "bignum_cdiv_exact_cmc" "x86/generic/bignum_cdiv_exact.o"
[
...
let bignum_cdiv_exact_mc = define_trimmed "bignum_cdiv_exact_mc" bignum_cdiv_exact_cmc;;
...
let BIGNUM_CDIV_EXACT_IBT_SUBROUTINE_CORRECT = prove
 (`!k z n x m a pc stackpointer returnaddress.
        nonoverlapping (z,8 * val k) (word_sub stackpointer (word 32),40) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),32))
            [(word pc,LENGTH bignum_cdiv_exact_cmc); (x,8 * val n)] /\
        nonoverlapping (word pc,LENGTH bignum_cdiv_exact_cmc) (z,8 * val k) /\
        (x = z \/ nonoverlapping(x,8 * val n) (z,8 * val k))
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_cdiv_exact_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [k;z;n;x;m] s /\
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bignum(z,val k);
                       memory :> bytes(word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_CDIV_EXACT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_cdiv_exact_cmc = define_from_elf
   "windows_bignum_cdiv_exact_cmc" "x86/generic/bignum_cdiv_exact.obj";;

let windows_bignum_cdiv_exact_mc = define_trimmed "windows_bignum_cdiv_exact_mc" windows_bignum_cdiv_exact_cmc;;
...
let WINDOWS_BIGNUM_CDIV_EXACT_SUBROUTINE_CORRECT = prove
 (`!k z n x m a pc stackpointer returnaddress.
        ...
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
            [(word pc,LENGTH windows_bignum_cdiv_exact_mc); (x,8 * val n)] /\
        nonoverlapping (word pc,LENGTH windows_bignum_cdiv_exact_mc) (z,8 * val k) /\
        (x = z \/ nonoverlapping(x,8 * val n) (z,8 * val k))
        ==> ensures x86
...
let WINDOWS_BIGNUM_CDIV_EXACT_IBT_SUBROUTINE_CORRECT = prove
 (`!k z n x m a pc stackpointer returnaddress.
        nonoverlapping (z,8 * val k) (word_sub stackpointer (word 48),56) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
            [(word pc,LENGTH windows_bignum_cdiv_exact_cmc); (x,8 * val n)] /\
        nonoverlapping (word pc,LENGTH windows_bignum_cdiv_exact_cmc) (z,8 * val k) /\
        (x = z \/ nonoverlapping(x,8 * val n) (z,8 * val k))
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_cdiv_exact_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [k;z;n;x;m] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bignum(z,val k);
                       memory :> bytes(word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_CDIV_EXACT_SUBROUTINE_CORRECT));;
