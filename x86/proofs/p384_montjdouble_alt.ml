...
let P384_MONTJDOUBLE_ALT_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 pc stackpointer.
           ALL (nonoverlapping (stackpointer,344))
           [word pc,11681; p1,144; p3,144] /\
           nonoverlapping (p3,144) (word pc,11681)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc)
                    (BUTLAST p384_montjdouble_alt_tmc) /\
                    read RIP s = word (pc + 17) /\
                    read RSP s = stackpointer /\
                    C_ARGUMENTS [p3; p1] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 11663) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events p1 p3 pc stackpointer /\
                         memaccess_inbounds e2
                         [p1,144; p3,144; stackpointer,344]
                         [p3,144; stackpointer,344]))
               (MAYCHANGE
                [RIP; RDI; RSI; RAX; RDX; RCX; R8; R9; R10; R11; RBX; RBP;
                 R12; R13; R14; R15] ,,
                MAYCHANGE SOME_FLAGS ,,
                MAYCHANGE [events] ,,
                MAYCHANGE
                [memory :> bytes (p3,144); memory :> bytes (stackpointer,344)])`,
  ASSERT_CONCL_TAC full_spec THEN
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars P384_MONTJDOUBLE_ALT_EXEC);;

let P384_MONTJDOUBLE_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 392),392))
        [word pc,LENGTH p384_montjdouble_alt_tmc; p1,144] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjdouble_alt_tmc;
         word_sub stackpointer (word 392),400]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjdouble_alt_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      read events s = APPEND e2 e /\
                      e2 =
                      f_events p1 p3 pc (word_sub stackpointer (word 392))
                      returnaddress /\
                      memaccess_inbounds e2
                      [p1,144; p3,144; word_sub stackpointer (word 392),400]
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,144);
              memory :> bytes (word_sub stackpointer (word 392),392)])`,
  X86_PROMOTE_RETURN_STACK_TAC p384_montjdouble_alt_tmc P384_MONTJDOUBLE_ALT_SAFE
    `[RBX; RBP; R12; R13; R14; R15]` 392
...
let P384_MONTJDOUBLE_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 pc stackpointer returnaddress.
        ...
        [word pc,LENGTH p384_montjdouble_alt_mc; p1,144] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjdouble_alt_mc;
         word_sub stackpointer (word 392),400]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjdouble_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,144);
              memory :> bytes (word_sub stackpointer (word 392),392)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P384_MONTJDOUBLE_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let P384_MONTJDOUBLE_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 408),408))
        [word pc,LENGTH p384_montjdouble_alt_windows_tmc; p1,144] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjdouble_alt_windows_tmc;
         word_sub stackpointer (word 408),416]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjdouble_alt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      read events s = APPEND e2 e /\
                      e2 =
                      f_events p1 p3 pc (word_sub stackpointer (word 408))
                      returnaddress /\
                      memaccess_inbounds e2
                      [p1,144; p3,144; word_sub stackpointer (word 408),416]
  ...
  WINDOWS_X86_WRAP_STACK_TAC
    p384_montjdouble_alt_windows_tmc p384_montjdouble_alt_tmc
    P384_MONTJDOUBLE_ALT_SAFE
    `[RBX; RBP; R12; R13; R14; R15]` 392
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let P384_MONTJDOUBLE_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 408),408))
        [word pc,LENGTH p384_montjdouble_alt_windows_mc; p1,144] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjdouble_alt_windows_mc;
         word_sub stackpointer (word 408),416]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjdouble_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,144);
              memory :> bytes (word_sub stackpointer (word 408),408)])`,
MATCH_ACCEPT_TAC(ADD_IBT_RULE P384_MONTJDOUBLE_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;