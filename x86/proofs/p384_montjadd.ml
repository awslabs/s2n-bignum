...
let p384_montjadd_cmc = define_assert_from_elf
  "p384_montjadd_cmc" "x86/p384/p384_montjadd.o"
[
...
let p384_montjadd_mc = define_trimmed "p384_montjadd_mc" p384_montjadd_cmc;;
...
let P384_MONTJADD_IBT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 400),400))
            [(word pc,LENGTH p384_montjadd_cmc); (p1,144); (p2,144)] /\
        ALL (nonoverlapping (p3,144))
            [(word pc,LENGTH p384_montjadd_cmc); (word_sub stackpointer (word 400),408)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) p384_montjadd_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [p3; p1; p2] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,144);
                      memory :> bytes(word_sub stackpointer (word 400),400)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P384_MONTJADD_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_p384_montjadd_cmc = define_from_elf "windows_p384_montjadd_cmc"
      "x86/p384/p384_montjadd.obj";;

let windows_p384_montjadd_mc = define_trimmed "windows_p384_montjadd_mc" windows_p384_montjadd_cmc;;

let WINDOWS_P384_MONTJADD_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 416),416))
            [(word pc,LENGTH windows_p384_montjadd_mc); (p1,144); (p2,144)] /\
        ALL (nonoverlapping (p3,144))
            [(word pc,LENGTH windows_p384_montjadd_mc); (word_sub stackpointer (word 416),424)]
        ==> ensures x86
...
let WINDOWS_P384_MONTJADD_IBT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 416),416))
            [(word pc,LENGTH windows_p384_montjadd_cmc); (p1,144); (p2,144)] /\
        ALL (nonoverlapping (p3,144))
            [(word pc,LENGTH windows_p384_montjadd_cmc); (word_sub stackpointer (word 416),424)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_p384_montjadd_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,144);
                      memory :> bytes(word_sub stackpointer (word 416),416)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_P384_MONTJADD_SUBROUTINE_CORRECT));;
