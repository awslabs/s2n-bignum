...
let P384_MONTJADD_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 p2 pc stackpointer.
           ALL (nonoverlapping (stackpointer,352))
           [word pc,22027; p1,144; p2,144; p3,144] /\
           nonoverlapping (p3,144) (word pc,22027)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST p384_montjadd_tmc) /\
                    read RIP s = word (pc + 17) /\
                    read RSP s = stackpointer /\
                    C_ARGUMENTS [p3; p1; p2] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 22009) /\
                    (exists e2.
...
let P384_MONTJADD_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 400),400))
        [word pc,LENGTH p384_montjadd_tmc; p1,144; p2,144] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjadd_tmc;
         word_sub stackpointer (word 400),408]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjadd_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
...
let P384_MONTJADD_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ...
        [word pc,LENGTH p384_montjadd_mc; p1,144; p2,144] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjadd_mc;
         word_sub stackpointer (word 400),408]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjadd_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,144);
              memory :> bytes (word_sub stackpointer (word 400),400)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P384_MONTJADD_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let P384_MONTJADD_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 416),416))
        [word pc,LENGTH p384_montjadd_windows_tmc; p1,144; p2,144] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjadd_windows_tmc;
         word_sub stackpointer (word 416),424]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjadd_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
...
let P384_MONTJADD_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 416),416))
        [word pc,LENGTH p384_montjadd_windows_mc; p1,144; p2,144] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjadd_windows_mc;
         word_sub stackpointer (word 416),424]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjadd_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,144);
              memory :> bytes (word_sub stackpointer (word 416),416)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P384_MONTJADD_NOIBT_WINDOWS_SUBROUTINE_SAFE));;