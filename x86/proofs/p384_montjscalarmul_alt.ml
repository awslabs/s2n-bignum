...
let LOCAL_JADD_SAFETY_TAC (assump_name:string) (n:int) =
  REMOVE_THEN assump_name (fun safety_th ->
    X86_SUBROUTINE_SIM_TAC ~is_safety_thm:true
        (p384_montjscalarmul_alt_tmc,P384_MONTJSCALARMUL_ALT_EXEC,
          0x1c3a,p384_montjadd_alt_tmc,safety_th)
...
let LOCAL_JDOUBLE_SAFETY_TAC (assump_name:string) (n:int) =
  REMOVE_THEN assump_name (fun safety_th ->
    X86_SUBROUTINE_SIM_TAC ~is_safety_thm:true
        (p384_montjscalarmul_alt_tmc,P384_MONTJSCALARMUL_ALT_EXEC,
          0x756f,p384_montjdouble_alt_tmc,safety_th)
...
let P384_MONTJSCALARMUL_ALT_SAFE = time prove
 (`exists f_events.
       forall e res scalar point pc stackpointer.
           ALL (nonoverlapping (stackpointer,3096))
           [word pc,41744; res,144; scalar,48; point,144] /\
           nonoverlapping (res,144) (word pc,41744)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) p384_montjscalarmul_alt_tmc /\
  ...
  ASSUME_CALLEE_SAFETY_TAILED_TAC
     (let baseth = X86_SIMD_SHARPEN_RULE P384_MONTJADD_ALT_NOIBT_SUBROUTINE_SAFE
       (X86_PROMOTE_RETURN_STACK_TAC p384_montjadd_alt_tmc P384_MONTJADD_ALT_SAFE
    ...
    "H_JADD_SAFE" THEN
  ASSUME_CALLEE_SAFETY_TAILED_TAC
      (let baseth = X86_SIMD_SHARPEN_RULE P384_MONTJDOUBLE_ALT_NOIBT_SUBROUTINE_SAFE
  ...
  REPEAT CONJ_TAC THENL [
    ...
    X86_STEPS_TAC P384_MONTJSCALARMUL_ALT_EXEC (1--68) THEN
    ...
    X86_STEPS_TAC P384_MONTJSCALARMUL_ALT_EXEC (133--522) THEN
    ...
    X86_STEPS_TAC P384_MONTJSCALARMUL_ALT_EXEC (1--37) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    DISCHARGE_SAFETY_PROPERTY_TAC
  ] THEN
  ...
  X86_STEPS_TAC P384_MONTJSCALARMUL_ALT_EXEC (22--45) THEN
  X86_STEPS_TAC P384_MONTJSCALARMUL_ALT_EXEC (46--434) THEN
  LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 435 THEN
  X86_STEPS_TAC P384_MONTJSCALARMUL_ALT_EXEC (436--437) THEN
...
let P384_MONTJSCALARMUL_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 3144),3144))
        [word pc,LENGTH p384_montjscalarmul_alt_tmc; scalar,48; point,144] /\
        ALL (nonoverlapping (res,144))
        [word pc,LENGTH p384_montjscalarmul_alt_tmc;
        ...
        ==> ensures x86
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (res,144);
              memory :> bytes (word_sub stackpointer (word 3144),3144)])`,
  X86_ADD_RETURN_STACK_TAC P384_MONTJSCALARMUL_ALT_EXEC
    P384_MONTJSCALARMUL_ALT_SAFE `[RBX; RBP; R12; R13; R14; R15]` 3144
...
let P384_MONTJSCALARMUL_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ...
        [word pc,LENGTH p384_montjscalarmul_alt_mc; scalar,48; point,144] /\
        ALL (nonoverlapping (res,144))
        [word pc,LENGTH p384_montjscalarmul_alt_mc;
         word_sub stackpointer (word 3144),3152]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjscalarmul_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [res; scalar; point] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (res,144);
              memory :> bytes (word_sub stackpointer (word 3144),3144)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P384_MONTJSCALARMUL_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let P384_MONTJSCALARMUL_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 3168),3168))
        [word pc,LENGTH p384_montjscalarmul_alt_windows_tmc; scalar,48;
         point,144] /\
        ALL (nonoverlapping (res,144))
        [word pc,LENGTH p384_montjscalarmul_alt_windows_tmc;
         word_sub stackpointer (word 3168),3176]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjscalarmul_alt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [res; scalar; point] s /\
  ...
  let WINDOWS_P384_MONTJSCALARMUL_ALT_EXEC =
    X86_MK_EXEC_RULE p384_montjscalarmul_alt_windows_tmc
  and baseth =
    X86_SIMD_SHARPEN_RULE P384_MONTJSCALARMUL_ALT_NOIBT_SUBROUTINE_SAFE
    (X86_ADD_RETURN_STACK_TAC P384_MONTJSCALARMUL_ALT_EXEC
     P384_MONTJSCALARMUL_ALT_SAFE `[RBX; RBP; R12; R13; R14; R15]` 3144 THEN
     DISCHARGE_SAFETY_PROPERTY_TAC) in
  let subth =
    CONV_RULE(ONCE_DEPTH_CONV NUM_MULT_CONV)
     (REWRITE_RULE[bignum_triple_from_memory] baseth) in
...
let P384_MONTJSCALARMUL_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 3168),3168))
        [word pc,LENGTH p384_montjscalarmul_alt_windows_mc; scalar,48;
         point,144] /\
        ALL (nonoverlapping (res,144))
        [word pc,LENGTH p384_montjscalarmul_alt_windows_mc;
         word_sub stackpointer (word 3168),3176]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjscalarmul_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [res; scalar; point] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (res,144);
              memory :> bytes (word_sub stackpointer (word 3168),3168)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P384_MONTJSCALARMUL_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;