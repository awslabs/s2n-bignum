...
let bignum_montinv_sm2_cmc = define_assert_from_elf "bignum_montinv_sm2_cmc" "x86/sm2/bignum_montinv_sm2.o"
[
...
let bignum_montinv_sm2_mc = define_trimmed "bignum_montinv_sm2_mc" bignum_montinv_sm2_cmc;;
...
let BIGNUM_MONTINV_SM2_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 288),288))
            [(word pc,LENGTH bignum_montinv_sm2_cmc); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word_sub stackpointer (word 288),296); (word pc,LENGTH bignum_montinv_sm2_cmc)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montinv_sm2_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [z; x] s /\
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                    memory :> bytes(word_sub stackpointer (word 288),288)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTINV_SM2_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_montinv_sm2_cmc = define_from_elf "windows_bignum_montinv_sm2_cmc"
      "x86/sm2/bignum_montinv_sm2.obj";;

let windows_bignum_montinv_sm2_mc = define_trimmed "windows_bignum_montinv_sm2_mc" windows_bignum_montinv_sm2_cmc;;

let WINDOWS_BIGNUM_MONTINV_SM2_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 304),304))
            [(word pc,LENGTH windows_bignum_montinv_sm2_mc); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word_sub stackpointer (word 304),312); (word pc,LENGTH windows_bignum_montinv_sm2_mc)]
        ==> ensures x86
...
let WINDOWS_BIGNUM_MONTINV_SM2_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 304),304))
            [(word pc,LENGTH windows_bignum_montinv_sm2_cmc); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word_sub stackpointer (word 304),312); (word pc,LENGTH windows_bignum_montinv_sm2_cmc)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_montinv_sm2_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                    memory :> bytes(word_sub stackpointer (word 304),304)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_MONTINV_SM2_SUBROUTINE_CORRECT));;
