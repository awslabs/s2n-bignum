...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_sub_p521" subroutine_signatures)
    BIGNUM_SUB_P521_CORRECT
    BIGNUM_SUB_P521_EXEC;;

let BIGNUM_SUB_P521_SAFE = time prove
 (`exists f_events.
       forall e z x y pc.
           nonoverlapping (word pc,155) (z,8 * 9)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_sub_p521_tmc) /\
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_SUB_P521_EXEC);;

let BIGNUM_SUB_P521_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x y pc stackpointer returnaddress.
           ...
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_sub_p521_tmc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [z; x; y] s /\
               ...
               (\s.
                    ...
                    (exists e2.
                         ...
                         memaccess_inbounds e2
                            [x,72; y,72; z,72; word_sub stackpointer (word 16),24]
               ...
               (MAYCHANGE [RSP] ,,
                ...
                [memory :> bignum (z,9);
                 memory :> bytes (word_sub stackpointer (word 16),16)])`,
  X86_PROMOTE_RETURN_STACK_TAC bignum_sub_p521_tmc BIGNUM_SUB_P521_SAFE
    `[RBX; R12]` 16 THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_SUB_P521_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x y pc stackpointer returnaddress.
           nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 9) /\
           ALL (nonoverlapping (word_sub stackpointer (word 16),16))
           ...
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_sub_p521_mc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [z; x; y] s /\
               ...
               (MAYCHANGE [RSP] ,,
                ...
                [memory :> bignum (z,9);
                 memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SUB_P521_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_SUB_P521_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x y pc stackpointer returnaddress.
           nonoverlapping (word_sub stackpointer (word 32),40) (z,8 * 9) /\
           ALL (nonoverlapping (word_sub stackpointer (word 32),32))
           [word pc,LENGTH bignum_sub_p521_windows_tmc; x,8 * 9; y,8 * 9] /\
           nonoverlapping (word pc,LENGTH bignum_sub_p521_windows_tmc)
           (z,8 * 9)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_sub_p521_windows_tmc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    WINDOWS_C_ARGUMENTS [z; x; y] s /\
               ...
               (\s.
                    ...
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events x y z pc (word_sub stackpointer (word 32))
                                       returnaddress /\
                         memaccess_inbounds e2
                            [x,72; y,72; z,72; word_sub stackpointer (word 32),40]
               ...
               (MAYCHANGE [RSP] ,,
                ...
                [memory :> bignum (z,9);
                 memory :> bytes (word_sub stackpointer (word 32),32)])`,
  WINDOWS_X86_WRAP_STACK_TAC bignum_sub_p521_windows_tmc bignum_sub_p521_tmc
    BIGNUM_SUB_P521_SAFE `[RBX; R12]` 16 THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_SUB_P521_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x y pc stackpointer returnaddress.
           nonoverlapping (word_sub stackpointer (word 32),40) (z,8 * 9) /\
           ALL (nonoverlapping (word_sub stackpointer (word 32),32))
           [word pc,LENGTH bignum_sub_p521_windows_mc; x,8 * 9; y,8 * 9] /\
           nonoverlapping (word pc,LENGTH bignum_sub_p521_windows_mc)
           (z,8 * 9)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_sub_p521_windows_mc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    WINDOWS_C_ARGUMENTS [z; x; y] s /\
               ...
               (MAYCHANGE [RSP] ,,
                ...
                [memory :> bignum (z,9);
                 memory :> bytes (word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SUB_P521_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
