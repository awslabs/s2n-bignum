...
let bignum_cmnegadd_cmc =
  define_assert_from_elf "bignum_cmnegadd_cmc" "x86/generic/bignum_cmnegadd.o"
[
...
let bignum_cmnegadd_mc = define_trimmed "bignum_cmnegadd_mc" bignum_cmnegadd_cmc;;
...
let BIGNUM_CMNEGADD_IBT_SUBROUTINE_CORRECT = prove
 (`!p z d c n x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * val p) (word_sub stackpointer (word 8),16) /\
        ALL (nonoverlapping (word_sub stackpointer (word 8),8))
            [(word pc,LENGTH bignum_cmnegadd_cmc); (x,8 * val n)] /\
        nonoverlapping (word pc,LENGTH bignum_cmnegadd_cmc) (z,8 * val p) /\
        (x = z \/ nonoverlapping(x,8 * val n) (z,8 * val p))
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_cmnegadd_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [p;z;c;n;x] s /\
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bignum(z,val p);
                       memory :> bytes(word_sub stackpointer (word 8),8)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_CMNEGADD_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_cmnegadd_cmc = define_from_elf
   "windows_bignum_cmnegadd_cmc" "x86/generic/bignum_cmnegadd.obj";;

let windows_bignum_cmnegadd_mc = define_trimmed "windows_bignum_cmnegadd_mc" windows_bignum_cmnegadd_cmc;;
...
let WINDOWS_BIGNUM_CMNEGADD_IBT_SUBROUTINE_CORRECT = prove
 (`!p z d c n x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * val p) (word_sub stackpointer (word 24),32) /\
        ALL (nonoverlapping (word_sub stackpointer (word 24),24))
            [(word pc,LENGTH windows_bignum_cmnegadd_cmc); (x,8 * val n)] /\
        nonoverlapping (word pc,LENGTH windows_bignum_cmnegadd_cmc) (z,8 * val p) /\
        (x = z \/ nonoverlapping(x,8 * val n) (z,8 * val p))
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_cmnegadd_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [p;z;c;n;x] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bignum(z,val p);
                       memory :> bytes(word_sub stackpointer (word 24),24)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_CMNEGADD_SUBROUTINE_CORRECT));;
