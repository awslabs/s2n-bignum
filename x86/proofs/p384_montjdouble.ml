...
let P384_MONTJDOUBLE_IBT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 392),392))
            [(word pc,LENGTH p384_montjdouble_cmc); (p1,144)] /\
        ALL (nonoverlapping (p3,144))
            [(word pc,LENGTH p384_montjdouble_cmc); (word_sub stackpointer (word 392),400)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) p384_montjdouble_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [p3; p1] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,144);
                      memory :> bytes(word_sub stackpointer (word 392),392)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P384_MONTJDOUBLE_SUBROUTINE_CORRECT));;
...
let WINDOWS_P384_MONTJDOUBLE_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 408),408))
            [(word pc,LENGTH windows_p384_montjdouble_mc); (p1,144)] /\
        ALL (nonoverlapping (p3,144))
            [(word pc,LENGTH windows_p384_montjdouble_mc); (word_sub stackpointer (word 408),416)]
        ==> ensures x86
...
let WINDOWS_P384_MONTJDOUBLE_IBT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 408),408))
            [(word pc,LENGTH windows_p384_montjdouble_cmc); (p1,144)] /\
        ALL (nonoverlapping (p3,144))
            [(word pc,LENGTH windows_p384_montjdouble_cmc); (word_sub stackpointer (word 408),416)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_p384_montjdouble_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [p3; p1] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,144);
                      memory :> bytes(word_sub stackpointer (word 408),408)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_P384_MONTJDOUBLE_SUBROUTINE_CORRECT));;
