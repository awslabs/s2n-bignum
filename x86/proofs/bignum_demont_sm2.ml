...
let bignum_demont_sm2_cmc =
  define_assert_from_elf "bignum_demont_sm2_cmc" "x86/sm2/bignum_demont_sm2.o"
[
...
let bignum_demont_sm2_mc = define_trimmed "bignum_demont_sm2_mc" bignum_demont_sm2_cmc;;
...
let BIGNUM_DEMONT_SM2_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_demont_sm2_cmc) (z,8 * 4) /\
        nonoverlapping (stackpointer,8) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_demont_sm2_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [z; x] s /\
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_DEMONT_SM2_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_demont_sm2_cmc = define_from_elf
   "windows_bignum_demont_sm2_cmc" "x86/sm2/bignum_demont_sm2.obj";;

let windows_bignum_demont_sm2_mc = define_trimmed "windows_bignum_demont_sm2_mc" windows_bignum_demont_sm2_cmc;;
...
let WINDOWS_BIGNUM_DEMONT_SM2_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        ...
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH windows_bignum_demont_sm2_mc); (x,8 * 4)] /\
        nonoverlapping (word pc,LENGTH windows_bignum_demont_sm2_mc) (z,8 * 4)
        ==> ensures x86
...
let WINDOWS_BIGNUM_DEMONT_SM2_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 16),24) /\
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH windows_bignum_demont_sm2_cmc); (x,8 * 4)] /\
        nonoverlapping (word pc,LENGTH windows_bignum_demont_sm2_cmc) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_demont_sm2_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                       memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_DEMONT_SM2_SUBROUTINE_CORRECT));;
