...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "p384_montjadd_alt" subroutine_signatures)
    P384_MONTJADD_ALT_CORRECT
    P384_MONTJADD_ALT_EXEC;;

let P384_MONTJADD_ALT_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 p2 pc stackpointer.
           ALL (nonoverlapping (stackpointer,352))
           [word pc,22837; p1,144; p2,144; p3,144] /\
           nonoverlapping (p3,144) (word pc,22837)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST p384_montjadd_alt_tmc) /\
                    read RIP s = word (pc + 17) /\
                    read RSP s = stackpointer /\
                    C_ARGUMENTS [p3; p1; p2] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 22819) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events p1 p2 p3 pc stackpointer /\
                         memaccess_inbounds e2
                         [p1,144; p2,144; p3,144; stackpointer,352]
                         [p3,144; stackpointer,352]))
               (MAYCHANGE
                [RIP; RSI; RAX; RDX; RCX; R8; R9; R10; R11; RBX; RBP; R12;
                 R13; R14; R15] ,,
                MAYCHANGE SOME_FLAGS ,,
                MAYCHANGE [events] ,,
                MAYCHANGE
                [memory :> bytes (p3,144); memory :> bytes (stackpointer,352)])`,
  ASSERT_CONCL_TAC full_spec THEN
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars P384_MONTJADD_ALT_EXEC);;

let P384_MONTJADD_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 400),400))
        [word pc,LENGTH p384_montjadd_alt_tmc; p1,144; p2,144] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjadd_alt_tmc;
         word_sub stackpointer (word 400),408]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjadd_alt_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      read events s = APPEND e2 e /\
                      e2 =
                      f_events p1 p2 p3 pc (word_sub stackpointer (word 400))
                      returnaddress /\
                      memaccess_inbounds e2
  ...
  X86_PROMOTE_RETURN_STACK_TAC p384_montjadd_alt_tmc P384_MONTJADD_ALT_SAFE
    `[RBX; RBP; R12; R13; R14; R15]` 400 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let P384_MONTJADD_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 400),400))
        [word pc,LENGTH p384_montjadd_alt_mc; p1,144; p2,144] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjadd_alt_mc;
         word_sub stackpointer (word 400),408]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjadd_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,144);
              memory :> bytes (word_sub stackpointer (word 400),400)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P384_MONTJADD_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let P384_MONTJADD_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 416),416))
        [word pc,LENGTH p384_montjadd_alt_windows_tmc; p1,144; p2,144] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjadd_alt_windows_tmc;
         word_sub stackpointer (word 416),424]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjadd_alt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      read events s = APPEND e2 e /\
                      e2 =
                      f_events p1 p2 p3 pc (word_sub stackpointer (word 416))
                      returnaddress /\
                      memaccess_inbounds e2
                      [p1,144; p2,144; p3,144;
                       word_sub stackpointer (word 416),424]
                      [p3,144; word_sub stackpointer (word 416),416]))
            (MAYCHANGE [RSP] ,,
  ...
  WINDOWS_X86_WRAP_STACK_TAC
    p384_montjadd_alt_windows_tmc p384_montjadd_alt_tmc
    P384_MONTJADD_ALT_SAFE
    `[RBX; RBP; R12; R13; R14; R15]` 400 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let P384_MONTJADD_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 416),416))
        [word pc,LENGTH p384_montjadd_alt_windows_mc; p1,144; p2,144] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjadd_alt_windows_mc;
         word_sub stackpointer (word 416),424]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjadd_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,144);
              memory :> bytes (word_sub stackpointer (word 416),416)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P384_MONTJADD_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;