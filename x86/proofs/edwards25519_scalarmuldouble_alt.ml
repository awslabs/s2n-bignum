...
(**** print_literal_relocs_from_elf "x86/curve25519/edwards25519_scalarmuldouble_alt.o";;
 ****)

let edwards25519_scalarmuldouble_alt_body : 'a. int list -> (int list -> 'a) ->
                                                (string * int -> 'a) -> 'a list =
  ...
  0x48; 0x8d; 0x2d]); ADDR ("WHOLE_READONLY_data",(-4)); b[
                           (* LEA (% rbp) (Riprel (word_sx (iword (&WHOLE_READONLY_data - (&pc + &1173))))) *)
  ...
  0x45; 0x31; 0xff;        (* XOR (% r15d) (% r15d) *)
  0x48; 0x8d; 0x2d]; ADDR ("WHOLE_READONLY_data",(-4)); b[
                           (* LEA (% rbp) (Riprel (word_sx (iword (&WHOLE_READONLY_data - (&pc + &3754))))) *)
  0x48; 0x83; 0xbc; 0x24; 0xa0; 0x05; 0x00; 0x00; 0x01;
                           (* CMP (Memop Quadword (%% (rsp,1440))) (Imm8 (word 1)) *)
  0x48; 0x8b; 0x75; 0x00;  (* MOV (% rsi) (Memop Quadword (%% (rbp,0))) *)
  0x48; 0x0f; 0x44; 0xc6;  (* CMOVE (% rax) (% rsi) *)
...
let edwards25519_scalarmuldouble_alt_mc,
    (edwards25519_scalarmuldouble_alt_constant_data::_) =
    define_assert_relocs_from_elf "edwards25519_scalarmuldouble_alt_mc"
    "x86/curve25519/edwards25519_scalarmuldouble_alt.o"
    (fun b ADDR -> edwards25519_scalarmuldouble_alt_body
       [0xf3; 0x0f; 0x1e; 0xfa]  (* ENDBR64 *)
       b ADDR);;

let edwards25519_scalarmuldouble_alt_tmc =
  define_trimmed "edwards25519_scalarmuldouble_alt_tmc" edwards25519_scalarmuldouble_alt_mc;;

let EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC =
  X86_MK_EXEC_RULE edwards25519_scalarmuldouble_alt_tmc;;

let edwards25519_scalarmuldouble_alt_tmc' =
  SPECL [`pc:num`; `tables:num`] edwards25519_scalarmuldouble_alt_tmc;;
...
let BYTES_LOADED_DATA = prove
 (`bytes_loaded s tab edwards25519_scalarmuldouble_alt_constant_data <=>
  ...
  REWRITE_TAC[bytes_loaded; READ_BYTELIST_EQ_BYTES;
    CONV_RULE (RAND_CONV LENGTH_CONV)
     (AP_TERM `LENGTH:byte list->num` edwards25519_scalarmuldouble_alt_constant_data)]);;
...
let EDWARDS25519DOUBLEBASE_TABLE_LEMMA = prove
 (`read (memory :> bytes(wpc,768)) s =
  ...
  REWRITE_TAC[GSYM BYTES_LOADED_DATA;
              edwards25519_scalarmuldouble_alt_constant_data] THEN
...
let LOCAL_MUL_P25519_TAC =
  X86_MACRO_SIM_ABBREV_TAC edwards25519_scalarmuldouble_alt_tmc' 129 lvs
   `!(t:x86state) pcin pcout q3 n3 q1 n1 q2 n2.
      ...
      ==>
      nonoverlapping (word pc,0x6fc9) (word_add (read q3 t) (word n3),8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (edwards25519_scalarmuldouble_alt_tmc pc tables) /\
                read RIP s = pcin /\
...
let LOCAL_SQR_4_TAC =
  X86_MACRO_SIM_ABBREV_TAC edwards25519_scalarmuldouble_alt_tmc' 109 lvs
   `!(t:x86state) pcin pcout p3 n3 p1 n1.
      ...
      ==>
      nonoverlapping (word pc,0x6fc9) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (edwards25519_scalarmuldouble_alt_tmc pc tables) /\
                read RIP s = pcin /\
                read RSP s = read RSP t /\
...
let LOCAL_EPDOUBLE_CORRECT = time prove
 (`!p3 p1 T1 pc stackpointer.
    ALL (nonoverlapping (stackpointer,160))
        [(word pc,0x6fc9); (p3,128); (p1,96)] /\
    nonoverlapping (p3,128) (word pc,0x6fc9)
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
                 (edwards25519_scalarmuldouble_alt_tmc pc tables) /\
...
let LOCAL_PDOUBLE_CORRECT = time prove
 (`!p3 p1 T1 pc stackpointer.
    ALL (nonoverlapping (stackpointer,160))
        [(word pc,0x6fc9); (p3,96); (p1,96)] /\
    nonoverlapping (p3,96) (word pc,0x6fc9)
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
                 (edwards25519_scalarmuldouble_alt_tmc pc tables) /\
...
let LOCAL_EPADD_CORRECT = time prove
 (`!p3 p1 Q1 p2 Q2 pc stackpointer.
    ALL (nonoverlapping (stackpointer,192))
        [(word pc,0x6fc9); (p3,128); (p1,128); (p2,128)] /\
    nonoverlapping (p3,128) (word pc,0x6fc9)
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
                 (edwards25519_scalarmuldouble_alt_tmc pc tables) /\
...
let LOCAL_PEPADD_CORRECT = time prove
 (`!p3 p1 Q1 p2 T2 pc stackpointer.
    ALL (nonoverlapping (stackpointer,192))
        [(word pc,0x6fc9); (p3,128); (p1,128); (p2,96)] /\
    nonoverlapping (p3,128) (word pc,0x6fc9)
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
                 (edwards25519_scalarmuldouble_alt_tmc pc tables) /\
...
let EDWARDS25519_SCALARMULDOUBLE_ALT_CORRECT = time prove
 (`!tables res scalar point bscalar n xy m pc stackpointer.
    ...
    ALL (nonoverlapping (res,64)) [(word pc,0x6fc9); (word tables,768)]
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
                (edwards25519_scalarmuldouble_alt_tmc pc tables) /\
  ...
  REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (155--155) THEN
    ...
    ANTS_TAC THENL
     [EXPAND_TAC "wpc" THEN IMP_REWRITE_TAC[RIP_REL_ADDR_FOLD] THEN
      ASM_REWRITE_TAC[];
      DISCH_TAC] THEN
    X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (156--386) THEN
    MAP_EVERY ABBREV_TAC
  ...
  X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (23--23) THEN
  FIRST_X_ASSUM(MP_TAC o SPEC `wpc:int64` o MATCH_MP (MESON[]
   `read RBP s = x ==> !w. x = w ==> read RBP s = w`)) THEN
  ANTS_TAC THENL
   [EXPAND_TAC "wpc" THEN IMP_REWRITE_TAC[RIP_REL_ADDR_FOLD] THEN
    ASM_REWRITE_TAC[];
    DISCH_TAC] THEN
  X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (24--242) THEN
...
(* Bridge equation relating the IBT mc to the trimmed mc. *)
let EDWARDS25519_SCALARMULDOUBLE_ALT_MC_BRIDGE = prove
 (`!pc tables. edwards25519_scalarmuldouble_alt_mc pc tables =
     APPEND [word 0xf3:byte; word 0x0f; word 0x1e; word 0xfa]
            (edwards25519_scalarmuldouble_alt_tmc (pc + 4) tables)`,
  REPEAT GEN_TAC THEN
  REWRITE_TAC[edwards25519_scalarmuldouble_alt_mc;
...
let LENGTH_EDWARDS25519_SCALARMULDOUBLE_ALT_MC = prove
 (`!pc tables. LENGTH (edwards25519_scalarmuldouble_alt_mc pc tables) = 0x6fcd`,
  REWRITE_TAC[EDWARDS25519_SCALARMULDOUBLE_ALT_MC_BRIDGE; LENGTH_APPEND; LENGTH;
              fst EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC] THEN ARITH_TAC);;
 ...
 (`!tables res scalar point bscalar n xy m pc stackpointer returnaddress.
    ...
    ==> ensures x86
         ...
         (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
          MAYCHANGE [memory :> bytes(res,64);
                     memory :> bytes(word_sub stackpointer (word 1720),1720)])`,
  REWRITE_TAC[LENGTH_EDWARDS25519_SCALARMULDOUBLE_ALT_MC] THEN
  MATCH_ACCEPT_TAC(ADD_IBT_RULE
                     ~bridge:(Some EDWARDS25519_SCALARMULDOUBLE_ALT_MC_BRIDGE)
                     (REWRITE_RULE[fst EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC]
                       EDWARDS25519_SCALARMULDOUBLE_ALT_NOIBT_SUBROUTINE_CORRECT)));;
...
let edwards25519_scalarmuldouble_alt_windows_mc,_ =
...
let edwards25519_scalarmuldouble_alt_windows_tmc =
  define_trimmed "edwards25519_scalarmuldouble_alt_windows_tmc"
                 edwards25519_scalarmuldouble_alt_windows_mc;;

let WINDOWS_EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC =
  X86_MK_EXEC_RULE edwards25519_scalarmuldouble_alt_windows_tmc;;

let EDWARDS25519_SCALARMULDOUBLE_ALT_WINDOWS_MC_BRIDGE = prove
 (`!pc tables. edwards25519_scalarmuldouble_alt_windows_mc pc tables =
     APPEND [word 0xf3:byte; word 0x0f; word 0x1e; word 0xfa]
            (edwards25519_scalarmuldouble_alt_windows_tmc (pc + 4) tables)`,
  REPEAT GEN_TAC THEN
  REWRITE_TAC[edwards25519_scalarmuldouble_alt_windows_mc;
...
let LENGTH_EDWARDS25519_SCALARMULDOUBLE_ALT_WINDOWS_MC = prove
 (`!pc tables. LENGTH (edwards25519_scalarmuldouble_alt_windows_mc pc tables) =
               0x6fe3`,
  REWRITE_TAC[EDWARDS25519_SCALARMULDOUBLE_ALT_WINDOWS_MC_BRIDGE;
              LENGTH_APPEND; LENGTH;
              fst WINDOWS_EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC] THEN ARITH_TAC);;
...
let EDWARDS25519_SCALARMULDOUBLE_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!tables res scalar point bscalar n xy m pc stackpointer returnaddress.
    ...
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
                (edwards25519_scalarmuldouble_alt_windows_tmc pc tables) /\
              read RIP s = word pc /\
              bytes_loaded s (word tables)
                edwards25519_scalarmuldouble_alt_constant_data /\
              read RSP s = stackpointer /\
  ...
  let subth =
    ...
    (X86_SIMD_SHARPEN_RULE
     ...
     (REWRITE_TAC[BYTES_LOADED_DATA;
      ...
      X86_ADD_RETURN_STACK_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC
       (REWRITE_RULE[BYTES_LOADED_DATA;
                     fst EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC]
        EDWARDS25519_SCALARMULDOUBLE_ALT_CORRECT)
       `[RBX; RBP; R12; R13; R14; R15]` 1720)) in
  REWRITE_TAC[BYTES_LOADED_DATA;
              fst WINDOWS_EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC] THEN
...
let EDWARDS25519_SCALARMULDOUBLE_ALT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!tables res scalar point bscalar n xy m pc stackpointer returnaddress.
    ...
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
                (edwards25519_scalarmuldouble_alt_windows_mc pc tables) /\
              read RIP s = word pc /\
              bytes_loaded s (word tables)
                edwards25519_scalarmuldouble_alt_constant_data /\
              read RSP s = stackpointer /\
       ...
       (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
        MAYCHANGE [memory :> bytes(res,64);
                   memory :> bytes(word_sub stackpointer (word 1744),1744)])`,
  REWRITE_TAC[LENGTH_EDWARDS25519_SCALARMULDOUBLE_ALT_WINDOWS_MC] THEN
  MATCH_ACCEPT_TAC(ADD_IBT_RULE
                     ~bridge:(Some EDWARDS25519_SCALARMULDOUBLE_ALT_WINDOWS_MC_BRIDGE)
                     (REWRITE_RULE[fst WINDOWS_EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC]
                       EDWARDS25519_SCALARMULDOUBLE_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT)));;
...
let LOCAL_MODINV_SAFETY_TAC (assump_name:string) (n:int) =
  REMOVE_THEN assump_name (fun safety_th ->
    X86_SUBROUTINE_SIM_TAC ~is_safety_thm:true
        ...
        `pc + 0x1962`; `word_add stackpointer (word 200):int64`] n
    THENL [
      EXISTS_E2_TAC(ref
    ...
    ==> ensures x86
         ...
         (\s. read RIP s = word (pc + 0x3fe1) /\
          ...
          (MAYCHANGE [RIP;  RAX; RBX; RCX; RDX;
           ...
           MAYCHANGE [memory :> bytes(p3,128);
                      memory :> bytes(stackpointer,160)])`,
  PROVE_SAFETY_SPEC_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC);;
 ...
 (`exists f_events.
       forall e tables res scalar point bscalar pc stackpointer.
           ...
           [word pc,28617; word tables,768;
            res,64; scalar,32; point,64; bscalar,32] /\
           ALL (nonoverlapping (res,64)) [word pc,28617; word tables,768]
           ==> ensures x86
               (\s.
  ...
  ENSURES_EVENTS_WHILE_UP2_TAC `63` `pc + 0xe18` `pc + 0x1952`(*+3 insn *)
   `\i s.
      read (memory :> bytes(word tables,768)) s =
      num_of_bytelist edwards25519_scalarmuldouble_alt_constant_data /\
      read RSP s = word_add stackpointer (word 200) /\
  ...
  REPEAT CONJ_TAC THENL [
    ...
    X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (1--87) THEN
    X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (88--(87+9+120(*MUL_4*))) THEN
    ...
    X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (148--155) THEN
    UNDISCH_TAC
     `read RBP s155 =
    ...
    DISCH_TAC THEN
    X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (156--709) THEN
    X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (710--714) THEN
    ...
    X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (1--2) THEN
    LOCAL_MODINV_SAFETY_TAC "H_INV" 3 THEN
    X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (4--(3+129(*MUL_P25519*))) THEN
    X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (5--(4+129(*MUL_P25519*))) THEN
    X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (6--12) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  DISCH_TAC THEN
  X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (24--242) THEN
  X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (243--277) THEN
  X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (278--289) THEN
  X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (290--601) THEN
  X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (602--627) THEN
  X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (628--631) THEN
  ...
  X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (663--664) THEN
  X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (665--665) THEN
  X86_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (666--667) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONJ_TAC THENL [
...
let EDWARDS25519_SCALARMULDOUBLE_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
  ...
  X86_ADD_RETURN_STACK_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC
   (REWRITE_RULE[BYTES_LOADED_DATA; fst EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC]
    EDWARDS25519_SCALARMULDOUBLE_ALT_SAFE)
    `[RBX; RBP; R12; R13; R14; R15]` 1720
...
let EDWARDS25519_SCALARMULDOUBLE_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
  ...
  MATCH_ACCEPT_TAC(ADD_IBT_RULE
                     ~bridge:(Some EDWARDS25519_SCALARMULDOUBLE_ALT_MC_BRIDGE)
                     (REWRITE_RULE[fst EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC]
                       EDWARDS25519_SCALARMULDOUBLE_ALT_NOIBT_SUBROUTINE_SAFE)));;
    ...
    (edwards25519_scalarmuldouble_alt_windows_tmc,
     ...
     0x16,edwards25519_scalarmuldouble_alt_tmc,subth)
        [`e:(uarch_event)list`; `tables:num`;
         ...
         `pc + 0x16`; `read RSP s`; `read (memory :> bytes64 (read RSP s)) s`]
        8) THENL [
    EXISTS_E2_TAC (ref[`tables:num`;`stackpointer:int64`;`pc:num`]);
    ALL_TAC
  ] THEN
  X86_STEPS_TAC WINDOWS_EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (9--11) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[]
...
let EDWARDS25519_SCALARMULDOUBLE_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
  ...
  MATCH_ACCEPT_TAC(ADD_IBT_RULE
                     ~bridge:(Some EDWARDS25519_SCALARMULDOUBLE_ALT_WINDOWS_MC_BRIDGE)
                     (REWRITE_RULE[fst WINDOWS_EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC]
                       EDWARDS25519_SCALARMULDOUBLE_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE)));;
