...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_mod_n521_9_alt" subroutine_signatures)
    BIGNUM_MOD_N521_9_ALT_CORRECT
    BIGNUM_MOD_N521_9_ALT_EXEC;;

let BIGNUM_MOD_N521_9_ALT_SAFE = time prove
 (`exists f_events.
       forall e z x pc.
           nonoverlapping (word pc,299) (z,8 * 9) /\
           (x = z \/ nonoverlapping (x,8 * 9) (z,8 * 9))
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc)
                    (BUTLAST bignum_mod_n521_9_alt_tmc) /\
                    read RIP s = word pc /\
                    C_ARGUMENTS [z; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 298) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_MOD_N521_9_ALT_EXEC);;

let BIGNUM_MOD_N521_9_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mod_n521_9_alt_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             MAYCHANGE [memory :> bignum (z,9)])`,
  X86_PROMOTE_RETURN_NOSTACK_TAC
    bignum_mod_n521_9_alt_tmc BIGNUM_MOD_N521_9_ALT_SAFE
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MOD_N521_9_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_mod_n521_9_alt_mc) (z,8 * 9) /\
        nonoverlapping (stackpointer,8) (z,8 * 9) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mod_n521_9_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE [memory :> bignum (z,9)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MOD_N521_9_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_MOD_N521_9_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z x pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        [word pc,LENGTH bignum_mod_n521_9_alt_windows_tmc; x,8 * 9] /\
        nonoverlapping (word pc,LENGTH bignum_mod_n521_9_alt_windows_tmc)
        (z,8 * 9) /\
        nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 9) /\
        (x = z \/ nonoverlapping (x,8 * 9) (z,8 * 9))
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mod_n521_9_alt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
  ...
  WINDOWS_X86_WRAP_NOSTACK_TAC
    bignum_mod_n521_9_alt_windows_tmc bignum_mod_n521_9_alt_tmc
    BIGNUM_MOD_N521_9_ALT_SAFE
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MOD_N521_9_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z x pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        [word pc,LENGTH bignum_mod_n521_9_alt_windows_mc; x,8 * 9] /\
        nonoverlapping (word pc,LENGTH bignum_mod_n521_9_alt_windows_mc)
        (z,8 * 9) /\
        nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 9) /\
        (x = z \/ nonoverlapping (x,8 * 9) (z,8 * 9))
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mod_n521_9_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,9);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MOD_N521_9_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;