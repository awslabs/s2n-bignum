...
let bignum_bitsize_cmc = define_assert_from_elf "bignum_bitsize_cmc" "x86/generic/bignum_bitsize.o"
[
...
let bignum_bitsize_mc = define_trimmed "bignum_bitsize_mc" bignum_bitsize_cmc;;
...
let BIGNUM_BITSIZE_IBT_SUBROUTINE_CORRECT = prove
 (`!k a x pc stackpointer returnaddress.
        ensures x86
         (\s. bytes_loaded s (word pc) bignum_bitsize_cmc /\
              read RIP s = word pc /\
              read RSP s = stackpointer /\
              read (memory :> bytes64 stackpointer) s = returnaddress /\
              C_ARGUMENTS [k;a] s /\
         ...
         (\s'. read RIP s' = returnaddress /\
               ...
               C_RETURN s' = word(bitsize x))
         (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI)`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_BITSIZE_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_bitsize_cmc = define_from_elf
   "windows_bignum_bitsize_cmc" "x86/generic/bignum_bitsize.obj";;

let windows_bignum_bitsize_mc = define_trimmed "windows_bignum_bitsize_mc" windows_bignum_bitsize_cmc;;
...
let WINDOWS_BIGNUM_BITSIZE_IBT_SUBROUTINE_CORRECT = prove
 (`!k a x pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH windows_bignum_bitsize_cmc); (a,8 * val k)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_bitsize_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [k;a] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_BITSIZE_SUBROUTINE_CORRECT));;
