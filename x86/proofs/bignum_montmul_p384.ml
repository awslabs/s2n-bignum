...
let bignum_montmul_p384_cmc =
  define_assert_from_elf "bignum_montmul_p384_cmc" "x86/p384/bignum_montmul_p384.o"
[
...
let bignum_montmul_p384_mc = define_trimmed "bignum_montmul_p384_mc" bignum_montmul_p384_cmc;;
...
let BIGNUM_MONTMUL_P384_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 48),56) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
            [(word pc,LENGTH bignum_montmul_p384_cmc); (x,8 * 6); (y,8 * 6)] /\
        nonoverlapping (word pc,LENGTH bignum_montmul_p384_cmc) (z,8 * 6)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montmul_p384_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [z; x; y] s /\
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 6);
                       memory :> bytes(word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTMUL_P384_SUBROUTINE_CORRECT));;
...
let BIGNUM_AMONTMUL_P384_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 48),56) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
            [(word pc,LENGTH bignum_montmul_p384_cmc); (x,8 * 6); (y,8 * 6)] /\
        ...
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 6);
                       memory :> bytes(word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_AMONTMUL_P384_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_montmul_p384_cmc = define_from_elf
   "windows_bignum_montmul_p384_cmc" "x86/p384/bignum_montmul_p384.obj";;

let windows_bignum_montmul_p384_mc = define_trimmed "windows_bignum_montmul_p384_mc" windows_bignum_montmul_p384_cmc;;
...
let WINDOWS_BIGNUM_MONTMUL_P384_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
            [(word pc,LENGTH windows_bignum_montmul_p384_cmc); (x,8 * 6); (y,8 * 6)] /\
        nonoverlapping (word pc,LENGTH windows_bignum_montmul_p384_cmc) (z,8 * 6)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_montmul_p384_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x; y] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 6);
                       memory :> bytes(word_sub stackpointer (word 64),64)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_MONTMUL_P384_SUBROUTINE_CORRECT));;
...
let WINDOWS_BIGNUM_AMONTMUL_P384_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
            [(word pc,LENGTH windows_bignum_montmul_p384_cmc); (x,8 * 6); (y,8 * 6)] /\
        ...
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 6);
                       memory :> bytes(word_sub stackpointer (word 64),64)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_AMONTMUL_P384_SUBROUTINE_CORRECT));;
