...
let LOCAL_MUL_P25519_SAFE = time prove
 (`exists f_events.
        forall e z x y pc.
            nonoverlapping (word pc,2478) (z,8 * 4)
            ==> ensures x86
                (\s.
                     bytes_loaded s (word pc) edwards25519_decode_alt_tmc /\
                     read RIP s = word (pc + 1472) /\
                     C_ARGUMENTS [z; x; y] s /\
                     read events s = e)
                (\s.
                     read RIP s = word (pc + 1911) /\
                     (exists e2.
                ...
                (MAYCHANGE
                 ...
                 MAYCHANGE [events])`,
  PROVE_SAFETY_SPEC_TAC EDWARDS25519_DECODE_ALT_EXEC);;
...
let LOCAL_MUL_SAFETY_TAC (assump_name:string) (n:int) =
  REMOVE_THEN assump_name (fun safety_th ->
    X86_SUBROUTINE_SIM_TAC ~is_safety_thm:true
      (edwards25519_decode_alt_tmc,EDWARDS25519_DECODE_ALT_EXEC,
        0x0,edwards25519_decode_alt_tmc,safety_th)
...
let LOCAL_NSQR_P25519_SAFE = time prove
 (`exists f_events.
  ...
  ENSURES_EVENTS_WHILE_UP2_TAC `val (k:int64)` `pc + 0x7a7` `pc + 0x972`(*+1*)
   `\i s. read RSP s = stackpointer /\
          read RDI s = z /\
    ...
    X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (1--17) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    DISCHARGE_SAFETY_PROPERTY_TAC
  ] THEN
  ...
  ENSURES_INIT_TAC "s0" THEN
  STRIP_EXISTS_ASSUM_TAC THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (1--110) THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (111--111) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONJ_TAC THENL [
...
let full_spec,public_vars = mk_safety_spec ~keep_maychanges:true
      ...
      EDWARDS25519_DECODE_ALT_CORRECT
      EDWARDS25519_DECODE_ALT_EXEC;;

let EDWARDS25519_DECODE_ALT_SAFE = time prove
 (`exists f_events.
       forall e z c pc stackpointer.
           ALL (nonoverlapping (stackpointer,264))
           [word pc,2478; z,8 * 8; c,8 * 4] /\
           nonoverlapping (word pc,2478) (z,8 * 8)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) edwards25519_decode_alt_tmc /\
               ...
               (\s.
                    ...
                    (exists e2.
                         ...
                         memaccess_inbounds e2 [c,32; z,64; stackpointer,264]
  ...
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (1--19) THEN
  (* now the second part of ENSURES_SEQUENCE_TAC. *)
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (20--23) THEN
  LOCAL_NSQR_SAFETY_TAC "H_NSQR" 24 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (25--50) THEN
  LOCAL_MUL_SAFETY_TAC "H_MUL" 51 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (52--64) THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (65--68) THEN
  LOCAL_MUL_SAFETY_TAC "H_MUL" 69 THEN

  (*** The power tower. Simply add 100 to the step numbers of fn correctness
       proof... ***)

  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (151--155) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR"  156 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (157--161) THEN LOCAL_MUL_SAFETY_TAC "H_MUL"  162 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (163--167) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR"  168 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (169--173) THEN LOCAL_MUL_SAFETY_TAC "H_MUL"  174 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (175--179) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR"  180 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (181--185) THEN LOCAL_MUL_SAFETY_TAC "H_MUL"  186 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (187--191) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR"  192 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (193--197) THEN LOCAL_MUL_SAFETY_TAC "H_MUL"  198 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (199--203) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR"  204 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (205--209) THEN LOCAL_MUL_SAFETY_TAC "H_MUL"  210 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (211--215) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR"  216 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (217--221) THEN LOCAL_MUL_SAFETY_TAC "H_MUL"  222 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (223--227) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR"  228 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (229--233) THEN LOCAL_MUL_SAFETY_TAC "H_MUL"  234 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (235--239) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR"  240 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (241--245) THEN LOCAL_MUL_SAFETY_TAC "H_MUL"  246 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (247--251) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR"  252 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (253--257) THEN LOCAL_MUL_SAFETY_TAC "H_MUL"  258 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (259--263) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR"  264 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (265--269) THEN LOCAL_MUL_SAFETY_TAC "H_MUL"  270 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (271--275) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR"  276 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (277--281) THEN LOCAL_MUL_SAFETY_TAC "H_MUL"  282 THEN

  (*** e^2 * w computation ***)
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (283--287) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR" 288 THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (289--293) THEN LOCAL_MUL_SAFETY_TAC "H_MUL" 294 THEN

  (*** s = u * e ***)
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (295--299) THEN LOCAL_MUL_SAFETY_TAC "H_MUL" 300 THEN

  (*** t = j_25519 * s ***)
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (301--313) THEN
  LOCAL_MUL_SAFETY_TAC "H_MUL" 314 THEN

  (*** Comparison with 0 or 1 ***)
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (315--325) THEN

  (*** Comparison with -1 ***)
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (326--333) THEN

  (*** Multiplexing of s or t ***)
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (334--350) THEN

  (*** Error indication (so far) ***)
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (351--355) THEN

  (*** Computation of p_25519 - x and all ***)
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (356--367) THEN
  X86_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (368--400) THEN
...
(*
  generate_four_variants_of_x86_safety_specs
    ...
    EDWARDS25519_DECODE_ALT_EXEC
    EDWARDS25519_DECODE_ALT_NOIBT_SUBROUTINE_CORRECT
    EDWARDS25519_DECODE_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT;;
*)
let EDWARDS25519_DECODE_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z c pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 312),312))
        [word pc,LENGTH edwards25519_decode_alt_tmc; z,8 * 8; c,8 * 4] /\
        ALL (nonoverlapping (z,8 * 8))
        [word pc,LENGTH edwards25519_decode_alt_tmc;
        ...
        ==> ensures x86
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 8);
              memory :> bytes (word_sub stackpointer (word 312),312)])`,
  X86_ADD_RETURN_STACK_TAC
    EDWARDS25519_DECODE_ALT_EXEC EDWARDS25519_DECODE_ALT_SAFE
    `[RBX; RBP; R12; R13; R14; R15]` 312
...
let EDWARDS25519_DECODE_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z c pc stackpointer returnaddress.
        ...
        [word pc,LENGTH edwards25519_decode_alt_mc; z,8 * 8; c,8 * 4] /\
        ALL (nonoverlapping (z,8 * 8))
        [word pc,LENGTH edwards25519_decode_alt_mc;
         word_sub stackpointer (word 312),320]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) edwards25519_decode_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; c] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 8);
              memory :> bytes (word_sub stackpointer (word 312),312)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE EDWARDS25519_DECODE_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let EDWARDS25519_DECODE_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z c pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 336),336))
        [word pc,LENGTH edwards25519_decode_alt_windows_tmc; z,8 * 8; c,8 * 4] /\
        ALL (nonoverlapping (z,8 * 8))
        [word pc,LENGTH edwards25519_decode_alt_windows_tmc;
         word_sub stackpointer (word 336),344]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) edwards25519_decode_alt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; c] s /\
  ...
  and subth =
   X86_SIMD_SHARPEN_RULE
    (REWRITE_RULE[fst EDWARDS25519_DECODE_ALT_EXEC]
      EDWARDS25519_DECODE_ALT_NOIBT_SUBROUTINE_SAFE)
    (X86_ADD_RETURN_STACK_TAC
     EDWARDS25519_DECODE_ALT_EXEC EDWARDS25519_DECODE_ALT_SAFE
...
let EDWARDS25519_DECODE_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z c pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 336),336))
        [word pc,LENGTH edwards25519_decode_alt_windows_mc; z,8 * 8; c,8 * 4] /\
        ALL (nonoverlapping (z,8 * 8))
        [word pc,LENGTH edwards25519_decode_alt_windows_mc;
         word_sub stackpointer (word 336),344]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) edwards25519_decode_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; c] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 8);
              memory :> bytes (word_sub stackpointer (word 336),336)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE EDWARDS25519_DECODE_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;