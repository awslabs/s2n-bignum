...
let edwards25519_decode_alt_cmc = define_assert_from_elf "edwards25519_decode_alt_cmc" "x86/curve25519/edwards25519_decode_alt.o"
[
...
let edwards25519_decode_alt_mc = define_trimmed "edwards25519_decode_alt_mc" edwards25519_decode_alt_cmc;;
...
let EDWARDS25519_DECODE_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!z c n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 312),312))
            [(word pc,LENGTH edwards25519_decode_alt_cmc); (z,8 * 8); (c,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 8))
            [(word pc,LENGTH edwards25519_decode_alt_cmc); (word_sub stackpointer (word 312),320)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) edwards25519_decode_alt_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [z; c] s /\
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 8);
                    memory :> bytes(word_sub stackpointer (word 312),312)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE EDWARDS25519_DECODE_ALT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_edwards25519_decode_alt_cmc = define_from_elf
  "windows_edwards25519_decode_alt_cmc"
  "x86/curve25519/edwards25519_decode_alt.obj";;

let windows_edwards25519_decode_alt_mc = define_trimmed "windows_edwards25519_decode_alt_mc" windows_edwards25519_decode_alt_cmc;;
...
let WINDOWS_EDWARDS25519_DECODE_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!z c n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 336),336))
            [(word pc,LENGTH windows_edwards25519_decode_alt_cmc); (z,8 * 8); (c,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 8))
            [(word pc,LENGTH windows_edwards25519_decode_alt_cmc); (word_sub stackpointer (word 336),344)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_edwards25519_decode_alt_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; c] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 8);
                    memory :> bytes(word_sub stackpointer (word 336),336)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_EDWARDS25519_DECODE_ALT_SUBROUTINE_CORRECT));;
