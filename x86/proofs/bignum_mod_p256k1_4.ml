...
let bignum_mod_p256k1_4_cmc =
  define_assert_from_elf "bignum_mod_p256k1_4_cmc" "x86/secp256k1/bignum_mod_p256k1_4.o"
[
...
let bignum_mod_p256k1_4_mc = define_trimmed "bignum_mod_p256k1_4_mc" bignum_mod_p256k1_4_cmc;;
...
let BIGNUM_MOD_P256K1_4_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
      nonoverlapping (word pc,LENGTH bignum_mod_p256k1_4_cmc) (z,8 * 4) /\
      nonoverlapping (stackpointer,8) (z,8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) bignum_mod_p256k1_4_cmc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                C_ARGUMENTS [z; x] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,4)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MOD_P256K1_4_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_mod_p256k1_4_cmc = define_from_elf
   "windows_bignum_mod_p256k1_4_cmc" "x86/secp256k1/bignum_mod_p256k1_4.obj";;

let windows_bignum_mod_p256k1_4_mc = define_trimmed "windows_bignum_mod_p256k1_4_mc" windows_bignum_mod_p256k1_4_cmc;;

let WINDOWS_BIGNUM_MOD_P256K1_4_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH windows_bignum_mod_p256k1_4_mc); (x,8 * 4)] /\
      nonoverlapping (word pc,LENGTH windows_bignum_mod_p256k1_4_mc) (z,8 * 4) /\
...
let WINDOWS_BIGNUM_MOD_P256K1_4_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH windows_bignum_mod_p256k1_4_cmc); (x,8 * 4)] /\
      nonoverlapping (word pc,LENGTH windows_bignum_mod_p256k1_4_cmc) (z,8 * 4) /\
      nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) windows_bignum_mod_p256k1_4_cmc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                WINDOWS_C_ARGUMENTS [z; x] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,4);
                      memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_MOD_P256K1_4_SUBROUTINE_CORRECT));;
