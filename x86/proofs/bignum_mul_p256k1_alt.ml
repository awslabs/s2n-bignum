...
let bignum_mul_p256k1_alt_cmc = define_assert_from_elf "bignum_mul_p256k1_alt_cmc" "x86/secp256k1/bignum_mul_p256k1_alt.o"
[
...
let bignum_mul_p256k1_alt_mc = define_trimmed "bignum_mul_p256k1_alt_mc" bignum_mul_p256k1_alt_cmc;;
...
let BIGNUM_MUL_P256K1_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x y m n pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_mul_p256k1_alt_cmc) (z,8 * 4) /\
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 32),40) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),32))
            [(word pc,LENGTH bignum_mul_p256k1_alt_cmc); (x,8 * 4); (y,8 * 4)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_mul_p256k1_alt_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [z; x; y] s /\
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                     memory :> bytes(word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MUL_P256K1_ALT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_mul_p256k1_alt_cmc = define_from_elf
   "windows_bignum_mul_p256k1_alt_cmc" "x86/secp256k1/bignum_mul_p256k1_alt.obj";;

let windows_bignum_mul_p256k1_alt_mc = define_trimmed "windows_bignum_mul_p256k1_alt_mc" windows_bignum_mul_p256k1_alt_cmc;;

let WINDOWS_BIGNUM_MUL_P256K1_ALT_SUBROUTINE_CORRECT = time prove
 (`!z x y m n pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH windows_bignum_mul_p256k1_alt_mc) (z,8 * 4) /\
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 48),56) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
            [(word pc,LENGTH windows_bignum_mul_p256k1_alt_mc); (x,8 * 4); (y,8 * 4)]
        ==> ensures x86
...
let WINDOWS_BIGNUM_MUL_P256K1_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x y m n pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH windows_bignum_mul_p256k1_alt_cmc) (z,8 * 4) /\
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 48),56) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
            [(word pc,LENGTH windows_bignum_mul_p256k1_alt_cmc); (x,8 * 4); (y,8 * 4)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_mul_p256k1_alt_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x; y] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                     memory :> bytes(word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_MUL_P256K1_ALT_SUBROUTINE_CORRECT));;
