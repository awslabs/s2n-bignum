...
let LOCAL_WORD_DIVSTEP59_SAFE = prove
 (`exists f_events. forall e stackpointer pc.
    nonoverlapping (stackpointer,240) (word pc,0x156a)
    ==> ensures x86
        (\s. bytes_loaded s (word pc)
                  (TRIM_LIST (17,18) bignum_montinv_p256_tmc) /\
        ...
        (\s. read RIP s = word(pc + 0x132d) /\
             (exists e2.
                ...
                memaccess_inbounds e2 [stackpointer,240] [stackpointer,240]))
        (MAYCHANGE [RIP; RDI; RSI; RAX; RBX; RCX; RDX; RBP;
                  R8; R9; R10; R11; R12; R13; R14; R15] ,,
        MAYCHANGE [memory :> bytes(word_add stackpointer (word 192),32)] ,,
        MAYCHANGE SOME_FLAGS ,, MAYCHANGE [events])`,

  PROVE_SAFETY_SPEC_TAC CORE_MONTINV_P256_EXEC);;

let CORE_MONTINV_P256_SAFE = time prove
 (`exists f_events. forall e z x pc stackpointer.
        ...
        ==> ensures x86
             ...
             (\s. read RIP s = word (pc + 0x156a) /\
                ...
                memaccess_inbounds e2
                  [x,32; z,32; stackpointer,240]
                  [z,32; stackpointer,240]))
          (MAYCHANGE [RIP; RDI; RSI; RAX; RBX; RCX; RDX; RBP;
                      R8; R9; R10; R11; R12; R13; R14; R15] ,,
           MAYCHANGE SOME_FLAGS ,, MAYCHANGE [events] ,,
           MAYCHANGE [memory :> bytes(z,8 * 4);
  ...
  ENSURES_EVENTS_WHILE_UP2_TAC `9` `pc + 0x60b` `pc + 0x60b`
   `\i s.
      read RSP s = stackpointer /\
      read (memory :> bytes64(word_add stackpointer (word 224))) s = z /\
      read (memory :> bytes64(word_add stackpointer (word 176))) s =
  ...
  THEN REPEAT CONJ_TAC THENL [
    ...
    X86_SIM_TAC ~preprocess_tac:(TRY STRIP_EXISTS_ASSUM_TAC)
        ~canonicalize_pc_diff:false CORE_MONTINV_P256_EXEC (1--49) THEN
    ...
    USE_THEN "H_DIVSTEP59_SAFE"
      (MP_TAC o SPECL [`e:(uarch_event)list`;`stackpointer:int64`;`pc:num`]) THEN
    ...
    ANTS_TAC THENL [FIRST_ASSUM MATCH_ACCEPT_TAC; ALL_TAC] THEN
    X86_BIGSTEP_TAC CORE_MONTINV_P256_EXEC "s1" THENL [
    ...
    X86_STEPS_TAC CORE_MONTINV_P256_EXEC (5--328) THEN
  ...
  X86_STEPS_TAC CORE_MONTINV_P256_EXEC (2--142) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  (* since automation of DISCHARGE_SAFETY_PROPERTY_TAC is insufficient,
      manually write the proof here *)
  SAFE_META_EXISTS_TAC allowed_vars_e THEN
...
let BIGNUM_MONTINV_P256_SAFE = time prove
 (`exists f_events. forall e z x pc stackpointer.
        ALL (nonoverlapping (stackpointer,240))
            [(word pc,0x158d); (z,8 * 4); (x,8 * 4)] /\
        nonoverlapping (word pc,0x158d) (z,8 * 4)
        ==> ensures x86
          ...
          (MAYCHANGE [RIP; RDI; RSI; RAX; RBX; RCX; RDX; RBP;
           ...
           MAYCHANGE [memory :> bytes(z,8 * 4);
                      memory :> bytes(stackpointer,240)])`,
  ASSUME_CALLEE_SAFETY_TAC CORE_MONTINV_P256_SAFE "H_CORE_SAFE" THEN
  META_EXISTS_TAC THEN STRIP_TAC THEN

  MAP_EVERY X_GEN_TAC [`z:int64`; `x:int64`; `pc:num`; `stackpointer:int64`] THEN
  REWRITE_TAC[ALLPAIRS; ALL] THEN
  REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS] THEN
  DISCH_THEN(REPEAT_TCL CONJUNCTS_THEN ASSUME_TAC) THEN
  REWRITE_TAC(!simulation_precanon_thms) THEN ENSURES_INIT_TAC "s0" THEN
  REMOVE_THEN "H_CORE_SAFE" (fun safe_th ->
    X86_SUBROUTINE_SIM_TAC
      ((GEN_REWRITE_CONV RAND_CONV [bignum_montinv_p256_tmc] THENC
        REWRITE_CONV[BUTLAST_CLAUSES])
       `BUTLAST bignum_montinv_p256_tmc`,
...
let BIGNUM_MONTINV_P256_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events. forall e z x pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 288),288))
            [(word pc,LENGTH bignum_montinv_p256_tmc); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word_sub stackpointer (word 288),296); (word pc,LENGTH bignum_montinv_p256_tmc)]
        ==> ensures x86
             ...
             (\s. read RIP s = returnaddress /\
                  ...
                  (exists e2.
                    read events s = APPEND e2 e /\
                    e2 = f_events x z pc (word_sub stackpointer (word 288))
                                  returnaddress /\
                    memaccess_inbounds e2
                      [x,32; z,32;
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                    memory :> bytes(word_sub stackpointer (word 288),288)])`,
  X86_PROMOTE_RETURN_STACK_TAC bignum_montinv_p256_tmc
     BIGNUM_MONTINV_P256_SAFE
      `[RBX; RBP; R12; R13; R14; R15]` 288 THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MONTINV_P256_SUBROUTINE_SAFE = time prove
 (`exists f_events. forall e z x pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 288),288))
            [(word pc,LENGTH bignum_montinv_p256_mc); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
        ...
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                    memory :> bytes(word_sub stackpointer (word 288),288)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTINV_P256_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_MONTINV_P256_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events. forall e z x pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 304),304))
            [(word pc,LENGTH bignum_montinv_p256_windows_tmc); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
        ...
        ==> ensures x86
             ...
             (\s. read RIP s = returnaddress /\
                  ...
                  (exists e2.
                    read events s = APPEND e2 e /\
                    e2 = f_events x z pc (word_sub stackpointer (word 304))
                                  returnaddress /\
                    memaccess_inbounds e2
                      [x,32; z,32;
                       (word_sub stackpointer (word 304)),312]
                      [z,32;
...
let BIGNUM_MONTINV_P256_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events. forall e z x pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 304),304))
            [(word pc,LENGTH bignum_montinv_p256_windows_mc); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
        ...
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                    memory :> bytes(word_sub stackpointer (word 304),304)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTINV_P256_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
