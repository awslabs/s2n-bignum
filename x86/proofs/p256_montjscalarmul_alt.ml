...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "p256_montjscalarmul_alt" subroutine_signatures)
    P256_MONTJSCALARMUL_ALT_CORRECT
    P256_MONTJSCALARMUL_ALT_EXEC;;

let P256_MONTJSCALARMUL_ALT_SAFE = time prove
 (`exists f_events.
       forall e res scalar point pc stackpointer.
           ALL (nonoverlapping (stackpointer,1320))
           [word pc,18350; res,96; scalar,32; point,96] /\
           nonoverlapping (res,96) (word pc,18350)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) p256_montjscalarmul_alt_tmc /\
                    read RIP s = word (pc + 17) /\
                    read RSP s = word_add stackpointer (word 296) /\
                    C_ARGUMENTS [res; scalar; point] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 2923) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events scalar point res pc stackpointer /\
                         memaccess_inbounds e2
                         [scalar,32; point,96; res,96; stackpointer,1320]
                         [res,96; stackpointer,1320]))
               (MAYCHANGE [RIP] ,,
                MAYCHANGE [RAX; RCX; RDX; RSI; RDI; R8; R9; R10; R11] ,,
  ...
  ASSUME_CALLEE_SAFETY_TAILED_TAC
      (X86_SIMD_SHARPEN_RULE P256_MONTJADD_ALT_NOIBT_SUBROUTINE_SAFE
        (X86_PROMOTE_RETURN_STACK_TAC p256_montjadd_alt_tmc P256_MONTJADD_ALT_SAFE
          `[RBX; RBP; R12; R13; R14; R15]` 272 THEN
        DISCHARGE_SAFETY_PROPERTY_TAC))
      "H_JADD_SAFE" THEN
  ASSUME_CALLEE_SAFETY_TAILED_TAC
      (X86_SIMD_SHARPEN_RULE P256_MONTJDOUBLE_ALT_NOIBT_SUBROUTINE_SAFE
        (X86_PROMOTE_RETURN_STACK_TAC p256_montjdouble_alt_tmc
          P256_MONTJDOUBLE_ALT_SAFE `[RBX; R12; R13; R14; R15]` 232 THEN
        DISCHARGE_SAFETY_PROPERTY_TAC))
      "H_JDOUBLE_SAFE" THEN
  ...
  ENSURES_EVENTS_WHILE_UP2_TAC `63` `pc + 0x62d`
    `pc + 0xaf8`(*after jne*)
    `\i s.
      read RSP s = word_add stackpointer (word 296) /\
      read (memory :> bytes64(word_add stackpointer (word 1288))) s = res /\
      read RBP s = word (4 * (63 - i))` THEN
  REPEAT CONJ_TAC THENL [
    ARITH_TAC;
    ...
    X86_STEPS_TAC P256_MONTJSCALARMUL_ALT_EXEC (1--85) THEN
    LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 86 THEN
    X86_STEPS_TAC P256_MONTJSCALARMUL_ALT_EXEC (87--90) THEN
    LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 91 THEN
    X86_STEPS_TAC P256_MONTJSCALARMUL_ALT_EXEC (92--94) THEN
    LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 95 THEN
    X86_STEPS_TAC P256_MONTJSCALARMUL_ALT_EXEC (96--99) THEN
    LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 100 THEN
    X86_STEPS_TAC P256_MONTJSCALARMUL_ALT_EXEC (101--103) THEN
    ...
    X86_STEPS_TAC P256_MONTJSCALARMUL_ALT_EXEC (1--25) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    DISCHARGE_SAFETY_PROPERTY_TAC
  ] THEN
  ...
  X86_STEPS_TAC P256_MONTJSCALARMUL_ALT_EXEC (6--8) THEN
  LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 9 THEN
  X86_STEPS_TAC P256_MONTJSCALARMUL_ALT_EXEC (10--12) THEN
  LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 13 THEN
  X86_STEPS_TAC P256_MONTJSCALARMUL_ALT_EXEC (14--16) THEN
  LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 17 THEN
  X86_STEPS_TAC P256_MONTJSCALARMUL_ALT_EXEC (18--27) THEN
  X86_STEPS_TAC P256_MONTJSCALARMUL_ALT_EXEC (28--143) THEN
  X86_STEPS_TAC P256_MONTJSCALARMUL_ALT_EXEC (144--175) THEN
  X86_STEPS_TAC P256_MONTJSCALARMUL_ALT_EXEC (176--179) THEN
  LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 180 THEN
  X86_STEPS_TAC P256_MONTJSCALARMUL_ALT_EXEC (181--182) THEN
...
let P256_MONTJSCALARMUL_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 1368),1368))
        [word pc,LENGTH p256_montjscalarmul_alt_tmc; scalar,32; point,96] /\
        ALL (nonoverlapping (res,96))
        [word pc,LENGTH p256_montjscalarmul_alt_tmc;
         word_sub stackpointer (word 1368),1376]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjscalarmul_alt_tmc /\
                 read RIP s = word pc /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      (word_sub stackpointer (word 1368))
                      returnaddress /\
                      memaccess_inbounds e2
                      [scalar,32; point,96; res,96;
                       word_sub stackpointer (word 1368),1376]
                      [res,96; word_sub stackpointer (word 1368),1368]))
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (res,96);
              memory :> bytes (word_sub stackpointer (word 1368),1368)])`,
  X86_ADD_RETURN_STACK_TAC P256_MONTJSCALARMUL_ALT_EXEC
    P256_MONTJSCALARMUL_ALT_SAFE `[RBX; RBP; R12; R13; R14; R15]` 1368
...
let P256_MONTJSCALARMUL_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ...
        [word pc,LENGTH p256_montjscalarmul_alt_mc; scalar,32; point,96] /\
        ALL (nonoverlapping (res,96))
        [word pc,LENGTH p256_montjscalarmul_alt_mc;
         word_sub stackpointer (word 1368),1376]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjscalarmul_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [res; scalar; point] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (res,96);
              memory :> bytes (word_sub stackpointer (word 1368),1368)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P256_MONTJSCALARMUL_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let P256_MONTJSCALARMUL_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 1392),1392))
        [word pc,LENGTH p256_montjscalarmul_alt_windows_tmc; scalar,32;
         point,96] /\
        ALL (nonoverlapping (res,96))
        [word pc,LENGTH p256_montjscalarmul_alt_windows_tmc;
         word_sub stackpointer (word 1392),1400]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjscalarmul_alt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [res; scalar; point] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      e2 =
                      f_events scalar point res pc
                      (word_sub stackpointer (word 1392))
                      returnaddress /\
                      memaccess_inbounds e2
                      [scalar,32; point,96; res,96;
                       word_sub stackpointer (word 1392),1400]
                      [res,96; word_sub stackpointer (word 1392),1392]))
            (MAYCHANGE [RSP] ,,
  ...
  let WINDOWS_P256_MONTJSCALARMUL_ALT_EXEC =
    X86_MK_EXEC_RULE p256_montjscalarmul_alt_windows_tmc
  and baseth =
    X86_SIMD_SHARPEN_RULE P256_MONTJSCALARMUL_ALT_NOIBT_SUBROUTINE_SAFE
    (X86_ADD_RETURN_STACK_TAC P256_MONTJSCALARMUL_ALT_EXEC
     P256_MONTJSCALARMUL_ALT_SAFE `[RBX; RBP; R12; R13; R14; R15]` 1368 THEN
  ...
  REWRITE_TAC[fst WINDOWS_P256_MONTJSCALARMUL_ALT_EXEC] THEN
  REPLICATE_TAC 4 GEN_TAC THEN WORD_FORALL_OFFSET_TAC 1392 THEN
  REWRITE_TAC[ALL; WINDOWS_C_ARGUMENTS; SOME_FLAGS;
              WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI] THEN
  REWRITE_TAC[NONOVERLAPPING_CLAUSES] THEN REPEAT STRIP_TAC THEN
  ENSURES_PRESERVED_TAC "rsi_init" `RSI` THEN
...
let P256_MONTJSCALARMUL_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 1392),1392))
        [word pc,LENGTH p256_montjscalarmul_alt_windows_mc; scalar,32;
         point,96] /\
        ALL (nonoverlapping (res,96))
        [word pc,LENGTH p256_montjscalarmul_alt_windows_mc;
         word_sub stackpointer (word 1392),1400]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjscalarmul_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [res; scalar; point] s /\
  ...
  MATCH_ACCEPT_TAC(ADD_IBT_RULE
    P256_MONTJSCALARMUL_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;