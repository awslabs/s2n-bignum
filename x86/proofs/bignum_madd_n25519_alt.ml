...
let full_spec,public_vars = mk_safety_spec
    ...
    BIGNUM_MADD_N25519_ALT_CORRECT
    BIGNUM_MADD_N25519_ALT_EXEC;;

let BIGNUM_MADD_N25519_ALT_SAFE = time prove
 (`exists f_events.
       forall e z x y c pc.
           nonoverlapping (word pc,1002) (z,8 * 4)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc)
                    (BUTLAST bignum_madd_n25519_alt_tmc) /\
                    read RIP s = word (pc + 10) /\
                    C_ARGUMENTS [z; x; y; c] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 991) /\
                    (exists e2.
...
let BIGNUM_MADD_N25519_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z x y c pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_madd_n25519_alt_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x; y; c] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,4);
              memory :> bytes (word_sub stackpointer (word 48),48)])`,
  X86_PROMOTE_RETURN_STACK_TAC
    bignum_madd_n25519_alt_tmc BIGNUM_MADD_N25519_ALT_SAFE
...
let BIGNUM_MADD_N25519_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z x y c pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_madd_n25519_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x; y; c] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,4);
              memory :> bytes (word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MADD_N25519_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_MADD_N25519_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z x y c pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 64),72) (z,8 * 4) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
        [word pc,LENGTH bignum_madd_n25519_alt_windows_tmc; x,8 * 4;
         y,8 * 4; c,8 * 4] /\
        nonoverlapping (word pc,LENGTH bignum_madd_n25519_alt_windows_tmc)
        (z,8 * 4)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_madd_n25519_alt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x; y; c] s /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
    bignum_madd_n25519_alt_windows_tmc bignum_madd_n25519_alt_tmc
    BIGNUM_MADD_N25519_ALT_SAFE `[RBX;RBP;R12;R13;R14;R15]` 48
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MADD_N25519_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z x y c pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 64),72) (z,8 * 4) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
        [word pc,LENGTH bignum_madd_n25519_alt_windows_mc; x,8 * 4; y,8 * 4;
         c,8 * 4] /\
        nonoverlapping (word pc,LENGTH bignum_madd_n25519_alt_windows_mc)
        (z,8 * 4)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_madd_n25519_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x; y; c] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,4);
              memory :> bytes (word_sub stackpointer (word 64),64)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MADD_N25519_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;