...
let bignum_ge_cmc =
  define_assert_from_elf "bignum_ge_cmc" "x86/generic/bignum_ge.o"
[
...
let bignum_ge_mc = define_trimmed "bignum_ge_mc" bignum_ge_cmc;;
...
let BIGNUM_GE_IBT_SUBROUTINE_CORRECT = prove
 (`!m a x n b y pc stackpointer returnaddress.
        ensures x86
          (\s. bytes_loaded s (word pc) bignum_ge_cmc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               C_ARGUMENTS [m;a;n;b] s /\
          ...
          (\s'. read RIP s' = returnaddress /\
                ...
                C_RETURN s' = if x >= y then word 1 else word 0)
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI)`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_GE_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_ge_cmc = define_from_elf
   "windows_bignum_ge_cmc" "x86/generic/bignum_ge.obj";;

let windows_bignum_ge_mc = define_trimmed "windows_bignum_ge_mc" windows_bignum_ge_cmc;;
...
let WINDOWS_BIGNUM_GE_SUBROUTINE_CORRECT = prove
 (`!m a x n b y pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH windows_bignum_ge_mc); (a,8 * val m); (b,8 * val n)]
        ==> ensures x86
              (\s. bytes_loaded s (word pc) windows_bignum_ge_mc /\
                   read RIP s = word pc /\
                   read RSP s = stackpointer /\
...
let WINDOWS_BIGNUM_GE_IBT_SUBROUTINE_CORRECT = prove
 (`!m a x n b y pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH windows_bignum_ge_cmc); (a,8 * val m); (b,8 * val n)]
        ==> ensures x86
              (\s. bytes_loaded s (word pc) windows_bignum_ge_cmc /\
                   read RIP s = word pc /\
                   read RSP s = stackpointer /\
                   read (memory :> bytes64 stackpointer) s = returnaddress /\
                   WINDOWS_C_ARGUMENTS [m;a;n;b] s /\
              ...
              (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_GE_SUBROUTINE_CORRECT));;
