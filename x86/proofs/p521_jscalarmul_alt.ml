...
(* Autogenerated with:
    ...
    P521_JSCALARMUL_ALT_EXEC
  .. and adds extra `read RSP s = ...` to the postcondition and
  `MAYCHANGE [RSP] to the frame condition *)
let LOCAL_JDOUBLE_SAFE = time prove
 (`exists f_events.
        forall e p3 p1 pc stackpointer.
            ALL (nonoverlapping (stackpointer,600))
            [word pc,16210; p1,216; p3,216] /\
            nonoverlapping (p3,216) (word pc,16210)
            ==> ensures x86
                (\s.
                     bytes_loaded s (word pc) p521_jscalarmul_alt_tmc /\
                     read RIP s = word (pc + 10477) /\
                     read RSP s = word_add stackpointer (word 80) /\
                     C_ARGUMENTS [p3; p1] s /\
                     read events s = e)
                (\s.
                     read RIP s = word (pc + 13240) /\
                     read RSP s = word_add stackpointer (word 80) /\
                     (exists e2.
                          read events s = APPEND e2 e /\
                          e2 = f_events p1 p3 pc stackpointer /\
                          memaccess_inbounds e2
                          [p1,216; p3,216; stackpointer,600]
                          [p3,216; stackpointer,600]))
                (MAYCHANGE
                 [RIP; RDI; RSI; RAX; RBX; RBP; RCX; RDX; R8; R9; R10; R11;
                  R12; R13; R14; R15] ,,
                 MAYCHANGE SOME_FLAGS ,,
                 MAYCHANGE [events] ,, MAYCHANGE [RSP] ,,
                 MAYCHANGE
                 [memory :> bytes (p3,216);
                  memory :> bytes (stackpointer,600)])`,
  PROVE_SAFETY_SPEC_TAC P521_JSCALARMUL_ALT_EXEC);;

let LOCAL_JDOUBLE_SUBR_SAFE = time prove
 (`exists f_events.
        forall e p3 p1 pc stackpointer returnaddress.
            ALL (nonoverlapping (word_sub stackpointer (word 648),648))
              [word pc,16210; p1,216] /\
            ALL (nonoverlapping (p3,216))
              [word pc,16210; word_sub stackpointer (word 648),656]
            ==> ensures x86
                (\s.
                     bytes_loaded s (word pc) p521_jscalarmul_alt_tmc /\
                     read RIP s = word (pc + 10460) /\
                     read RSP s = stackpointer /\
                     read (memory :> bytes64 stackpointer) s = returnaddress /\
                     C_ARGUMENTS [p3; p1] s /\
                     read events s = e)
                (\s.
                     ...
                     (exists e2.
                          ...
                          e2 = f_events p1 p3 pc
                              (word_sub stackpointer (word 648))
                              returnaddress /\
                          memaccess_inbounds e2
                            [p1,216; p3,216;
                              word_sub stackpointer (word 648),656]
                            [p3,216;
                              word_sub stackpointer (word 648),648]))
                (MAYCHANGE [RSP] ,,
                 MAYCHANGE [RIP] ,,
                 MAYCHANGE [RAX; RCX; RDX; RSI; RDI; R8; R9; R10; R11] ,,
                 ...
                 MAYCHANGE
                    [memory :> bytes (p3,216);
                     memory :> bytes (word_sub stackpointer (word 648),648)])`,
  X86_ADD_RETURN_STACK_TAC P521_JSCALARMUL_ALT_EXEC LOCAL_JDOUBLE_SAFE
    `[RBX; RBP; R12; R13; R14; R15]` 648 THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

(* Autogenerated with:
    mk_safety_spec
    ~keep_maychanges:true
    (assoc "p521_jadd_alt" subroutine_signatures)
    LOCAL_JADD_CORRECT
    P521_JSCALARMUL_ALT_EXEC
  .. and adds extra `read RSP s = ...` to the postcondition and
  `MAYCHANGE [RSP] to the frame condition *)
let LOCAL_JADD_SAFE = time prove
 (`exists f_events.
        forall e p3 p1 p2 pc stackpointer.
            ALL (nonoverlapping (stackpointer,608))
            [word pc,16210; p1,216; p2,216; p3,216] /\
            nonoverlapping (p3,216) (word pc,16210)
            ==> ensures x86
                (\s.
                     bytes_loaded s (word pc) p521_jscalarmul_alt_tmc /\
                     read RIP s = word (pc + 7167) /\
                     read RSP s = word_add stackpointer (word 80) /\
                     C_ARGUMENTS [p3; p1; p2] s /\
                     read events s = e)
                (\s.
                     read RIP s = word (pc + 10442) /\
                     read RSP s = word_add stackpointer (word 80) /\
                     (exists e2.
                          read events s = APPEND e2 e /\
                          e2 = f_events p1 p2 p3 pc stackpointer /\
                          memaccess_inbounds e2
                          [p1,216; p2,216; p3,216; stackpointer,608]
                          [p3,216; stackpointer,608]))
                (MAYCHANGE
                 [RIP; RDI; RSI; RAX; RBP; RBX; RCX; RDX; RBP; R8; R9; R10;
                  R11; R12; R13; R14; R15] ,,
                 MAYCHANGE SOME_FLAGS ,,
                 MAYCHANGE [events] ,, MAYCHANGE [RSP] ,,
                 MAYCHANGE
...
let LOCAL_JADD_SUBR_SAFE = time prove
 (`exists f_events.
        forall e p3 p1 p2 pc stackpointer returnaddress.
            ALL (nonoverlapping (word_sub stackpointer (word 656),656))
              [word pc,16210; p1,216; p2,216] /\
            ALL (nonoverlapping (p3,216))
              [word pc,16210; word_sub stackpointer (word 656),664]
            ==> ensures x86
                (\s.
                     bytes_loaded s (word pc) p521_jscalarmul_alt_tmc /\
                     read RIP s = word (pc + 7150) /\
                     read RSP s = stackpointer /\
                     read (memory :> bytes64 stackpointer) s = returnaddress /\
                     C_ARGUMENTS [p3; p1; p2] s /\
                     read events s = e)
                (\s.
                     ...
                     (exists e2.
                          ...
                          e2 = f_events p1 p2 p3 pc
                              (word_sub stackpointer (word 656))
                              returnaddress /\
                          memaccess_inbounds e2
                          [p1,216; p2,216; p3,216;
                            word_sub stackpointer (word 656),664]
                          [p3,216;
                            word_sub stackpointer (word 656),656]))
                (MAYCHANGE [RSP] ,,
                MAYCHANGE [RIP] ,,
                MAYCHANGE [RAX; RCX; RDX; RSI; RDI; R8; R9; R10; R11] ,,
                MAYCHANGE [CF; PF; AF; ZF; SF; OF] ,,
                ...
                MAYCHANGE
                  [memory :> bytes (p3,216);
                memory :> bytes (word_sub stackpointer (word 656),656)])`,
  X86_ADD_RETURN_STACK_TAC P521_JSCALARMUL_ALT_EXEC LOCAL_JADD_SAFE
    `[RBX; RBP; R12; R13; R14; R15]` 656 THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;
...
let LOCAL_JADD_SAFETY_TAC (assump_name:string) (n:int) =
  REMOVE_THEN assump_name (fun safety_th ->
    ...
    (p521_jscalarmul_alt_tmc,P521_JSCALARMUL_ALT_EXEC,
      0x0,p521_jscalarmul_alt_tmc,safety_th)
    [`e:(uarch_event)list`; `read RDI s`; `read RSI s`;
    `read RDX s`; `pc:num`; `read RSP s`;
    `read (memory :> bytes64(read RSP s)) s`] n
    THENL [
      EXISTS_E2_TAC(ref
...
let LOCAL_JDOUBLE_SAFETY_TAC (assump_name:string) (n:int) =
  REMOVE_THEN assump_name (fun safety_th ->
    ...
    (p521_jscalarmul_alt_tmc,P521_JSCALARMUL_ALT_EXEC,
      0x0,p521_jscalarmul_alt_tmc,safety_th)
    [`e:(uarch_event)list`; `read RDI s`; `read RSI s`;
     `pc:num`; `read RSP s`; `read (memory :> bytes64(read RSP s)) s`] n
    THENL [
      EXISTS_E2_TAC(ref
        [`scalar:int64`;`point:int64`;`res:int64`;`pc:num`;
         `stackpointer:int64`;
...
let LOCAL_MOD_N521_SAFETY_TAC (assump_name:string) (n:int) =
  REMOVE_THEN assump_name (fun safety_th ->
    X86_SUBROUTINE_SIM_TAC ~is_safety_thm:true
    (p521_jscalarmul_alt_tmc,P521_JSCALARMUL_ALT_EXEC,
      0x1ac3,bignum_mod_n521_9_alt_tmc,safety_th)
    [`e:(uarch_event)list`; `read RDI s`; `read RSI s`;
    `pc + 0x1ac3`; `read RSP s`; `read (memory :> bytes64(read RSP s)) s`] n
    THENL [
      EXISTS_E2_TAC(ref
        [`scalar:int64`;`point:int64`;`res:int64`;`pc:num`;
         `stackpointer:int64`;
...
let LOCAL_MOD_P521_SAFETY_TAC (assump_name:string) (n:int) =
  REMOVE_THEN assump_name (fun safety_th ->
    X86_SUBROUTINE_SIM_TAC ~is_safety_thm:true
    (p521_jscalarmul_alt_tmc,P521_JSCALARMUL_ALT_EXEC,
      0x1a1b,bignum_mod_p521_9_tmc,safety_th)
    [`e:(uarch_event)list`; `read RDI s`; `read RSI s`;
    `pc + 0x1a1b`; `read RSP s`; `read (memory :> bytes64(read RSP s)) s`] n
    THENL [
      EXISTS_E2_TAC(ref
        [`scalar:int64`;`point:int64`;`res:int64`;`pc:num`;
         `stackpointer:int64`;
...
let P521_JSCALARMUL_ALT_SAFE = time prove
 (`exists f_events.
       forall e res scalar point pc stackpointer.
           ALL (nonoverlapping (stackpointer,4696))
           [word pc,16210; res,216; scalar,72; point,216] /\
           nonoverlapping (res,216) (word pc,16210)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) p521_jscalarmul_alt_tmc /\
                    read RIP s = word (pc + 0x11) /\
                    ...
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 0x1a09) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events scalar point res pc stackpointer /\
                         memaccess_inbounds e2
                         [scalar,72; point,216; res,216; stackpointer,4696]
                         [res,216; stackpointer,4696]))
               (MAYCHANGE [RIP] ,,
                MAYCHANGE [RAX; RCX; RDX; RSI; RDI; R8; R9; R10; R11] ,,
                MAYCHANGE [CF; PF; AF; ZF; SF; OF] ,,
                MAYCHANGE [events] ,,
                MAYCHANGE [RBX; RBP; R12; R13; R14; R15] ,,
                MAYCHANGE
                [memory :> bytes (res,216);
                 memory :> bytes (stackpointer,4696)])`,

  ASSERT_CONCL_TAC full_spec THEN

  ASSUME_CALLEE_SAFETY_TAILED_TAC
    (X86_SIMD_SHARPEN_RULE BIGNUM_MOD_N521_9_ALT_NOIBT_SUBROUTINE_SAFE
      (X86_PROMOTE_RETURN_NOSTACK_TAC
        bignum_mod_n521_9_alt_tmc BIGNUM_MOD_N521_9_ALT_SAFE THEN
      DISCHARGE_SAFETY_PROPERTY_TAC))
    "H_MOD_N521_9_SAFE" THEN
  ASSUME_CALLEE_SAFETY_TAILED_TAC
    (X86_SIMD_SHARPEN_RULE BIGNUM_MOD_P521_9_NOIBT_SUBROUTINE_SAFE
      (X86_PROMOTE_RETURN_STACK_TAC bignum_mod_p521_9_tmc BIGNUM_MOD_P521_9_SAFE
      `[RBX]` 8 THEN
      DISCHARGE_SAFETY_PROPERTY_TAC))
    "H_MOD_P521_9_SAFE" THEN
  ASSUME_CALLEE_SAFETY_TAILED_TAC
    (CONV_RULE(ONCE_DEPTH_CONV NUM_MULT_CONV)
      (REWRITE_RULE[bignum_triple_from_memory; bignum_pair_from_memory]
       LOCAL_JADD_SUBR_SAFE))
    "H_JADD_SAFE" THEN
  ASSUME_CALLEE_SAFETY_TAILED_TAC
    (CONV_RULE(ONCE_DEPTH_CONV NUM_MULT_CONV)
      (REWRITE_RULE[bignum_triple_from_memory; bignum_pair_from_memory]
       LOCAL_JDOUBLE_SUBR_SAFE))
    "H_JDOUBLE_SAFE" THEN
  ...
  CONCRETIZE_F_EVENTS_TAC
    `\(scalar:int64) (point:int64) (res:int64) (pc:num) (stackpointer:int64).
      APPEND
        ...
        (APPEND
          (ENUMERATEL 104 (f_ev_loop scalar point res pc stackpointer))
  ...
  (*** Main loop invariant setup. ***)
  ENSURES_EVENTS_WHILE_UP2_TAC `104` `pc + 0x654` `pc + 6322`(*after jne*)
    `\i s.
      read RSP s = word_add stackpointer (word 664) /\
      read (memory :> bytes64(word_add stackpointer (word 4624))) s = res /\
      read RBP s = word (5 * (104 - i))` THEN
  REPEAT CONJ_TAC THENL [
    ARITH_TAC;
    ...
    ENSURES_INIT_TAC "s0" THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (1--4) THEN
    LOCAL_MOD_N521_SAFETY_TAC "H_MOD_N521_9_SAFE" 5 THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (6--8) THEN
    LOCAL_MOD_P521_SAFETY_TAC "H_MOD_P521_9_SAFE" 9 THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (10--12) THEN
    LOCAL_MOD_P521_SAFETY_TAC "H_MOD_P521_9_SAFE" 13 THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (14--16) THEN
    LOCAL_MOD_P521_SAFETY_TAC "H_MOD_P521_9_SAFE" 17 THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (18--100) THEN
    LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 101 THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (102--105) THEN
    LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 106 THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (107--109) THEN
    LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 110 THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (111--114) THEN
    LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 115 THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (116--118) THEN
    LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 119 THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (120--123) THEN
    LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 124 THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (125--127) THEN
    LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 128 THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (129--132) THEN
    LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 133 THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (134--136) THEN
    LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 137 THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (138--141) THEN
    LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 142 THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (143--145) THEN
    LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 146 THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (147--150) THEN
    LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 151 THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (152--154) THEN
    LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 155 THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (156--159) THEN
    LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 160 THEN
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (161--163) THEN
    LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 164 THEN

    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (165--293) THEN
    ...
    X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (1--55) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    DISCHARGE_SAFETY_PROPERTY_TAC
  ] THEN
  ...
  X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (22--54) THEN
  X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (55--579) THEN
  X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (580--615) THEN
  LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 616 THEN
  X86_STEPS_TAC P521_JSCALARMUL_ALT_EXEC (617--618) THEN
...
let P521_JSCALARMUL_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 4744),4744))
        [word pc,LENGTH p521_jscalarmul_alt_tmc; scalar,72; point,216] /\
        ALL (nonoverlapping (res,216))
        [word pc,LENGTH p521_jscalarmul_alt_tmc;
         word_sub stackpointer (word 4744),4752]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p521_jscalarmul_alt_tmc /\
                 read RIP s = word pc /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      (word_sub stackpointer (word 4744))
                      returnaddress /\
                      memaccess_inbounds e2
                      [scalar,72; point,216; res,216;
                       word_sub stackpointer (word 4744),4752]
                      [res,216; word_sub stackpointer (word 4744),4744]))
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (res,216);
              memory :> bytes (word_sub stackpointer (word 4744),4744)])`,
  X86_ADD_RETURN_STACK_TAC P521_JSCALARMUL_ALT_EXEC
    P521_JSCALARMUL_ALT_SAFE `[RBX; RBP; R12; R13; R14; R15]` 4744
...
let P521_JSCALARMUL_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ...
        [word pc,LENGTH p521_jscalarmul_alt_mc; scalar,72; point,216] /\
        ALL (nonoverlapping (res,216))
        [word pc,LENGTH p521_jscalarmul_alt_mc;
         word_sub stackpointer (word 4744),4752]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p521_jscalarmul_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [res; scalar; point] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (res,216);
              memory :> bytes (word_sub stackpointer (word 4744),4744)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P521_JSCALARMUL_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let P521_JSCALARMUL_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 4768),4768))
        [word pc,LENGTH p521_jscalarmul_alt_windows_tmc; scalar,72; point,216] /\
        ALL (nonoverlapping (res,216))
        [word pc,LENGTH p521_jscalarmul_alt_windows_tmc;
         word_sub stackpointer (word 4768),4776]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p521_jscalarmul_alt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [res; scalar; point] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      e2 =
                      f_events scalar point res pc
                      (word_sub stackpointer (word 4768))
                      returnaddress /\
                      memaccess_inbounds e2
                      [scalar,72; point,216; res,216;
                       word_sub stackpointer (word 4768),4776]
                      [res,216; word_sub stackpointer (word 4768),4768]))
            (MAYCHANGE [RSP] ,,
  ...
  let WINDOWS_P521_JSCALARMUL_ALT_EXEC =
    X86_MK_EXEC_RULE p521_jscalarmul_alt_windows_tmc
  and baseth =
    X86_SIMD_SHARPEN_RULE P521_JSCALARMUL_ALT_NOIBT_SUBROUTINE_SAFE
    (X86_ADD_RETURN_STACK_TAC P521_JSCALARMUL_ALT_EXEC
      P521_JSCALARMUL_ALT_SAFE `[RBX; RBP; R12; R13; R14; R15]` 4744 THEN
  ...
  REWRITE_TAC[fst WINDOWS_P521_JSCALARMUL_ALT_EXEC] THEN
  REPLICATE_TAC 4 GEN_TAC THEN WORD_FORALL_OFFSET_TAC 4768 THEN
  REWRITE_TAC[ALL; WINDOWS_C_ARGUMENTS; SOME_FLAGS;
              WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI] THEN
  REWRITE_TAC[NONOVERLAPPING_CLAUSES] THEN REPEAT STRIP_TAC THEN
  ENSURES_PRESERVED_TAC "rsi_init" `RSI` THEN
  ...
  [ EXISTS_E2_TAC(ref[`pc:num`;`stackpointer:int64`]);
...
let P521_JSCALARMUL_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 4768),4768))
        [word pc,LENGTH p521_jscalarmul_alt_windows_mc; scalar,72; point,216] /\
        ALL (nonoverlapping (res,216))
        [word pc,LENGTH p521_jscalarmul_alt_windows_mc;
         word_sub stackpointer (word 4768),4776]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p521_jscalarmul_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [res; scalar; point] s /\
  ...
  MATCH_ACCEPT_TAC(ADD_IBT_RULE
    P521_JSCALARMUL_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;