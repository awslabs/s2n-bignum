...
let bignum_eq_cmc =
  define_assert_from_elf "bignum_eq_cmc" "x86/generic/bignum_eq.o"
[
...
let bignum_eq_mc = define_trimmed "bignum_eq_mc" bignum_eq_cmc;;
...
let BIGNUM_EQ_IBT_SUBROUTINE_CORRECT = prove
 (`!m a x n b y pc stackpointer returnaddress.
        ensures x86
          (\s. bytes_loaded s (word pc) bignum_eq_cmc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               C_ARGUMENTS [m;a;n;b] s /\
          ...
          (\s'. read RIP s' = returnaddress /\
                ...
                C_RETURN s' = if x = y then word 1 else word 0)
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI)`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_EQ_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_eq_cmc = define_from_elf
   "windows_bignum_eq_cmc" "x86/generic/bignum_eq.obj";;

let windows_bignum_eq_mc = define_trimmed "windows_bignum_eq_mc" windows_bignum_eq_cmc;;
...
let WINDOWS_BIGNUM_EQ_IBT_SUBROUTINE_CORRECT = prove
 (`!m a x n b y pc stackpointer returnaddress.
     ALL (nonoverlapping (word_sub stackpointer (word 16),16))
         [(word pc,LENGTH windows_bignum_eq_cmc); (a,8 * val m); (b,8 * val n)]
     ==> ensures x86
          (\s. bytes_loaded s (word pc) windows_bignum_eq_cmc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               WINDOWS_C_ARGUMENTS [m;a;n;b] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
          MAYCHANGE [memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_EQ_SUBROUTINE_CORRECT));;
