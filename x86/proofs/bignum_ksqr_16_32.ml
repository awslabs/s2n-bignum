...
let bignum_ksqr_16_32_cmc =
  define_assert_from_elf "bignum_ksqr_16_32_cmc" "x86/fastmul/bignum_ksqr_16_32.o"
[
...
let bignum_ksqr_16_32_mc = define_trimmed "bignum_ksqr_16_32_mc" bignum_ksqr_16_32_cmc;;
...
let BIGNUM_KSQR_16_32_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
     nonoverlapping (word_sub stackpointer (word 40),48) (z,8 * 32) /\
     ALL (nonoverlapping (word_sub stackpointer (word 40),40))
         [(word pc,LENGTH bignum_ksqr_16_32_cmc); (x,8 * 16)] /\
     nonoverlapping (word pc,LENGTH bignum_ksqr_16_32_cmc) (z,8 * 32) /\
     nonoverlapping (x,8 * 16) (z,8 * 32)
     ==> ensures x86
          (\s. bytes_loaded s (word pc) bignum_ksqr_16_32_cmc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               C_ARGUMENTS [z; x] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(z,8 * 32);
                   memory :> bytes(word_sub stackpointer (word 40),40)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_KSQR_16_32_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_ksqr_16_32_cmc = define_from_elf
   "windows_bignum_ksqr_16_32_cmc" "x86/fastmul/bignum_ksqr_16_32.obj";;

let windows_bignum_ksqr_16_32_mc = define_trimmed "windows_bignum_ksqr_16_32_mc" windows_bignum_ksqr_16_32_cmc;;
...
let WINDOWS_BIGNUM_KSQR_16_32_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
     nonoverlapping (word_sub stackpointer (word 56),64) (z,8 * 32) /\
     ALL (nonoverlapping (word_sub stackpointer (word 56),56))
         [(word pc,LENGTH windows_bignum_ksqr_16_32_cmc); (x,8 * 16)] /\
     nonoverlapping (word pc,LENGTH windows_bignum_ksqr_16_32_cmc) (z,8 * 32) /\
     nonoverlapping (x,8 * 16) (z,8 * 32)
     ==> ensures x86
          (\s. bytes_loaded s (word pc) windows_bignum_ksqr_16_32_cmc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               WINDOWS_C_ARGUMENTS [z; x] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(z,8 * 32);
                   memory :> bytes(word_sub stackpointer (word 56),56)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_KSQR_16_32_SUBROUTINE_CORRECT));;
