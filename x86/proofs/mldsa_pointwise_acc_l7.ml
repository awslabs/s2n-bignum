...
(*** print_literal_from_elf "x86/mldsa/mldsa_pointwise_acc_l7.o";;
 ***)

let mldsa_pointwise_acc_l7_mc = define_assert_from_elf "mldsa_pointwise_acc_l7_mc" "x86/mldsa/mldsa_pointwise_acc_l7.o"
[
...
let MLDSA_POINTWISE_ACC_L7_NOIBT_SUBROUTINE_CORRECT = prove
 (`!c a b consts x y pc stackpointer returnaddress.
    ...
    nonoverlapping (word pc,LENGTH mldsa_pointwise_acc_l7_tmc) (consts, 64) /\
    nonoverlapping (c, 1024) (a, 7168) /\
    nonoverlapping (c, 1024) (b, 7168) /\
    nonoverlapping (c, 1024) (consts, 64) /\
    nonoverlapping (a, 7168) (b, 7168) /\
...
let MLDSA_POINTWISE_ACC_L7_SUBROUTINE_CORRECT = prove
 (`!c a b consts x y pc stackpointer returnaddress.
    ...
    nonoverlapping (word pc,LENGTH mldsa_pointwise_acc_l7_mc) (consts, 64) /\
    nonoverlapping (c, 1024) (a, 7168) /\
    nonoverlapping (c, 1024) (b, 7168) /\
    nonoverlapping (c, 1024) (consts, 64) /\
    nonoverlapping (a, 7168) (b, 7168) /\
...
let mldsa_pointwise_acc_l7_windows_mc = define_from_elf
   "mldsa_pointwise_acc_l7_windows_mc" "x86/mldsa/mldsa_pointwise_acc_l7.obj";;

let mldsa_pointwise_acc_l7_windows_tmc =
  define_trimmed "mldsa_pointwise_acc_l7_windows_tmc"
    mldsa_pointwise_acc_l7_windows_mc;;

let MLDSA_POINTWISE_ACC_L7_WINDOWS_TMC_EXEC =
  X86_MK_EXEC_RULE mldsa_pointwise_acc_l7_windows_tmc;;

let MLDSA_POINTWISE_ACC_L7_NOIBT_WINDOWS_SUBROUTINE_CORRECT = prove
 (`!c a b consts x y pc stackpointer returnaddress.
    aligned 32 c /\
    aligned 32 a /\
    aligned 32 b /\
    aligned 32 consts /\
    nonoverlapping (word pc,LENGTH mldsa_pointwise_acc_l7_windows_tmc) (c, 1024) /\
    nonoverlapping (word pc,LENGTH mldsa_pointwise_acc_l7_windows_tmc) (a, 7168) /\
    nonoverlapping (word pc,LENGTH mldsa_pointwise_acc_l7_windows_tmc) (b, 7168) /\
    nonoverlapping (word pc,LENGTH mldsa_pointwise_acc_l7_windows_tmc) (consts, 64) /\
    nonoverlapping (c, 1024) (a, 7168) /\
    nonoverlapping (c, 1024) (b, 7168) /\
    nonoverlapping (c, 1024) (consts, 64) /\
    nonoverlapping (a, 7168) (b, 7168) /\
    nonoverlapping (a, 7168) (consts, 64) /\
    nonoverlapping (b, 7168) (consts, 64) /\
    nonoverlapping (word_sub stackpointer (word 176),184) (c, 1024) /\
    nonoverlapping (word_sub stackpointer (word 176),184) (a, 7168) /\
    nonoverlapping (word_sub stackpointer (word 176),184) (b, 7168) /\
    nonoverlapping (word_sub stackpointer (word 176),184) (consts, 64) /\
    nonoverlapping (word pc,LENGTH mldsa_pointwise_acc_l7_windows_tmc)
                   (word_sub stackpointer (word 176),176)
    ==> ensures x86
          (\s. bytes_loaded s (word pc) mldsa_pointwise_acc_l7_windows_tmc /\
              read RIP s = word pc /\
              read RSP s = stackpointer /\
              read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  REWRITE_TAC[fst MLDSA_POINTWISE_ACC_L7_WINDOWS_TMC_EXEC] THEN
  ...
  (* Execute Windows prologue: 17 instructions *)
  ENSURES_INIT_TAC "s0" THEN
  ...
  MP_TAC(SPECL
    [`c:int64`; `a:int64`; `b:int64`; `consts:int64`;
     `x:num->int32`; `y:num->int32`; `pc + 98`]
    (CONV_RULE(ONCE_DEPTH_CONV(REWRITE_CONV
       [MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI; SOME_FLAGS]))
      (CONV_RULE TWEAK_CONV MLDSA_POINTWISE_ACC_L7_CORRECT))) THEN
  ASM_REWRITE_TAC[C_ARGUMENTS] THEN
  ANTS_TAC THENL [NONOVERLAPPING_TAC; ALL_TAC] THEN

  (* Execute core computation as one big step *)
  X86_BIGSTEP_TAC MLDSA_POINTWISE_ACC_L7_WINDOWS_TMC_EXEC "s18" THENL
   [FIRST_ASSUM(MATCH_ACCEPT_TAC o MATCH_MP
     (BYTES_LOADED_SUBPROGRAM_RULE mldsa_pointwise_acc_l7_windows_tmc
     (REWRITE_RULE[BUTLAST_CLAUSES]
      (AP_TERM `BUTLAST:byte list->byte list` mldsa_pointwise_acc_l7_tmc))
     98));
    RULE_ASSUM_TAC(CONV_RULE(TRY_CONV RIP_PLUS_CONV))] THEN

  (* Capture final YMM states before epilogue *)
  MAP_EVERY ABBREV_TAC
   [`ymm6_epilog = read YMM6 s18`;
    `ymm7_epilog = read YMM7 s18`;
    `ymm8_epilog = read YMM8 s18`;
    `ymm9_epilog = read YMM9 s18`;
    `ymm10_epilog = read YMM10 s18`;
    `ymm11_epilog = read YMM11 s18`;
    `ymm12_epilog = read YMM12 s18`;
    `ymm13_epilog = read YMM13 s18`;
    `ymm14_epilog = read YMM14 s18`;
    `ymm15_epilog = read YMM15 s18`] THEN

  (* Execute Windows epilogue: 14 instructions including RET *)
  X86_STEPS_TAC MLDSA_POINTWISE_ACC_L7_WINDOWS_TMC_EXEC (19--32) THEN
...
let MLDSA_POINTWISE_ACC_L7_WINDOWS_SUBROUTINE_CORRECT = prove
 (`!c a b consts x y pc stackpointer returnaddress.
    aligned 32 c /\
    aligned 32 a /\
    aligned 32 b /\
    aligned 32 consts /\
    nonoverlapping (word pc,LENGTH mldsa_pointwise_acc_l7_windows_mc) (c, 1024) /\
    nonoverlapping (word pc,LENGTH mldsa_pointwise_acc_l7_windows_mc) (a, 7168) /\
    nonoverlapping (word pc,LENGTH mldsa_pointwise_acc_l7_windows_mc) (b, 7168) /\
    nonoverlapping (word pc,LENGTH mldsa_pointwise_acc_l7_windows_mc) (consts, 64) /\
    nonoverlapping (c, 1024) (a, 7168) /\
    nonoverlapping (c, 1024) (b, 7168) /\
    nonoverlapping (c, 1024) (consts, 64) /\
    nonoverlapping (a, 7168) (b, 7168) /\
    ...
    nonoverlapping (word_sub stackpointer (word 176),184) (b, 7168) /\
    nonoverlapping (word_sub stackpointer (word 176),184) (consts, 64) /\
    nonoverlapping (word pc,LENGTH mldsa_pointwise_acc_l7_windows_mc)
                   (word_sub stackpointer (word 176),176)
    ==> ensures x86
          (\s. bytes_loaded s (word pc) mldsa_pointwise_acc_l7_windows_mc /\
              read RIP s = word pc /\
              read RSP s = stackpointer /\
              read (memory :> bytes64 stackpointer) s = returnaddress /\
              WINDOWS_C_ARGUMENTS [c; a; b; consts] s /\
  ...
  CONV_TAC TWEAK_CONV THEN
  MATCH_ACCEPT_TAC(ADD_IBT_RULE
    (CONV_RULE TWEAK_CONV MLDSA_POINTWISE_ACC_L7_NOIBT_WINDOWS_SUBROUTINE_CORRECT)));;
...
let MLDSA_POINTWISE_ACC_L7_SAFE = time prove
 (`exists f_events.
       forall e c a b consts pc.
           aligned 32 c /\
           aligned 32 a /\
           aligned 32 b /\
           aligned 32 consts /\
           nonoverlapping (word pc,733) (c,1024) /\
           nonoverlapping (c, 1024) (a, 7168) /\
           nonoverlapping (c, 1024) (b, 7168) /\
           nonoverlapping (c, 1024) (consts, 64)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST mldsa_pointwise_acc_l7_tmc) /\
                    read RIP s = word pc /\
                    C_ARGUMENTS [c; a; b; consts] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 732) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events a b consts c pc /\
                         memaccess_inbounds e2
                           [a,7168; b,7168; consts,64; c,1024]
                           [c,1024]))
               (MAYCHANGE [events] ,,
                MAYCHANGE [memory :> bytes (c,1024)] ,,
                MAYCHANGE [RIP] ,,
...
let MLDSA_POINTWISE_ACC_L7_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e c a b consts pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) mldsa_pointwise_acc_l7_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [c; a; b; consts] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      read events s = APPEND e2 e /\
                      e2 = f_events a b consts c pc stackpointer returnaddress /\
                      memaccess_inbounds e2
                        [a,7168; b,7168; consts,64; c,1024; stackpointer,8]
                        [c,1024; stackpointer,0]))
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE [memory :> bytes (c,1024)])`,
  X86_PROMOTE_RETURN_NOSTACK_TAC mldsa_pointwise_acc_l7_tmc
    MLDSA_POINTWISE_ACC_L7_SAFE
...
let MLDSA_POINTWISE_ACC_L7_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e c a b consts pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) mldsa_pointwise_acc_l7_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [c; a; b; consts] s /\
...
let MLDSA_POINTWISE_ACC_L7_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e c a b consts pc stackpointer returnaddress.
        aligned 32 c /\
        aligned 32 a /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) mldsa_pointwise_acc_l7_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [c; a; b; consts] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      read events s = APPEND e2 e /\
                      e2 =
                      f_events a b consts c pc (word_sub stackpointer (word 176))
                      returnaddress /\
                      memaccess_inbounds e2
                      [a,7168; b,7168; consts,64; c,1024;
                       word_sub stackpointer (word 176),184]
                      [c,1024; word_sub stackpointer (word 176),176]))
            (MAYCHANGE [RSP] ,,
             MAYCHANGE [memory :> bytes (c,1024)] ,,
             WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE
             [memory :> bytes (word_sub stackpointer (word 176),176)])`,
  ASSUME_CALLEE_SAFETY_TAC MLDSA_POINTWISE_ACC_L7_SAFE "H_subth" THEN
  ...
  W(fun (asl,w) ->
    ...
    then failwith "More than 'read events .. = ..?'"
      ...
      REMOVE_THEN "H_subth"
        (MP_TAC o SPECL [hd current_events; `c:int64`; `a:int64`;
                         `b:int64`; `consts:int64`; `pc + 98`]))
  THEN
...
let MLDSA_POINTWISE_ACC_L7_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e c a b consts pc stackpointer returnaddress.
        aligned 32 c /\
        aligned 32 a /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) mldsa_pointwise_acc_l7_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [c; a; b; consts] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             MAYCHANGE
             [memory :> bytes (word_sub stackpointer (word 176),176)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE MLDSA_POINTWISE_ACC_L7_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
  *)
