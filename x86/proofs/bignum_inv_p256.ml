...
let bignum_inv_p256_cmc = define_assert_from_elf "bignum_inv_p256_cmc" "x86/p256/bignum_inv_p256.o"
[
...
let bignum_inv_p256_mc = define_trimmed "bignum_inv_p256_mc" bignum_inv_p256_cmc;;
...
let BIGNUM_INV_P256_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 288),288))
            [(word pc,LENGTH bignum_inv_p256_cmc); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word_sub stackpointer (word 288),296); (word pc,LENGTH bignum_inv_p256_cmc)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_inv_p256_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [z; x] s /\
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                    memory :> bytes(word_sub stackpointer (word 288),288)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_INV_P256_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_inv_p256_cmc = define_from_elf "windows_bignum_inv_p256_cmc"
      "x86/p256/bignum_inv_p256.obj";;

let windows_bignum_inv_p256_mc = define_trimmed "windows_bignum_inv_p256_mc" windows_bignum_inv_p256_cmc;;

let WINDOWS_BIGNUM_INV_P256_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 304),304))
            [(word pc,LENGTH windows_bignum_inv_p256_mc); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word_sub stackpointer (word 304),312); (word pc,LENGTH windows_bignum_inv_p256_mc)]
        ==> ensures x86
...
let WINDOWS_BIGNUM_INV_P256_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 304),304))
            [(word pc,LENGTH windows_bignum_inv_p256_cmc); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word_sub stackpointer (word 304),312); (word pc,LENGTH windows_bignum_inv_p256_cmc)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_inv_p256_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                    memory :> bytes(word_sub stackpointer (word 304),304)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_INV_P256_SUBROUTINE_CORRECT));;
