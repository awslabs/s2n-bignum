...
let bignum_sqr_6_12_cmc =
  define_assert_from_elf "bignum_sqr_6_12_cmc" "x86/fastmul/bignum_sqr_6_12.o"
[
...
let bignum_sqr_6_12_mc = define_trimmed "bignum_sqr_6_12_mc" bignum_sqr_6_12_cmc;;
...
let BIGNUM_SQR_6_12_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
     nonoverlapping (word_sub stackpointer (word 48),56) (z,8 * 12) /\
     ALL (nonoverlapping (word_sub stackpointer (word 48),48))
         [(word pc,LENGTH bignum_sqr_6_12_cmc); (x,8 * 6)] /\
     nonoverlapping (word pc,LENGTH bignum_sqr_6_12_cmc) (z,8 * 12) /\
     (x = z \/ nonoverlapping (x,8 * 6) (z,8 * 12))
     ==> ensures x86
          (\s. bytes_loaded s (word pc) bignum_sqr_6_12_cmc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               C_ARGUMENTS [z; x] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(z,8 * 12);
                      memory :> bytes(word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_6_12_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_sqr_6_12_cmc = define_from_elf
   "windows_bignum_sqr_6_12_cmc" "x86/fastmul/bignum_sqr_6_12.obj";;

let windows_bignum_sqr_6_12_mc = define_trimmed "windows_bignum_sqr_6_12_mc" windows_bignum_sqr_6_12_cmc;;
...
let WINDOWS_BIGNUM_SQR_6_12_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
     ...
     ALL (nonoverlapping (word_sub stackpointer (word 64),64))
         [(word pc,LENGTH windows_bignum_sqr_6_12_mc); (x,8 * 6)] /\
     nonoverlapping (word pc,LENGTH windows_bignum_sqr_6_12_mc) (z,8 * 12) /\
     (x = z \/ nonoverlapping (x,8 * 6) (z,8 * 12))
     ==> ensures x86
...
let WINDOWS_BIGNUM_SQR_6_12_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
     nonoverlapping (word_sub stackpointer (word 64),72) (z,8 * 12) /\
     ALL (nonoverlapping (word_sub stackpointer (word 64),64))
         [(word pc,LENGTH windows_bignum_sqr_6_12_cmc); (x,8 * 6)] /\
     nonoverlapping (word pc,LENGTH windows_bignum_sqr_6_12_cmc) (z,8 * 12) /\
     (x = z \/ nonoverlapping (x,8 * 6) (z,8 * 12))
     ==> ensures x86
          (\s. bytes_loaded s (word pc) windows_bignum_sqr_6_12_cmc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               WINDOWS_C_ARGUMENTS [z; x] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(z,8 * 12);
                      memory :> bytes(word_sub stackpointer (word 64),64)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_SQR_6_12_SUBROUTINE_CORRECT));;
