...
let bignum_sqr_4_8_alt_cmc =
  define_assert_from_elf "bignum_sqr_4_8_alt_cmc" "x86/fastmul/bignum_sqr_4_8_alt.o"
[
...
let bignum_sqr_4_8_alt_mc = define_trimmed "bignum_sqr_4_8_alt_mc" bignum_sqr_4_8_alt_cmc;;
...
let BIGNUM_SQR_4_8_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
     ALL (nonoverlapping (z,8 * 8))
         [(word pc,LENGTH bignum_sqr_4_8_alt_cmc); (x,8 * 4); (stackpointer,8)]
     ==> ensures x86
          (\s. bytes_loaded s (word pc) bignum_sqr_4_8_alt_cmc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               C_ARGUMENTS [z; x] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(z,8 * 8)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_4_8_ALT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_sqr_4_8_alt_cmc = define_from_elf
   "windows_bignum_sqr_4_8_alt_cmc" "x86/fastmul/bignum_sqr_4_8_alt.obj";;

let windows_bignum_sqr_4_8_alt_mc = define_trimmed "windows_bignum_sqr_4_8_alt_mc" windows_bignum_sqr_4_8_alt_cmc;;

let WINDOWS_BIGNUM_SQR_4_8_ALT_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH windows_bignum_sqr_4_8_alt_mc); (x,8 * 4)] /\
     ALL (nonoverlapping (z,8 * 8))
         [(word pc,LENGTH windows_bignum_sqr_4_8_alt_mc); (x,8 * 4); (word_sub stackpointer (word 16),24)]
     ==> ensures x86
...
let WINDOWS_BIGNUM_SQR_4_8_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH windows_bignum_sqr_4_8_alt_cmc); (x,8 * 4)] /\
     ALL (nonoverlapping (z,8 * 8))
         [(word pc,LENGTH windows_bignum_sqr_4_8_alt_cmc); (x,8 * 4); (word_sub stackpointer (word 16),24)]
     ==> ensures x86
          (\s. bytes_loaded s (word pc) windows_bignum_sqr_4_8_alt_cmc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               WINDOWS_C_ARGUMENTS [z; x] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(z,8 * 8);
                      memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_SQR_4_8_ALT_SUBROUTINE_CORRECT));;
