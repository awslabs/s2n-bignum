...
let SM2_MONTJDOUBLE_IBT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 232),232))
            [(word pc,LENGTH sm2_montjdouble_cmc); (p1,96)] /\
        ALL (nonoverlapping (p3,96))
            [(word pc,LENGTH sm2_montjdouble_cmc); (word_sub stackpointer (word 232),240)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) sm2_montjdouble_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [p3; p1] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,96);
                      memory :> bytes(word_sub stackpointer (word 232),232)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE SM2_MONTJDOUBLE_SUBROUTINE_CORRECT));;
...
let WINDOWS_SM2_MONTJDOUBLE_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 248),248))
            [(word pc,LENGTH windows_sm2_montjdouble_mc); (p1,96)] /\
        ALL (nonoverlapping (p3,96))
            [(word pc,LENGTH windows_sm2_montjdouble_mc); (word_sub stackpointer (word 248),256)]
        ==> ensures x86
...
let WINDOWS_SM2_MONTJDOUBLE_IBT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 248),248))
            [(word pc,LENGTH windows_sm2_montjdouble_cmc); (p1,96)] /\
        ALL (nonoverlapping (p3,96))
            [(word pc,LENGTH windows_sm2_montjdouble_cmc); (word_sub stackpointer (word 248),256)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_sm2_montjdouble_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [p3; p1] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,96);
                      memory :> bytes(word_sub stackpointer (word 248),248)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_SM2_MONTJDOUBLE_SUBROUTINE_CORRECT));;
