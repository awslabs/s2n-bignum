  ...
  | [0b1100010:7; vl] -> if has_pfxs pfxs then NONE else
    ...
    (match m with
      VEXM_0F38 ->
        ...
        | [0x00:8] ->
          ...
          (read_ModRM rex l >>= \((reg,rm),l).
          SOME (VPSHUFB (mmreg reg sz) (mmreg v sz) (simd_of_RM sz rm),l))
        | [0x28:8] ->
          let sz = vexL_size L in
          (read_ModRM rex l >>= \((reg,rm),l).
        ...
        | [0x36:8] ->
          ...
          (read_ModRM rex l >>= \((reg,rm),l).
           ...
           SOME (VPERMD (mmreg reg sz) (mmreg v sz) (simd_of_RM sz rm),l))
        | [0x58:8] ->
          let sz = vexL_size L in
          (read_ModRM rex l >>= \((reg,rm),l).
    ...
    | VEXM_0F3A ->
        ...
        | [0x0e:8] ->
          ...
          (read_ModRM rex l >>= \((reg,rm),l).
            read_imm Byte l >>= \(imm8,l).
            SOME (VPBLENDW (mmreg reg sz) (mmreg v sz) (simd_of_RM sz rm) imm8,l))
        | _ -> NONE)
    | _ -> NONE)
