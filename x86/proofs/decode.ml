...
let decode_aux = new_definition `!pfxs rex l. decode_aux pfxs rex l =
  ...
  | [0xc3:8] -> if has_pfxs pfxs then NONE else
    SOME (RET,l)
  | [0b11001:5; vex_type:2; vl] -> if has_pfxs pfxs then NONE else
    if is_some rex then NONE else
    read_VEX (vex_type = word 0) l >>= \((rex,m,v,L,pfxs),l).
    (match m with
      VEXM_0F38 ->
        read_byte l >>= \(b,l).
        (bitmatch b with
    ...
    | VEXM_0F3A ->
        ...
        | [0x4A:8] ->
          ...
          read_imm Byte l >>= \(imm8,l).
            (match pfxs with
            | (F, Rep0, SG0) -> SOME (VPBLENDD (mmreg reg sz) (mmreg v sz) (simd_of_RM sz rm) imm8, l)
           | _ -> NONE)
        | _ -> NONE)
    | _ -> NONE)
