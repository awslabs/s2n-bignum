...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_deamont_p384_alt" subroutine_signatures)
    BIGNUM_DEAMONT_P384_ALT_CORRECT
    BIGNUM_DEAMONT_P384_ALT_EXEC;;

let BIGNUM_DEAMONT_P384_ALT_SAFE = time prove
 (`exists f_events.
       forall e z x pc.
           nonoverlapping (word pc,600) (z,8 * 6)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc)
                    (BUTLAST bignum_deamont_p384_alt_tmc) /\
                    read RIP s = word (pc + 4) /\
                    C_ARGUMENTS [z; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 595) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_DEAMONT_P384_ALT_EXEC);;

let BIGNUM_DEAMONT_P384_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_deamont_p384_alt_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 6);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_deamont_p384_alt_tmc BIGNUM_DEAMONT_P384_ALT_SAFE
   `[R12; R13]` 16 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_DEAMONT_P384_ALT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 16),24) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_deamont_p384_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 6);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_DEAMONT_P384_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_DEAMONT_P384_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 32),40) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),32))
        [word pc,LENGTH bignum_deamont_p384_alt_windows_tmc; x,8 * 6] /\
        nonoverlapping (word pc,LENGTH bignum_deamont_p384_alt_windows_tmc)
        (z,8 * 6)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_deamont_p384_alt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   bignum_deamont_p384_alt_windows_tmc bignum_deamont_p384_alt_tmc
   BIGNUM_DEAMONT_P384_ALT_SAFE `[R12; R13]` 16
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_DEAMONT_P384_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 32),40) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),32))
        [word pc,LENGTH bignum_deamont_p384_alt_windows_mc; x,8 * 6] /\
        nonoverlapping (word pc,LENGTH bignum_deamont_p384_alt_windows_mc)
        (z,8 * 6)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_deamont_p384_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 6);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_DEAMONT_P384_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;