...
(* ========================================================================= *)
(* Functional correctness of mldsa_decompose_32 (x86_64 AVX2):               *)
...
(* Signature tuple for mldsa_decompose_32 (writes both a1 and a0), given
   inline to avoid touching the shared subroutine_signatures.ml table. *)
let mldsa_decompose_32_signature =
  ([(* args *)
...
(* The low halves of xmm10..xmm14 are callee-saved under the Microsoft x64   *)
(* ABI, so the Windows variant spills them (plus rdi/rsi) to a 96-byte stack *)
(* frame in the prologue and restores them in the epilogue. The core SysV    *)
...
let MLDSA_DECOMPOSE_32_NOIBT_WINDOWS_SUBROUTINE_CORRECT = prove
 (`!a1 a (x:num->int32) pc stackpointer returnaddress.
  ...
  (* Decompose writes BOTH output buffers a1 and a and touches only
     ...
     (tight) MAYCHANGE list rather than the broad ABI constant: the latter
