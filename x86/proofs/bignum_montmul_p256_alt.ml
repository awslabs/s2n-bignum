...
let bignum_montmul_p256_alt_cmc =
  define_assert_from_elf "bignum_montmul_p256_alt_cmc" "x86/p256/bignum_montmul_p256_alt.o"
[
...
let bignum_montmul_p256_alt_mc = define_trimmed "bignum_montmul_p256_alt_mc" bignum_montmul_p256_alt_cmc;;
...
let BIGNUM_MONTMUL_P256_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 40),48) /\
        ALL (nonoverlapping (word_sub stackpointer (word 40),40))
            [(word pc,LENGTH bignum_montmul_p256_alt_cmc); (x,8 * 4); (y,8 * 4)] /\
        nonoverlapping (word pc,LENGTH bignum_montmul_p256_alt_cmc) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montmul_p256_alt_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [z; x; y] s /\
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                         memory :> bytes(word_sub stackpointer (word 40),40)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTMUL_P256_ALT_SUBROUTINE_CORRECT));;
...
let BIGNUM_AMONTMUL_P256_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 40),48) /\
        ALL (nonoverlapping (word_sub stackpointer (word 40),40))
            [(word pc,LENGTH bignum_montmul_p256_alt_cmc); (x,8 * 4); (y,8 * 4)] /\
        ...
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                         memory :> bytes(word_sub stackpointer (word 40),40)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_AMONTMUL_P256_ALT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_montmul_p256_alt_cmc = define_from_elf
   "windows_bignum_montmul_p256_alt_cmc" "x86/p256/bignum_montmul_p256_alt.obj";;

let windows_bignum_montmul_p256_alt_mc = define_trimmed "windows_bignum_montmul_p256_alt_mc" windows_bignum_montmul_p256_alt_cmc;;
...
let WINDOWS_BIGNUM_MONTMUL_P256_ALT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        ...
        ALL (nonoverlapping (word_sub stackpointer (word 56),56))
            [(word pc,LENGTH windows_bignum_montmul_p256_alt_mc); (x,8 * 4); (y,8 * 4)] /\
        nonoverlapping (word pc,LENGTH windows_bignum_montmul_p256_alt_mc) (z,8 * 4)
        ==> ensures x86
...
let WINDOWS_BIGNUM_MONTMUL_P256_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 56),64) /\
        ALL (nonoverlapping (word_sub stackpointer (word 56),56))
            [(word pc,LENGTH windows_bignum_montmul_p256_alt_cmc); (x,8 * 4); (y,8 * 4)] /\
        nonoverlapping (word pc,LENGTH windows_bignum_montmul_p256_alt_cmc) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_montmul_p256_alt_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x; y] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                         memory :> bytes(word_sub stackpointer (word 56),56)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_MONTMUL_P256_ALT_SUBROUTINE_CORRECT));;
...
let WINDOWS_BIGNUM_AMONTMUL_P256_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 56),64) /\
        ALL (nonoverlapping (word_sub stackpointer (word 56),56))
            [(word pc,LENGTH windows_bignum_montmul_p256_alt_cmc); (x,8 * 4); (y,8 * 4)] /\
        ...
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                         memory :> bytes(word_sub stackpointer (word 56),56)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_AMONTMUL_P256_ALT_SUBROUTINE_CORRECT));;
