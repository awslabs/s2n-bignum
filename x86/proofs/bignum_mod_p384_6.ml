...
let BIGNUM_MOD_P384_6_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
      nonoverlapping (word pc,LENGTH bignum_mod_p384_6_cmc) (z,8 * 6) /\
      nonoverlapping (stackpointer,8) (z,8 * 6)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) bignum_mod_p384_6_cmc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                C_ARGUMENTS [z; x] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,6)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MOD_P384_6_SUBROUTINE_CORRECT));;
...
let WINDOWS_BIGNUM_MOD_P384_6_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH windows_bignum_mod_p384_6_mc); (x,8 * 6)] /\
      nonoverlapping (word pc,LENGTH windows_bignum_mod_p384_6_mc) (z,8 * 6) /\
...
let WINDOWS_BIGNUM_MOD_P384_6_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH windows_bignum_mod_p384_6_cmc); (x,8 * 6)] /\
      nonoverlapping (word pc,LENGTH windows_bignum_mod_p384_6_cmc) (z,8 * 6) /\
      nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 6)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) windows_bignum_mod_p384_6_cmc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                WINDOWS_C_ARGUMENTS [z; x] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,6);
                      memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_MOD_P384_6_SUBROUTINE_CORRECT));;
