...
let P256_MONTJDOUBLE_ALT_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 pc stackpointer.
           ALL (nonoverlapping (stackpointer,192))
           [word pc,5321; p1,96; p3,96] /\
           nonoverlapping (p3,96) (word pc,5321)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc)
                    (BUTLAST p256_montjdouble_alt_tmc) /\
                    read RIP s = word (pc + 16) /\
                    read RSP s = stackpointer /\
                    C_ARGUMENTS [p3; p1] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 5304) /\
                    (exists e2.
...
let P256_MONTJDOUBLE_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 232),232))
        [word pc,LENGTH p256_montjdouble_alt_tmc; p1,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH p256_montjdouble_alt_tmc;
         word_sub stackpointer (word 232),240]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjdouble_alt_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      returnaddress /\
                      memaccess_inbounds e2
                      [p1,96; p3,96; word_sub stackpointer (word 232),240]
...
let P256_MONTJDOUBLE_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 pc stackpointer returnaddress.
        ...
        [word pc,LENGTH p256_montjdouble_alt_mc; p1,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH p256_montjdouble_alt_mc;
         word_sub stackpointer (word 232),240]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjdouble_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 232),232)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P256_MONTJDOUBLE_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let P256_MONTJDOUBLE_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 248),248))
        [word pc,LENGTH p256_montjdouble_alt_windows_tmc; p1,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH p256_montjdouble_alt_windows_tmc;
         word_sub stackpointer (word 248),256]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjdouble_alt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      returnaddress /\
                      memaccess_inbounds e2
                      [p1,96; p3,96; word_sub stackpointer (word 248),256]
  ...
  WINDOWS_X86_WRAP_STACK_TAC
    p256_montjdouble_alt_windows_tmc p256_montjdouble_alt_tmc
    P256_MONTJDOUBLE_ALT_SAFE
    `[RBX; R12; R13; R14; R15]` 232 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let P256_MONTJDOUBLE_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 248),248))
        [word pc,LENGTH p256_montjdouble_alt_windows_mc; p1,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH p256_montjdouble_alt_windows_mc;
         word_sub stackpointer (word 248),256]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjdouble_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 248),248)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P256_MONTJDOUBLE_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;