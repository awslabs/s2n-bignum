...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_sub_p384" subroutine_signatures)
    BIGNUM_SUB_P384_CORRECT
    BIGNUM_SUB_P384_EXEC;;

let BIGNUM_SUB_P384_SAFE = time prove
 (`exists f_events.
       forall e z x y pc.
           nonoverlapping (word pc,117) (z,8 * 6)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_sub_p384_tmc) /\
                    read RIP s = word pc /\
                    C_ARGUMENTS [z; x; y] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 116) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_SUB_P384_EXEC);;

let BIGNUM_SUB_P384_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x y pc stackpointer returnaddress.
          ...
          nonoverlapping (stackpointer,8) (z,8 * 6)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_sub_p384_tmc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [z; x; y] s /\
               ...
               (\s.
                    ...
                    (exists e2.
                         ...
                         memaccess_inbounds e2
                            [x,48; y,48; z,48; stackpointer,8] [z,48]))
               (\s s'. true)`,
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_sub_p384_tmc BIGNUM_SUB_P384_SAFE THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_SUB_P384_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x y pc stackpointer returnaddress.
          nonoverlapping (word pc,LENGTH bignum_sub_p384_mc) (z,8 * 6) /\
          nonoverlapping (stackpointer,8) (z,8 * 6)
          ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_sub_p384_mc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [z; x; y] s /\
  ...
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SUB_P384_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_SUB_P384_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
      forall e z x y pc stackpointer returnaddress.
          ALL (nonoverlapping (word_sub stackpointer (word 16),16))
              [(word pc,LENGTH bignum_sub_p384_windows_tmc); (x,8 * 6); (y,8 * 6)] /\
          ...
          ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_sub_p384_windows_tmc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    WINDOWS_C_ARGUMENTS [z; x; y] s /\
  ...
  WINDOWS_X86_WRAP_NOSTACK_TAC bignum_sub_p384_windows_tmc bignum_sub_p384_tmc
    BIGNUM_SUB_P384_SAFE THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_SUB_P384_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x y pc stackpointer returnaddress.
          ALL (nonoverlapping (word_sub stackpointer (word 16),16))
              [(word pc,LENGTH bignum_sub_p384_windows_mc); (x,8 * 6); (y,8 * 6)] /\
          ...
          ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_sub_p384_windows_mc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    WINDOWS_C_ARGUMENTS [z; x; y] s /\
  ...
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SUB_P384_NOIBT_WINDOWS_SUBROUTINE_SAFE));;