...
let full_spec,public_vars = mk_safety_spec
    ...
    P256_MONTJSCALARMUL_CORRECT
    P256_MONTJSCALARMUL_EXEC;;

let P256_MONTJSCALARMUL_SAFE = time prove
 (`exists f_events.
       forall e res scalar point pc stackpointer.
           ALL (nonoverlapping (stackpointer,1320))
           [word pc,19164; res,96; scalar,32; point,96] /\
           nonoverlapping (res,96) (word pc,19164)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) p256_montjscalarmul_tmc /\
  ...
  ASSUME_CALLEE_SAFETY_TAILED_TAC
      (X86_SIMD_SHARPEN_RULE P256_MONTJADD_NOIBT_SUBROUTINE_SAFE
        (X86_PROMOTE_RETURN_STACK_TAC p256_montjadd_tmc P256_MONTJADD_SAFE
          `[RBX; RBP; R12; R13; R14; R15]` 272 THEN
        DISCHARGE_SAFETY_PROPERTY_TAC))
      "H_JADD_SAFE" THEN
  ASSUME_CALLEE_SAFETY_TAILED_TAC
      (X86_SIMD_SHARPEN_RULE P256_MONTJDOUBLE_NOIBT_SUBROUTINE_SAFE
  ...
  REPEAT CONJ_TAC THENL [
    ...
    X86_STEPS_TAC P256_MONTJSCALARMUL_EXEC (1--85) THEN
    ...
    X86_STEPS_TAC P256_MONTJSCALARMUL_EXEC (1--25) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    DISCHARGE_SAFETY_PROPERTY_TAC
  ] THEN
  ...
  X86_STEPS_TAC P256_MONTJSCALARMUL_EXEC (6--8) THEN
  LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 9 THEN
  X86_STEPS_TAC P256_MONTJSCALARMUL_EXEC (10--12) THEN
  LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 13 THEN
  X86_STEPS_TAC P256_MONTJSCALARMUL_EXEC (14--16) THEN
  LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 17 THEN
  X86_STEPS_TAC P256_MONTJSCALARMUL_EXEC (18--27) THEN
  X86_STEPS_TAC P256_MONTJSCALARMUL_EXEC (28--143) THEN
  X86_STEPS_TAC P256_MONTJSCALARMUL_EXEC (144--175) THEN
  X86_STEPS_TAC P256_MONTJSCALARMUL_EXEC (176--179) THEN
  LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 180 THEN
  X86_STEPS_TAC P256_MONTJSCALARMUL_EXEC (181--182) THEN
...
let P256_MONTJSCALARMUL_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 1368),1368))
        [word pc,LENGTH p256_montjscalarmul_tmc; scalar,32; point,96] /\
        ALL (nonoverlapping (res,96))
        [word pc,LENGTH p256_montjscalarmul_tmc;
        ...
        ==> ensures x86
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (res,96);
              memory :> bytes (word_sub stackpointer (word 1368),1368)])`,
  X86_ADD_RETURN_STACK_TAC P256_MONTJSCALARMUL_EXEC
     P256_MONTJSCALARMUL_SAFE `[RBX; RBP; R12; R13; R14; R15]` 1368
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let P256_MONTJSCALARMUL_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 1368),1368))
        [word pc,LENGTH p256_montjscalarmul_mc; scalar,32; point,96] /\
        ALL (nonoverlapping (res,96))
        [word pc,LENGTH p256_montjscalarmul_mc;
         word_sub stackpointer (word 1368),1376]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjscalarmul_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [res; scalar; point] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (res,96);
              memory :> bytes (word_sub stackpointer (word 1368),1368)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P256_MONTJSCALARMUL_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let P256_MONTJSCALARMUL_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 1392),1392))
        [word pc,LENGTH p256_montjscalarmul_windows_tmc; scalar,32; point,96] /\
        ALL (nonoverlapping (res,96))
        [word pc,LENGTH p256_montjscalarmul_windows_tmc;
         word_sub stackpointer (word 1392),1400]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjscalarmul_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [res; scalar; point] s /\
  ...
  let WINDOWS_P256_MONTJSCALARMUL_EXEC =
    X86_MK_EXEC_RULE p256_montjscalarmul_windows_tmc
  and baseth =
    X86_SIMD_SHARPEN_RULE P256_MONTJSCALARMUL_NOIBT_SUBROUTINE_SAFE
    (X86_ADD_RETURN_STACK_TAC P256_MONTJSCALARMUL_EXEC
     P256_MONTJSCALARMUL_SAFE `[RBX; RBP; R12; R13; R14; R15]` 1368 THEN
...
let P256_MONTJSCALARMUL_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 1392),1392))
        [word pc,LENGTH p256_montjscalarmul_windows_mc; scalar,32; point,96] /\
        ALL (nonoverlapping (res,96))
        [word pc,LENGTH p256_montjscalarmul_windows_mc;
         word_sub stackpointer (word 1392),1400]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjscalarmul_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [res; scalar; point] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (res,96);
              memory :> bytes (word_sub stackpointer (word 1392),1392)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P256_MONTJSCALARMUL_NOIBT_WINDOWS_SUBROUTINE_SAFE));;