...
let p256_scalarmulbase_alt_cmc = define_assert_from_elf
  "p256_scalarmulbase_alt_cmc" "x86/p256/p256_scalarmulbase_alt.o"
[
...
let p256_scalarmulbase_alt_mc = define_trimmed "p256_scalarmulbase_alt_mc" p256_scalarmulbase_alt_cmc;;
...
let LOCAL_DEMONT_TAC =
  ...
  X86_SUBROUTINE_SIM_TAC
   (p256_scalarmulbase_alt_mc,P256_SCALARMULBASE_ALT_EXEC,
    0x441,bignum_demont_p256_alt_mc,baseth)
...
let LOCAL_INV_TAC =
 ...
 (X86_PROMOTE_RETURN_STACK_TAC bignum_inv_p256_mc
  ...
  X86_SUBROUTINE_SIM_TAC
   (p256_scalarmulbase_alt_mc,P256_SCALARMULBASE_ALT_EXEC,
    0x500,bignum_inv_p256_mc,baseth)
  [`read RDI s`; `read RSI s`;
   `read(memory :> bytes(read RSI s,8 * 4)) s`;
...
let P256_SCALARMULBASE_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!res scalar blocksize tab n len tabulation pc stackpointer returnaddress.
        2 <= val blocksize /\ val blocksize <= 31 /\
        ALL (nonoverlapping (word_sub stackpointer (word 696),696))
            [(word pc,LENGTH p256_scalarmulbase_alt_cmc); (scalar,32); (tab,len)] /\
        ALL (nonoverlapping (res,64))
            [(word pc,LENGTH p256_scalarmulbase_alt_cmc); (word_sub stackpointer (word 696),704)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) p256_scalarmulbase_alt_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [res;scalar;blocksize;tab] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(res,64);
                      memory :> bytes(word_sub stackpointer (word 696),696)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P256_SCALARMULBASE_ALT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_p256_scalarmulbase_alt_cmc = define_from_elf "windows_p256_scalarmulbase_alt_cmc"
      "x86/p256/p256_scalarmulbase_alt.obj";;

let windows_p256_scalarmulbase_alt_mc = define_trimmed "windows_p256_scalarmulbase_alt_mc" windows_p256_scalarmulbase_alt_cmc;;

let WINDOWS_P256_SCALARMULBASE_ALT_SUBROUTINE_CORRECT = time prove
 (`!res scalar blocksize tab n len tabulation pc stackpointer returnaddress.
        2 <= val blocksize /\ val blocksize <= 31 /\
        ALL (nonoverlapping (word_sub stackpointer (word 728),728))
            [(word pc,LENGTH windows_p256_scalarmulbase_alt_mc); (scalar,32); (tab,len)] /\
        ALL (nonoverlapping (res,64))
            [(word pc,LENGTH windows_p256_scalarmulbase_alt_mc); (word_sub stackpointer (word 728),736)]
        ==> ensures x86
  ...
  and baseth =
    X86_SIMD_SHARPEN_RULE P256_SCALARMULBASE_ALT_SUBROUTINE_CORRECT
    (X86_ADD_RETURN_STACK_TAC P256_SCALARMULBASE_ALT_EXEC
  ...
  X86_SUBROUTINE_SIM_TAC
   (windows_p256_scalarmulbase_alt_mc,
    ...
    0x16,p256_scalarmulbase_alt_mc,subth)
...
let WINDOWS_P256_SCALARMULBASE_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!res scalar blocksize tab n len tabulation pc stackpointer returnaddress.
        2 <= val blocksize /\ val blocksize <= 31 /\
        ALL (nonoverlapping (word_sub stackpointer (word 728),728))
            [(word pc,LENGTH windows_p256_scalarmulbase_alt_cmc); (scalar,32); (tab,len)] /\
        ALL (nonoverlapping (res,64))
            [(word pc,LENGTH windows_p256_scalarmulbase_alt_cmc); (word_sub stackpointer (word 728),736)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_p256_scalarmulbase_alt_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [res;scalar;blocksize;tab] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(res,64);
                      memory :> bytes(word_sub stackpointer (word 728),728)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_P256_SCALARMULBASE_ALT_SUBROUTINE_CORRECT));;
