...
let BIGNUM_MULADD10_IBT_SUBROUTINE_CORRECT = time prove
 (`!k z d n pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_muladd10_cmc) (z,8 * val k) /\
        nonoverlapping (stackpointer,8) (z,8 * val k)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_muladd10_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [k; z; d] s /\
             ...
             (\s. read RIP s = returnaddress /\
              ...
              (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
               MAYCHANGE [memory :> bignum(z,val k)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MULADD10_SUBROUTINE_CORRECT));;
...
let WINDOWS_BIGNUM_MULADD10_IBT_SUBROUTINE_CORRECT = time prove
 (`!k z d n pc stackpointer returnaddress.
      nonoverlapping (word pc,LENGTH windows_bignum_muladd10_cmc) (z,8 * val k) /\
      ALL (nonoverlapping (word_sub stackpointer (word 16),24))
          [(word pc,LENGTH windows_bignum_muladd10_cmc); (z,8 * val k)]
      ==> ensures x86
           (\s. bytes_loaded s (word pc) windows_bignum_muladd10_cmc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                WINDOWS_C_ARGUMENTS [k; z; d] s /\
           ...
           (\s. read RIP s = returnaddress /\
            ...
            (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE [memory :> bignum(z,val k);
                        memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_MULADD10_SUBROUTINE_CORRECT));;
