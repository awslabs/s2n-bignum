...
let BIGNUM_NEG_P384_SAFE = time prove
 (`exists f_events.
       forall e z x pc.
           nonoverlapping (word pc,119) (z,8 * 6)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_neg_p384_tmc) /\
                    read RIP s = word pc /\
                    C_ARGUMENTS [z; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 118) /\
                    (exists e2.
...
let BIGNUM_NEG_P384_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
          ALL (nonoverlapping (z,8 * 6)) [(word pc,LENGTH bignum_neg_p384_tmc); (stackpointer,8)]
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_neg_p384_tmc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [z; x] s /\
               ...
               (\s.
                    ...
                    (exists e2.
                         ...
                         memaccess_inbounds e2
                            [x,48; z,48; stackpointer,8] [z,48]))
               (\s s'. true)`,
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_neg_p384_tmc BIGNUM_NEG_P384_SAFE THEN
 ...
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
          ALL (nonoverlapping (z,8 * 6)) [(word pc,LENGTH bignum_neg_p384_mc); (stackpointer,8)]
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_neg_p384_mc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [z; x] s /\
               ...
               (\s.
                    ...
                    (exists e2.
                         ...
                         memaccess_inbounds e2
                            [x,48; z,48; stackpointer,8] [z,48]))
               (\s s'. true)`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_NEG_P384_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_NEG_P384_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
        forall e z x pc stackpointer returnaddress.
          ALL (nonoverlapping (word_sub stackpointer (word 16),16))
              [(word pc,LENGTH bignum_neg_p384_windows_tmc); (x,8 * 6)] /\
          ALL (nonoverlapping (z,8 * 6)) [(word pc,LENGTH bignum_neg_p384_windows_tmc);
                                          (word_sub stackpointer (word 16),24)]
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_neg_p384_windows_tmc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    WINDOWS_C_ARGUMENTS [z; x] s /\
               ...
               (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
                MAYCHANGE [memory :> bignum(z,6);
                            memory :> bytes(word_sub stackpointer (word 16),16)])`,
  WINDOWS_X86_WRAP_NOSTACK_TAC bignum_neg_p384_windows_tmc bignum_neg_p384_tmc
      BIGNUM_NEG_P384_SAFE THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_NEG_P384_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
        forall e z x pc stackpointer returnaddress.
          ALL (nonoverlapping (word_sub stackpointer (word 16),16))
              [(word pc,LENGTH bignum_neg_p384_windows_mc); (x,8 * 6)] /\
          ALL (nonoverlapping (z,8 * 6)) [(word pc,LENGTH bignum_neg_p384_windows_mc);
                                          (word_sub stackpointer (word 16),24)]
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_neg_p384_windows_mc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    WINDOWS_C_ARGUMENTS [z; x] s /\
               ...
               (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
                MAYCHANGE [memory :> bignum(z,6);
                            memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_NEG_P384_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
