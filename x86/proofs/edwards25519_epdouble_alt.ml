...
let edwards25519_epdouble_alt_cmc = define_assert_from_elf
  "edwards25519_epdouble_alt_cmc" "x86/curve25519/edwards25519_epdouble_alt.o"
[
...
let edwards25519_epdouble_alt_mc = define_trimmed "edwards25519_epdouble_alt_mc" edwards25519_epdouble_alt_cmc;;
...
let EDWARDS25519_EPDOUBLE_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 T1 pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 200),200))
        [(word pc,LENGTH edwards25519_epdouble_alt_cmc); (p1,96)] /\
    nonoverlapping (p3,128) (word pc,LENGTH edwards25519_epdouble_alt_cmc) /\
    nonoverlapping (p3,128) (word_sub stackpointer (word 200),208)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) edwards25519_epdouble_alt_cmc /\
              read RIP s = word pc /\
              read RSP s = stackpointer /\
              read (memory :> bytes64 stackpointer) s = returnaddress /\
              C_ARGUMENTS [p3; p1] s /\
         ...
         (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,128);
                      memory :> bytes(word_sub stackpointer (word 200),200)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE EDWARDS25519_EPDOUBLE_ALT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_edwards25519_epdouble_alt_cmc = define_from_elf
  "windows_edwards25519_epdouble_alt_cmc"
  "x86/curve25519/edwards25519_epdouble_alt.obj";;

let windows_edwards25519_epdouble_alt_mc = define_trimmed "windows_edwards25519_epdouble_alt_mc" windows_edwards25519_epdouble_alt_cmc;;

let WINDOWS_EDWARDS25519_EPDOUBLE_ALT_SUBROUTINE_CORRECT = prove
 (`!p3 p1 T1 pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 216),216))
        [(word pc,LENGTH windows_edwards25519_epdouble_alt_mc); (p1,96)] /\
    nonoverlapping (p3,128) (word pc,LENGTH windows_edwards25519_epdouble_alt_mc) /\
...
let WINDOWS_EDWARDS25519_EPDOUBLE_ALT_IBT_SUBROUTINE_CORRECT = prove
 (`!p3 p1 T1 pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 216),216))
        [(word pc,LENGTH windows_edwards25519_epdouble_alt_cmc); (p1,96)] /\
    nonoverlapping (p3,128) (word pc,LENGTH windows_edwards25519_epdouble_alt_cmc) /\
    nonoverlapping (p3,128) (word_sub stackpointer (word 216),224)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) windows_edwards25519_epdouble_alt_cmc /\
              read RIP s = word pc /\
              read RSP s = stackpointer /\
              read (memory :> bytes64 stackpointer) s = returnaddress /\
              WINDOWS_C_ARGUMENTS [p3; p1] s /\
         ...
         (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
          MAYCHANGE [memory :> bytes(p3,128);
                     memory :> bytes(word_sub stackpointer (word 216),216)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_EDWARDS25519_EPDOUBLE_ALT_SUBROUTINE_CORRECT));;
