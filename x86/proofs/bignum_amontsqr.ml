...
let bignum_amontsqr_cmc =
  define_assert_from_elf "bignum_amontsqr_cmc" "x86/generic/bignum_amontsqr.o"
[
...
let bignum_amontsqr_mc = define_trimmed "bignum_amontsqr_mc" bignum_amontsqr_cmc;;
...
let BIGNUM_AMONTSQR_IBT_SUBROUTINE_CORRECT = time prove
 (`!k z x m a n pc stackpointer returnaddress.
       nonoverlapping (z,8 * val k) (word_sub stackpointer (word 48),56) /\
       ALLPAIRS nonoverlapping
           [(z,8 * val k); (word_sub stackpointer (word 48),48)]
           [(word pc,LENGTH bignum_amontsqr_cmc); (x,8 * val k); (m,8 * val k)]
      ==> ensures x86
           (\s. bytes_loaded s (word pc) bignum_amontsqr_cmc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                C_ARGUMENTS [k; z; x; m] s /\
           ...
           (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
            MAYCHANGE [memory :> bytes(z,8 * val k);
                       memory :> bytes(word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_AMONTSQR_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_amontsqr_cmc = define_from_elf
   "windows_bignum_amontsqr_cmc" "x86/generic/bignum_amontsqr.obj";;

let windows_bignum_amontsqr_mc = define_trimmed "windows_bignum_amontsqr_mc" windows_bignum_amontsqr_cmc;;
...
let WINDOWS_BIGNUM_AMONTSQR_SUBROUTINE_CORRECT = time prove
 (`!k z x m a n pc stackpointer returnaddress.
       ...
       ALLPAIRS nonoverlapping
           ...
           [(word pc,LENGTH windows_bignum_amontsqr_mc); (x,8 * val k); (m,8 * val k)]
      ==> ensures x86
...
let WINDOWS_BIGNUM_AMONTSQR_IBT_SUBROUTINE_CORRECT = time prove
 (`!k z x m a n pc stackpointer returnaddress.
       nonoverlapping (z,8 * val k) (word_sub stackpointer (word 64),72) /\
       ALLPAIRS nonoverlapping
           [(z,8 * val k); (word_sub stackpointer (word 64),64)]
           [(word pc,LENGTH windows_bignum_amontsqr_cmc); (x,8 * val k); (m,8 * val k)]
      ==> ensures x86
           (\s. bytes_loaded s (word pc) windows_bignum_amontsqr_cmc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                WINDOWS_C_ARGUMENTS [k; z; x; m] s /\
           ...
           (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
            MAYCHANGE [memory :> bytes(z,8 * val k);
                       memory :> bytes(word_sub stackpointer (word 64),64)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_AMONTSQR_SUBROUTINE_CORRECT));;
