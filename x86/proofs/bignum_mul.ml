...
let bignum_mul_cmc =
  define_assert_from_elf "bignum_mul_cmc" "x86/generic/bignum_mul.o"
[
...
let bignum_mul_mc = define_trimmed "bignum_mul_mc" bignum_mul_cmc;;
...
let BIGNUM_MUL_IBT_SUBROUTINE_CORRECT = prove
 (`!p m n z x y a b pc stackpointer returnaddress.
     nonoverlapping (z,8 * val p) (word_sub stackpointer (word 48),56) /\
     ALLPAIRS nonoverlapping
              [(z,8 * val p); (word_sub stackpointer (word 48),56)]
              [(word pc,LENGTH bignum_mul_cmc); (x,8 * val m); (y,8 * val n)]
     ==> ensures x86
          (\s. bytes_loaded s (word pc) bignum_mul_cmc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               C_ARGUMENTS [p; z; m; x; n; y] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,val p);
                      memory :> bytes(word_sub stackpointer (word 48),56)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MUL_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_mul_cmc = define_from_elf
   "windows_bignum_mul_cmc" "x86/generic/bignum_mul.obj";;

let windows_bignum_mul_mc = define_trimmed "windows_bignum_mul_mc" windows_bignum_mul_cmc;;
...
let WINDOWS_BIGNUM_MUL_SUBROUTINE_CORRECT = prove
 (`!p m n z x y a b pc stackpointer returnaddress.
     ...
     ALLPAIRS nonoverlapping
              ...
              [(word pc,LENGTH windows_bignum_mul_mc); (x,8 * val m); (y,8 * val n)]
     ==> ensures x86
...
let WINDOWS_BIGNUM_MUL_IBT_SUBROUTINE_CORRECT = prove
 (`!p m n z x y a b pc stackpointer returnaddress.
     nonoverlapping (z,8 * val p) (word_sub stackpointer (word 64),72) /\
     ALLPAIRS nonoverlapping
              [(z,8 * val p); (word_sub stackpointer (word 64),72)]
              [(word pc,LENGTH windows_bignum_mul_cmc); (x,8 * val m); (y,8 * val n)]
     ==> ensures x86
          (\s. bytes_loaded s (word pc) windows_bignum_mul_cmc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               WINDOWS_C_ARGUMENTS [p; z; m; x; n; y] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,val p);
                      memory :> bytes(word_sub stackpointer (word 64),72)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_MUL_SUBROUTINE_CORRECT));;
