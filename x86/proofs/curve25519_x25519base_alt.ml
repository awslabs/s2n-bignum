...
(**** print_literal_relocs_from_elf "x86/curve25519/curve25519_x25519base_alt.o";;
 ****)

(* The .text body of curve25519_x25519base_alt is identical between the Linux ABI .o
   and the Windows .obj — only the entry trampoline and IBT prefix differ (Windows
   shuffles arguments and invokes the Linux ABI entry).  We share the body bytes
   across both via a single body function parameterised over the IBT prefix
   (ENDBR64 alone for Linux ABI; ENDBR64 + ABI dispatcher for Windows). *)

let curve25519_x25519base_alt_body : 'a. int list -> (int list -> 'a) ->
                                         (string * int -> 'a) -> 'a list =
...
let curve25519_x25519base_alt_mc,
    (curve25519_x25519base_alt_constant_data::_) =
    define_assert_relocs_from_elf "curve25519_x25519base_alt_mc"
    "x86/curve25519/curve25519_x25519base_alt.o"
    (fun b ADDR -> curve25519_x25519base_alt_body
...
let BYTES_LOADED_DATA = prove
 (`bytes_loaded s tab curve25519_x25519base_alt_constant_data <=>
  ...
  REWRITE_TAC[bytes_loaded; READ_BYTELIST_EQ_BYTES;
    CONV_RULE (RAND_CONV LENGTH_CONV)
     (AP_TERM `LENGTH:byte list->num` curve25519_x25519base_alt_constant_data)]);;
...
let X25519BASE_TABLE_LEMMA = prove
 (`read (memory :> bytes(wpc,48576)) s =
  ...
  REWRITE_TAC[GSYM BYTES_LOADED_DATA; curve25519_x25519base_alt_constant_data] THEN
...
let curve25519_x25519base_alt_tmc' =
  SPECL [`pc:num`; `tables:num`] curve25519_x25519base_alt_tmc;;
...
let LOCAL_MUL_P25519_TAC =
  X86_MACRO_SIM_ABBREV_TAC curve25519_x25519base_alt_tmc' 129 lvs
   `!(t:x86state) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      ==>
      nonoverlapping (word pc,0x2f4d) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (curve25519_x25519base_alt_tmc pc tables) /\
                read RIP s = pcin /\
...
let LOCAL_MUL_4_TAC =
  X86_MACRO_SIM_ABBREV_TAC curve25519_x25519base_alt_tmc' 120 lvs
   `!(t:x86state) pcin pcout q3 n3 q1 n1 q2 n2.
      ...
      ==>
      nonoverlapping (word pc,0x2f4d) (word_add (read q3 t) (word n3),8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (curve25519_x25519base_alt_tmc pc tables) /\
                read RIP s = pcin /\
                read RSP s = read RSP t /\
...
let CURVE25519_X25519BASE_ALT_CORRECT = time prove
 (`!tables res scalar n pc stackpointer.
    ...
    ALL (nonoverlapping (stackpointer,488))
        [(word pc,0x2f4d); (word tables,48576); (res,32); (scalar,32)] /\
    ALL (nonoverlapping (res,32)) [(word pc,0x2f4d); (word tables,48576)]
    ==> ensures x86
         (\s. bytes_loaded s (word pc) (curve25519_x25519base_alt_tmc pc tables) /\
  ...
  ENSURES_WHILE_AUP_TAC `1` `64` `pc + 0x197` `pc + 0x184d`
   `\i s.
 ...
 (`!tables res scalar n pc stackpointer returnaddress.
    ...
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
                (curve25519_x25519base_alt_tmc pc tables) /\
              read RIP s = word pc /\
              bytes_loaded s (word tables)
                curve25519_x25519base_alt_constant_data /\
              read RSP s = stackpointer /\
...
let CURVE25519_X25519BASE_ALT_SUBROUTINE_CORRECT = time prove
 (`!tables res scalar n pc stackpointer returnaddress.
    ...
    ALL (nonoverlapping (word_sub stackpointer (word 536),536))
        [(word pc,0x2f51); (word tables,48576); (scalar,32)] /\
    ALL (nonoverlapping (res,32))
        [(word pc,0x2f51); (word tables,48576);
         (word_sub stackpointer (word 536),544)]
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
                (curve25519_x25519base_alt_mc pc tables) /\
              read RIP s = word pc /\
              bytes_loaded s (word tables)
                curve25519_x25519base_alt_constant_data /\
              read RSP s = stackpointer /\
...
let curve25519_x25519base_alt_windows_mc,_ =
...
let curve25519_x25519base_alt_windows_tmc =
...
let CURVE25519_X25519BASE_ALT_WINDOWS_MC_BRIDGE = prove
 (`!pc tables. curve25519_x25519base_alt_windows_mc pc tables =
     APPEND [word 0xf3:byte; word 0x0f; word 0x1e; word 0xfa]
            (curve25519_x25519base_alt_windows_tmc (pc + 4) tables)`,
  REPEAT GEN_TAC THEN
  REWRITE_TAC[curve25519_x25519base_alt_windows_mc;
              curve25519_x25519base_alt_windows_tmc; APPEND] THEN
  REWRITE_TAC[GSYM INT_OF_NUM_ADD] THEN
  AP_TERM_TAC THEN AP_TERM_TAC THEN AP_TERM_TAC THEN
...
let CURVE25519_X25519BASE_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!tables res scalar n pc stackpointer returnaddress.
    ...
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
                (curve25519_x25519base_alt_windows_tmc pc tables) /\
              read RIP s = word pc /\
              bytes_loaded s (word tables)
                curve25519_x25519base_alt_constant_data /\
              read RSP s = stackpointer /\
...
let CURVE25519_X25519BASE_ALT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!tables res scalar n pc stackpointer returnaddress.
    ...
    ALL (nonoverlapping (word_sub stackpointer (word 560),560))
        [(word pc,0x2f61); (word tables,48576); (scalar,32)] /\
    ALL (nonoverlapping (res,32))
        [(word pc,0x2f61); (word tables,48576);
         (word_sub stackpointer (word 560),568)]
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
                (curve25519_x25519base_alt_windows_mc pc tables) /\
              read RIP s = word pc /\
              bytes_loaded s (word tables)
                curve25519_x25519base_alt_constant_data /\
              read RSP s = stackpointer /\
  ...
  REMOVE_THEN assump_name (fun safety_th ->
    X86_SUBROUTINE_SIM_TAC ~is_safety_thm:true
       (curve25519_x25519base_alt_tmc',
        ...
        `pc + 0x195d`; `stackpointer:int64`] n
    THENL [
      EXISTS_E2_TAC(ref
...
let CURVE25519_X25519BASE_ALT_SAFE = time prove
 (`exists f_events.
  ...
  ENSURES_EVENTS_WHILE_UP2_TAC `63` `pc + 0x197` `pc + 0x185f`(*+2 insns *)
   `\i s.
      read (memory :> bytes(word tables,48576)) s =
          num_of_bytelist curve25519_x25519base_alt_constant_data /\
      read RSP s = stackpointer /\
  ...
  REPEAT CONJ_TAC THENL [
    ...
    X86_STEPS_TAC CURVE25519_X25519BASE_ALT_EXEC (1--239) THEN
    X86_STEPS_TAC CURVE25519_X25519BASE_ALT_EXEC (240--281) THEN
    X86_STEPS_TAC CURVE25519_X25519BASE_ALT_EXEC (282--
    ...
    X86_STEPS_TAC CURVE25519_X25519BASE_ALT_EXEC (1255--1256) THEN
    X86_STEPS_TAC CURVE25519_X25519BASE_ALT_EXEC (1257--1257) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    CONJ_TAC THENL [
  ...
  ENSURES_INIT_TAC "s0" THEN
  STRIP_EXISTS_ASSUM_TAC THEN
  X86_STEPS_TAC CURVE25519_X25519BASE_ALT_EXEC (1--2) THEN
  X86_STEPS_TAC CURVE25519_X25519BASE_ALT_EXEC (3--
      (2+(19(*ADD_TWICE4*) + 19(*SUB_TWICE4*)))) THEN
  LOCAL_MODINV_SAFETY_TAC "H_INV_SAFE" 41 THEN
  X86_STEPS_TAC CURVE25519_X25519BASE_ALT_EXEC (42--(41+129)) THEN

  X86_STEPS_TAC CURVE25519_X25519BASE_ALT_EXEC (171--171) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
...
let CURVE25519_X25519BASE_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e tables res scalar pc stackpointer returnaddress.
        ...
        ALL (nonoverlapping (word_sub stackpointer (word 536),536))
        [word pc,0x2f51; word tables,48576; scalar,32] /\
        nonoverlapping (res,32) (word pc,0x2f51) /\
        nonoverlapping (res,32) (word_sub stackpointer (word 536),544)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc)
...
let CURVE25519_X25519BASE_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e tables res scalar pc stackpointer returnaddress.
        ...
        ALL (nonoverlapping (word_sub stackpointer (word 560),560))
        [word pc,0x2f61; word tables,48576; scalar,32] /\
        nonoverlapping (res,32) (word pc,0x2f61) /\
        nonoverlapping (res,32) (word_sub stackpointer (word 560),568)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc)
