...
let BIGNUM_OF_WORD_IBT_SUBROUTINE_CORRECT = prove
 (`!k z n pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_of_word_cmc) (z,8 * val k) /\
        nonoverlapping (z,8 * val k) (stackpointer,8)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_of_word_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [k; z; n] s)
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bignum(z,val k)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_OF_WORD_SUBROUTINE_CORRECT));;
...
let WINDOWS_BIGNUM_OF_WORD_SUBROUTINE_CORRECT = prove
 (`!k z n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH windows_bignum_of_word_mc)] /\
        nonoverlapping (word pc,LENGTH windows_bignum_of_word_mc) (z,8 * val k) /\
...
let WINDOWS_BIGNUM_OF_WORD_IBT_SUBROUTINE_CORRECT = prove
 (`!k z n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH windows_bignum_of_word_cmc)] /\
        nonoverlapping (word pc,LENGTH windows_bignum_of_word_cmc) (z,8 * val k) /\
        nonoverlapping (z,8 * val k) (word_sub stackpointer (word 16),24)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_of_word_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [k; z; n] s)
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bignum(z,val k);
                         memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_OF_WORD_SUBROUTINE_CORRECT));;
