...
let bignum_ctz_cmc = define_assert_from_elf "bignum_ctz_cmc" "x86/generic/bignum_ctz.o"
[
...
let bignum_ctz_mc = define_trimmed "bignum_ctz_mc" bignum_ctz_cmc;;
...
let BIGNUM_CTZ_IBT_SUBROUTINE_CORRECT = prove
 (`!k a x pc stackpointer returnaddress.
        ensures x86
         (\s. bytes_loaded s (word pc) bignum_ctz_cmc /\
              read RIP s = word pc /\
              read RSP s = stackpointer /\
              read (memory :> bytes64 stackpointer) s = returnaddress /\
              C_ARGUMENTS [k;a] s /\
         ...
         (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI)`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_CTZ_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_ctz_cmc = define_from_elf
   "windows_bignum_ctz_cmc" "x86/generic/bignum_ctz.obj";;

let windows_bignum_ctz_mc = define_trimmed "windows_bignum_ctz_mc" windows_bignum_ctz_cmc;;

let WINDOWS_BIGNUM_CTZ_SUBROUTINE_CORRECT = prove
 (`!k a x pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH windows_bignum_ctz_mc); (a,8 * val k)]
        ==> ensures x86
...
let WINDOWS_BIGNUM_CTZ_IBT_SUBROUTINE_CORRECT = prove
 (`!k a x pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH windows_bignum_ctz_cmc); (a,8 * val k)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_ctz_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [k;a] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_CTZ_SUBROUTINE_CORRECT));;
