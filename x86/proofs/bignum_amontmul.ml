...
let bignum_amontmul_cmc =
  define_assert_from_elf "bignum_amontmul_cmc" "x86/generic/bignum_amontmul.o"
[
...
let bignum_amontmul_mc = define_trimmed "bignum_amontmul_mc" bignum_amontmul_cmc;;
...
let BIGNUM_AMONTMUL_IBT_SUBROUTINE_CORRECT = time prove
 (`!k z x y m a b n pc stackpointer returnaddress.
        nonoverlapping (z,8 * val k) (word_sub stackpointer (word 56),64) /\
        ALLPAIRS nonoverlapping
          [(z,8 * val k); (word_sub stackpointer (word 56),56)]
          [(word pc,LENGTH bignum_amontmul_cmc); (x,8 * val k); (y,8 * val k); (m,8 * val k)]
      ==> ensures x86
           (\s. bytes_loaded s (word pc) bignum_amontmul_cmc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                C_ARGUMENTS [k; z; x; y; m] s /\
           ...
           (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
            MAYCHANGE [memory :> bytes(z,8 * val k);
                       memory :> bytes(word_sub stackpointer (word 56),56)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_AMONTMUL_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_amontmul_cmc = define_from_elf
   "windows_bignum_amontmul_cmc" "x86/generic/bignum_amontmul.obj";;

let windows_bignum_amontmul_mc = define_trimmed "windows_bignum_amontmul_mc" windows_bignum_amontmul_cmc;;
...
let WINDOWS_BIGNUM_AMONTMUL_SUBROUTINE_CORRECT = time prove
 (`!k z x y m a b n pc stackpointer returnaddress.
        ...
        ALLPAIRS nonoverlapping
          ...
          [(word pc,LENGTH windows_bignum_amontmul_mc); (x,8 * val k); (y,8 * val k); (m,8 * val k)]
      ==> ensures x86
...
let WINDOWS_BIGNUM_AMONTMUL_IBT_SUBROUTINE_CORRECT = time prove
 (`!k z x y m a b n pc stackpointer returnaddress.
        nonoverlapping (z,8 * val k) (word_sub stackpointer (word 72),80) /\
        ALLPAIRS nonoverlapping
          [(z,8 * val k); (word_sub stackpointer (word 72),72)]
          [(word pc,LENGTH windows_bignum_amontmul_cmc); (x,8 * val k); (y,8 * val k); (m,8 * val k)]
      ==> ensures x86
           (\s. bytes_loaded s (word pc) windows_bignum_amontmul_cmc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                WINDOWS_C_ARGUMENTS [k; z; x; y; m] s /\
           ...
           (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
            MAYCHANGE [memory :> bytes(z,8 * val k);
                       memory :> bytes(word_sub stackpointer (word 72),72)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_AMONTMUL_SUBROUTINE_CORRECT));;
