...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_neg_p521" subroutine_signatures)
    BIGNUM_NEG_P521_CORRECT
    BIGNUM_NEG_P521_EXEC;;

let BIGNUM_NEG_P521_SAFE = time prove
 (`exists f_events.
       forall e z x pc.
           nonoverlapping (word pc,140) (z,8 * 9) /\
           (x = z \/ nonoverlapping (x,8 * 9) (z,8 * 9))
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_neg_p521_tmc) /\
                    read RIP s = word pc /\
                    C_ARGUMENTS [z; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 139) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_NEG_P521_EXEC);;

let BIGNUM_NEG_P521_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
           ...
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_neg_p521_tmc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [z; x] s /\
               ...
               (\s.
                    ...
                    (exists e2.
                         ...
                         memaccess_inbounds e2
                            [x,72; z,72; stackpointer,8] [z,72]))
               (MAYCHANGE [RSP] ,,
                MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
                MAYCHANGE [memory :> bignum (z,9)])`,
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_neg_p521_tmc BIGNUM_NEG_P521_SAFE THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_NEG_P521_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
           nonoverlapping (word pc,LENGTH bignum_neg_p521_mc) (z,8 * 9) /\
           nonoverlapping (stackpointer,8) (z,8 * 9) /\
           ...
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_neg_p521_mc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [z; x] s /\
               ...
               (MAYCHANGE [RSP] ,,
                MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
                MAYCHANGE [memory :> bignum (z,9)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_NEG_P521_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_NEG_P521_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
           ALL (nonoverlapping (word_sub stackpointer (word 16),16))
           [word pc,LENGTH bignum_neg_p521_windows_tmc; x,8 * 9] /\
           nonoverlapping (word pc,LENGTH bignum_neg_p521_windows_tmc)
           (z,8 * 9) /\
           nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 9) /\
           (x = z \/ nonoverlapping (x,8 * 9) (z,8 * 9))
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_neg_p521_windows_tmc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    WINDOWS_C_ARGUMENTS [z; x] s /\
               ...
               (MAYCHANGE [RSP] ,,
                ...
                [memory :> bignum (z,9);
                 memory :> bytes (word_sub stackpointer (word 16),16)])`,
  WINDOWS_X86_WRAP_NOSTACK_TAC bignum_neg_p521_windows_tmc bignum_neg_p521_tmc
    BIGNUM_NEG_P521_SAFE THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_NEG_P521_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
           ALL (nonoverlapping (word_sub stackpointer (word 16),16))
           [word pc,LENGTH bignum_neg_p521_windows_mc; x,8 * 9] /\
           nonoverlapping (word pc,LENGTH bignum_neg_p521_windows_mc)
           (z,8 * 9) /\
           nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 9) /\
           (x = z \/ nonoverlapping (x,8 * 9) (z,8 * 9))
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_neg_p521_windows_mc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    WINDOWS_C_ARGUMENTS [z; x] s /\
               ...
               (MAYCHANGE [RSP] ,,
                ...
                [memory :> bignum (z,9);
                 memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_NEG_P521_NOIBT_WINDOWS_SUBROUTINE_SAFE));;