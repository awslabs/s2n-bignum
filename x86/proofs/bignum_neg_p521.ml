...
let bignum_neg_p521_cmc = define_assert_from_elf "bignum_neg_p521_cmc" "x86/p521/bignum_neg_p521.o"
[
...
let bignum_neg_p521_mc = define_trimmed "bignum_neg_p521_mc" bignum_neg_p521_cmc;;
...
let BIGNUM_NEG_P521_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_neg_p521_cmc) (z,8 * 9) /\
        nonoverlapping (stackpointer,8) (z,8 * 9) /\
        (x = z \/ nonoverlapping (x,8 * 9) (z,8 * 9))
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_neg_p521_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [z; x] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,9)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_NEG_P521_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_neg_p521_cmc = define_from_elf
   "windows_bignum_neg_p521_cmc" "x86/p521/bignum_neg_p521.obj";;

let windows_bignum_neg_p521_mc = define_trimmed "windows_bignum_neg_p521_mc" windows_bignum_neg_p521_cmc;;

let WINDOWS_BIGNUM_NEG_P521_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH windows_bignum_neg_p521_mc); (x,8 * 9)] /\
        nonoverlapping (word pc,LENGTH windows_bignum_neg_p521_mc) (z,8 * 9) /\
...
let WINDOWS_BIGNUM_NEG_P521_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH windows_bignum_neg_p521_cmc); (x,8 * 9)] /\
        nonoverlapping (word pc,LENGTH windows_bignum_neg_p521_cmc) (z,8 * 9) /\
        nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 9) /\
        (x = z \/ nonoverlapping (x,8 * 9) (z,8 * 9))
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_neg_p521_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,9);
                      memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_NEG_P521_SUBROUTINE_CORRECT));;
