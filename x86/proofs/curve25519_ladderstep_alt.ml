...
let curve25519_ladderstep_alt_cmc = define_assert_from_elf
  "curve25519_ladderstep_alt_cmc" "x86/curve25519/curve25519_ladderstep_alt.o"
[
...
let curve25519_ladderstep_alt_mc = define_trimmed "curve25519_ladderstep_alt_mc" curve25519_ladderstep_alt_cmc;;
...
let CURVE25519_LADDERSTEP_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!rr point P pp Pm Pn b pc stackpointer returnaddress.
    ALLPAIRS nonoverlapping
     [(rr,128); (word_sub stackpointer (word 464),464)]
     [(word pc,LENGTH curve25519_ladderstep_alt_cmc); (point,64); (pp,128)] /\
    nonoverlapping (rr,128) (word_sub stackpointer (word 464),472)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) curve25519_ladderstep_alt_cmc /\
              read RIP s = word pc /\
              read RSP s = stackpointer /\
              read (memory :> bytes64 stackpointer) s = returnaddress /\
              C_ARGUMENTS [rr; point; pp; b] s /\
         ...
         (\s. read RIP s = returnaddress /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(rr,128);
                      memory :> bytes(word_sub stackpointer (word 464),464)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE CURVE25519_LADDERSTEP_ALT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_curve25519_ladderstep_alt_cmc = define_from_elf
   "windows_curve25519_ladderstep_alt_cmc"
"x86/curve25519/curve25519_ladderstep_alt.obj";;

let windows_curve25519_ladderstep_alt_mc = define_trimmed "windows_curve25519_ladderstep_alt_mc" windows_curve25519_ladderstep_alt_cmc;;

let WINDOWS_CURVE25519_LADDERSTEP_ALT_SUBROUTINE_CORRECT = time prove
 (`!rr point P pp Pm Pn b pc stackpointer returnaddress.
    ALLPAIRS nonoverlapping
     [(rr,128); (word_sub stackpointer (word 480),480)]
     [(word pc,LENGTH windows_curve25519_ladderstep_alt_mc); (point,64); (pp,128)] /\
...
let WINDOWS_CURVE25519_LADDERSTEP_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!rr point P pp Pm Pn b pc stackpointer returnaddress.
    ALLPAIRS nonoverlapping
     [(rr,128); (word_sub stackpointer (word 480),480)]
     [(word pc,LENGTH windows_curve25519_ladderstep_alt_cmc); (point,64); (pp,128)] /\
    nonoverlapping (rr,128) (word_sub stackpointer (word 480),488)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) windows_curve25519_ladderstep_alt_cmc /\
              read RIP s = word pc /\
              read RSP s = stackpointer /\
              read (memory :> bytes64 stackpointer) s = returnaddress /\
              WINDOWS_C_ARGUMENTS [rr; point; pp; b] s /\
         ...
         (\s. read RIP s = returnaddress /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(rr,128);
                      memory :> bytes(word_sub stackpointer (word 480),480)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_CURVE25519_LADDERSTEP_ALT_SUBROUTINE_CORRECT));;
