  ...
  REMOVE_THEN assump_name (fun safety_th ->
    X86_SUBROUTINE_SIM_TAC ~is_safety_thm:true
      ((GEN_REWRITE_CONV RAND_CONV [curve25519_x25519_tmc] THENC
        ...
        `pc + 0x3c0f`; `stackpointer:int64`] n
    THENL [
      EXISTS_E2_TAC(ref
...
let CURVE25519_X25519_SAFE = time prove
 (`exists f_events.
       forall e res scalar point pc stackpointer.
           ALL (nonoverlapping (stackpointer,416))
           [word pc,20946; res,32; scalar,32; point,32] /\
           nonoverlapping (res,32) (word pc,20946)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST curve25519_x25519_tmc) /\
                    read RIP s = word (pc + 17) /\
                    read RSP s = stackpointer /\
                    C_ARGUMENTS [res; scalar; point] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 20928) /\
                    (exists e2.
  ...
  ENSURES_EVENTS_WHILE_UP2_TAC `251` `pc + 0xa08` `pc + 0x1ed9`(*+2 insns *)
   `\i s.
      read RSP s = stackpointer /\
      read(memory :> bytes64(word_add stackpointer (word 384))) s = res /\
  ...
  REPEAT CONJ_TAC THENL [
    ...
    X86_STEPS_TAC CURVE25519_X25519_EXEC (1--
      (29 + 19(*SUB_TWICE4*) + 19(*ADD_TWICE4*) + 68(*SQR_4*) + 68(*SQR_4*) +
       19(*SUB_TWICE4*) + 31(*CMADD_4*) + 82(*MUL_4*) + 82(*MUL_4*))) THEN
    X86_STEPS_TAC CURVE25519_X25519_EXEC (350--351) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    X86_STEPS_TAC CURVE25519_X25519_EXEC (1--
      (19(*SUB_TWICE4*) + 19(*ADD_TWICE4*) + 19(*SUB_TWICE4*) +
       19(*ADD_TWICE4*))) THEN
    X86_STEPS_TAC CURVE25519_X25519_EXEC (77--84) THEN
    X86_STEPS_TAC CURVE25519_X25519_EXEC (85--
      (84+(16(*MUX_4*) + 16(*MUX_4*) + 75(*MUL_5*) + 75(*MUL_5*) + 68(*SQR_4*)
          + 29(*SUB5_4*) + 23(*ADD5_4*) + 68(*SQR_4*) + 68(*SQR_4*)
          + 19(*SUB_TWICE4*) + 68(*SQR_4*) + 31(*CMADD_4*) + 82(*MUL_4*)
          + 82(*MUL_4*) + 82(*MUL_4*)))) THEN
    X86_STEPS_TAC CURVE25519_X25519_EXEC (887--889) THEN
    X86_STEPS_TAC CURVE25519_X25519_EXEC (890--891) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    CONJ_TAC THENL [
  ...
  X86_STEPS_TAC CURVE25519_X25519_EXEC (1--4) THEN
  X86_STEPS_TAC CURVE25519_X25519_EXEC (5--
    (4+(16(*MUX_4*) + 16(*MUX_4*)))) THEN

  (*** The first doubling to get from nn/8 to nn/4 ***)

  X86_STEPS_TAC CURVE25519_X25519_EXEC (37--
    (36+(19(*SUB_TWICE4*) + 19(*ADD_TWICE4*) + 68(*SQR_4*) + 68(*SQR_4*) +
         19(*SUB_TWICE4*) + 31(*CMADD_4*) + 82(*MUL_4*) + 82(*MUL_4*)))) THEN

  (*** The next doubling to get from nn/4 to nn/2 ***)

  X86_STEPS_TAC CURVE25519_X25519_EXEC (425--
    (424+(19(*SUB_TWICE4*) + 19(*ADD_TWICE4*) + 68(*SQR_4*) + 68(*SQR_4*) +
         19(*SUB_TWICE4*) + 31(*CMADD_4*) + 82(*MUL_4*) + 82(*MUL_4*)))) THEN

  (*** The final doubling to get from nn/2 to nn ***)

  X86_STEPS_TAC CURVE25519_X25519_EXEC (813--
    (812+(19(*SUB_TWICE4*) + 19(*ADD_TWICE4*) + 68(*SQR_4*) + 68(*SQR_4*) +
         19(*SUB_TWICE4*) + 31(*CMADD_4*) + 82(*MUL_4*) + 82(*MUL_4*)))) THEN

  (*** The inlining of modular inverse ***)

  LOCAL_MODINV_SAFETY_TAC "H_INV_SAFE" 1201 THEN

  (*** Final multiplication ***)

  X86_STEPS_TAC CURVE25519_X25519_EXEC (1202--(1201+(91(*MUL_P25519*)-1))) THEN

  X86_STEPS_TAC CURVE25519_X25519_EXEC (1292--1293) THEN
...
let CURVE25519_X25519_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) curve25519_x25519_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [res; scalar; point] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (res,32);
              memory :> bytes (word_sub stackpointer (word 464),464)])`,
  X86_PROMOTE_RETURN_STACK_TAC
    curve25519_x25519_tmc CURVE25519_X25519_SAFE
...
let CURVE25519_X25519_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) curve25519_x25519_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [res; scalar; point] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (res,32);
              memory :> bytes (word_sub stackpointer (word 464),464)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE CURVE25519_X25519_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let CURVE25519_X25519_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 480),480))
        [word pc,LENGTH curve25519_x25519_windows_tmc; point,32; scalar,32] /\
        nonoverlapping (res,32)
        (word pc,LENGTH curve25519_x25519_windows_tmc) /\
        nonoverlapping (res,32) (word_sub stackpointer (word 480),488)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) curve25519_x25519_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [res; scalar; point] s /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
    curve25519_x25519_windows_tmc curve25519_x25519_tmc
    CURVE25519_X25519_SAFE
    `[RBX; RBP; R12; R13; R14; R15]` 464 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let CURVE25519_X25519_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar point pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 480),480))
        [word pc,LENGTH curve25519_x25519_windows_mc; point,32; scalar,32] /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) curve25519_x25519_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [res; scalar; point] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (res,32);
              memory :> bytes (word_sub stackpointer (word 480),480)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE CURVE25519_X25519_NOIBT_WINDOWS_SUBROUTINE_SAFE));;