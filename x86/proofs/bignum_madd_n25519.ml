...
let BIGNUM_MADD_N25519_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x m y n c r pc stackpointer returnaddress.
      nonoverlapping (word_sub stackpointer (word 48),56) (z,8 * 4) /\
      ALL (nonoverlapping (word_sub stackpointer (word 48),48))
          [(word pc,LENGTH bignum_madd_n25519_cmc); (x,8 * 4); (y,8 * 4); (c,8 * 4)] /\
      nonoverlapping (word pc,LENGTH bignum_madd_n25519_cmc) (z,8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) bignum_madd_n25519_cmc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                C_ARGUMENTS [z; x; y; c] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,4);
                      memory :> bytes(word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MADD_N25519_SUBROUTINE_CORRECT));;
...
let WINDOWS_BIGNUM_MADD_N25519_SUBROUTINE_CORRECT = time prove
 (`!z x m y n c r pc stackpointer returnaddress.
      ...
      ALL (nonoverlapping (word_sub stackpointer (word 64),64))
          [(word pc,LENGTH windows_bignum_madd_n25519_mc); (x,8 * 4); (y,8 * 4); (c,8 * 4)] /\
      nonoverlapping (word pc,LENGTH windows_bignum_madd_n25519_mc) (z,8 * 4)
      ==> ensures x86
...
let WINDOWS_BIGNUM_MADD_N25519_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x m y n c r pc stackpointer returnaddress.
      nonoverlapping (word_sub stackpointer (word 64),72) (z,8 * 4) /\
      ALL (nonoverlapping (word_sub stackpointer (word 64),64))
          [(word pc,LENGTH windows_bignum_madd_n25519_cmc); (x,8 * 4); (y,8 * 4); (c,8 * 4)] /\
      nonoverlapping (word pc,LENGTH windows_bignum_madd_n25519_cmc) (z,8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) windows_bignum_madd_n25519_cmc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                WINDOWS_C_ARGUMENTS [z; x; y; c] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,4);
                      memory :> bytes(word_sub stackpointer (word 64),64)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_MADD_N25519_SUBROUTINE_CORRECT));;
