...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_tomont_p384" subroutine_signatures)
    BIGNUM_TOMONT_P384_CORRECT
    BIGNUM_TOMONT_P384_EXEC;;

let BIGNUM_TOMONT_P384_SAFE = time prove
 (`exists f_events.
       forall e z x pc.
           nonoverlapping (word pc,1103) (z,8 * 6)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_tomont_p384_tmc) /\
                    read RIP s = word (pc + 9) /\
                    C_ARGUMENTS [z; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 1093) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events x z pc /\
                         memaccess_inbounds e2 [x,48; z,48] [z,48]))
               (MAYCHANGE
                [RIP; RSI; RAX; RBP; RCX; RDX; R8; R9; R10; R11; R12; R13;
                 R14; R15] ,,
                MAYCHANGE [memory :> bytes (z,8 * 6)] ,,
                MAYCHANGE SOME_FLAGS ,,
                MAYCHANGE [events])`,
  ASSERT_CONCL_TAC full_spec THEN
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_TOMONT_P384_EXEC);;

let BIGNUM_TOMONT_P384_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
           ...
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_tomont_p384_tmc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [z; x] s /\
               ...
               (\s.
                    ...
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events x z pc (word_sub stackpointer (word 40))
                                       returnaddress /\
               ...
               (MAYCHANGE [RSP] ,,
                ...
                [memory :> bytes (z,8 * 6);
                 memory :> bytes (word_sub stackpointer (word 40),40)])`,
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_tomont_p384_tmc BIGNUM_TOMONT_P384_SAFE
   `[RBP; R12; R13; R14; R15]` 40 THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_TOMONT_P384_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
           nonoverlapping (z,8 * 6) (word_sub stackpointer (word 40),48) /\
           ALL (nonoverlapping (word_sub stackpointer (word 40),40))
           ...
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_tomont_p384_mc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [z; x] s /\
               ...
               (MAYCHANGE [RSP] ,,
                ...
                [memory :> bytes (z,8 * 6);
                 memory :> bytes (word_sub stackpointer (word 40),40)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_TOMONT_P384_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_TOMONT_P384_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
           nonoverlapping (z,8 * 6) (word_sub stackpointer (word 56),64) /\
           ALL (nonoverlapping (word_sub stackpointer (word 56),56))
           [word pc,LENGTH bignum_tomont_p384_windows_tmc; x,8 * 6] /\
           nonoverlapping (word pc,LENGTH bignum_tomont_p384_windows_tmc)
           (z,8 * 6)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_tomont_p384_windows_tmc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    WINDOWS_C_ARGUMENTS [z; x] s /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   bignum_tomont_p384_windows_tmc bignum_tomont_p384_tmc
   BIGNUM_TOMONT_P384_SAFE `[RBP; R12; R13; R14; R15]` 40 THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_TOMONT_P384_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
           nonoverlapping (z,8 * 6) (word_sub stackpointer (word 56),64) /\
           ALL (nonoverlapping (word_sub stackpointer (word 56),56))
           [word pc,LENGTH bignum_tomont_p384_windows_mc; x,8 * 6] /\
           nonoverlapping (word pc,LENGTH bignum_tomont_p384_windows_mc)
           (z,8 * 6)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_tomont_p384_windows_mc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    WINDOWS_C_ARGUMENTS [z; x] s /\
               ...
               (MAYCHANGE [RSP] ,,
                ...
                [memory :> bytes (z,8 * 6);
                 memory :> bytes (word_sub stackpointer (word 56),56)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_TOMONT_P384_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
