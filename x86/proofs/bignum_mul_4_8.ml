...
let BIGNUM_MUL_4_8_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
     nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 8) /\
     ALL (nonoverlapping (word_sub stackpointer (word 16),16))
         [(word pc,LENGTH bignum_mul_4_8_cmc); (x,8 * 4); (y,8 * 4)] /\
     nonoverlapping (word pc,LENGTH bignum_mul_4_8_cmc) (z,8 * 8) /\
     (y = z \/ nonoverlapping (y,8 * 4) (z,8 * 8)) /\
     nonoverlapping (x,8 * 4) (z,8 * 8)
     ==> ensures x86
          (\s. bytes_loaded s (word pc) bignum_mul_4_8_cmc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               C_ARGUMENTS [z; x; y] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(z,8 * 8);
                memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MUL_4_8_SUBROUTINE_CORRECT));;
...
let WINDOWS_BIGNUM_MUL_4_8_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
     ...
     ALL (nonoverlapping (word_sub stackpointer (word 32),32))
         [(word pc,LENGTH windows_bignum_mul_4_8_mc); (x,8 * 4); (y,8 * 4)] /\
     nonoverlapping (word pc,LENGTH windows_bignum_mul_4_8_mc) (z,8 * 8) /\
     (y = z \/ nonoverlapping (y,8 * 4) (z,8 * 8)) /\
     nonoverlapping (x,8 * 4) (z,8 * 8)
     ==> ensures x86
...
let WINDOWS_BIGNUM_MUL_4_8_IBT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
     nonoverlapping (word_sub stackpointer (word 32),40) (z,8 * 8) /\
     ALL (nonoverlapping (word_sub stackpointer (word 32),32))
         [(word pc,LENGTH windows_bignum_mul_4_8_cmc); (x,8 * 4); (y,8 * 4)] /\
     nonoverlapping (word pc,LENGTH windows_bignum_mul_4_8_cmc) (z,8 * 8) /\
     (y = z \/ nonoverlapping (y,8 * 4) (z,8 * 8)) /\
     nonoverlapping (x,8 * 4) (z,8 * 8)
     ==> ensures x86
          (\s. bytes_loaded s (word pc) windows_bignum_mul_4_8_cmc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               WINDOWS_C_ARGUMENTS [z; x; y] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(z,8 * 8);
                memory :> bytes(word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_BIGNUM_MUL_4_8_SUBROUTINE_CORRECT));;
