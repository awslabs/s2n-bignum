...
let word_ctz_cmc = define_assert_from_elf "word_ctz_cmc" "x86/generic/word_ctz.o"
[
...
let word_ctz_mc = define_trimmed "word_ctz_mc" word_ctz_cmc;;
...
let WORD_CTZ_IBT_SUBROUTINE_CORRECT = prove
 (`!a pc stackpointer returnaddress.
        ensures x86
          (\s. bytes_loaded s (word pc) word_ctz_cmc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               C_ARGUMENTS [a] s)
          (\s. read RIP s = returnaddress /\
               ...
               C_RETURN s = word(word_ctz a))
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI)`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WORD_CTZ_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_word_ctz_cmc = define_from_elf
   "windows_word_ctz_cmc" "x86/generic/word_ctz.obj";;

let windows_word_ctz_mc = define_trimmed "windows_word_ctz_mc" windows_word_ctz_cmc;;

let WINDOWS_WORD_CTZ_SUBROUTINE_CORRECT = prove
 (`!a pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 16),16) (word pc,LENGTH windows_word_ctz_mc)
        ==> ensures x86
...
let WINDOWS_WORD_CTZ_IBT_SUBROUTINE_CORRECT = prove
 (`!a pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 16),16) (word pc,LENGTH windows_word_ctz_cmc)
        ==> ensures x86
              (\s. bytes_loaded s (word pc) windows_word_ctz_cmc /\
                   read RIP s = word pc /\
                   read RSP s = stackpointer /\
                   read (memory :> bytes64 stackpointer) s = returnaddress /\
                   WINDOWS_C_ARGUMENTS [a] s)
              ...
              (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_WORD_CTZ_SUBROUTINE_CORRECT));;
