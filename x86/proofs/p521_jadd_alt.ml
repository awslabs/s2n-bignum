...
let P521_JADD_ALT_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 p2 pc stackpointer.
           ALL (nonoverlapping (stackpointer,576))
           [word pc,35907; p1,216; p2,216; p3,216] /\
           nonoverlapping (p3,216) (word pc,35907)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST p521_jadd_alt_tmc) /\
                    read RIP s = word (pc + 17) /\
                    read RSP s = stackpointer /\
                    C_ARGUMENTS [p3; p1; p2] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 35889) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars P521_JADD_ALT_EXEC);;

let P521_JADD_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 624),624))
        [word pc,LENGTH p521_jadd_alt_tmc; p1,216; p2,216] /\
        ALL (nonoverlapping (p3,216))
        [word pc,LENGTH p521_jadd_alt_tmc;
         word_sub stackpointer (word 624),632]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p521_jadd_alt_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      read events s = APPEND e2 e /\
                      e2 =
                      f_events p1 p2 p3 pc (word_sub stackpointer (word 624))
                      returnaddress /\
                      memaccess_inbounds e2
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,216);
              memory :> bytes (word_sub stackpointer (word 624),624)])`,
  X86_PROMOTE_RETURN_STACK_TAC p521_jadd_alt_tmc P521_JADD_ALT_SAFE
    `[RBX; RBP; R12; R13; R14; R15]` 624 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let P521_JADD_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 624),624))
        [word pc,LENGTH p521_jadd_alt_mc; p1,216; p2,216] /\
        ALL (nonoverlapping (p3,216))
        [word pc,LENGTH p521_jadd_alt_mc;
         word_sub stackpointer (word 624),632]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p521_jadd_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,216);
              memory :> bytes (word_sub stackpointer (word 624),624)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P521_JADD_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let P521_JADD_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 640),640))
        [word pc,LENGTH p521_jadd_alt_windows_tmc; p1,216; p2,216] /\
        ALL (nonoverlapping (p3,216))
        [word pc,LENGTH p521_jadd_alt_windows_tmc;
         word_sub stackpointer (word 640),648]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p521_jadd_alt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      read events s = APPEND e2 e /\
                      e2 =
                      f_events p1 p2 p3 pc (word_sub stackpointer (word 640))
                      returnaddress /\
                      memaccess_inbounds e2
                      [p1,216; p2,216; p3,216;
                       word_sub stackpointer (word 640),648]
...
let P521_JADD_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 640),640))
        [word pc,LENGTH p521_jadd_alt_windows_mc; p1,216; p2,216] /\
        ALL (nonoverlapping (p3,216))
        [word pc,LENGTH p521_jadd_alt_windows_mc;
         word_sub stackpointer (word 640),648]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p521_jadd_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,216);
              memory :> bytes (word_sub stackpointer (word 640),640)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P521_JADD_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;