...
let p521_jadd_alt_cmc = define_assert_from_elf
  "p521_jadd_alt_cmc" "x86/p521/p521_jadd_alt.o"
[
...
let p521_jadd_alt_mc = define_trimmed "p521_jadd_alt_mc" p521_jadd_alt_cmc;;
...
let P521_JADD_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 624),624))
            [(word pc,LENGTH p521_jadd_alt_cmc); (p1,216); (p2,216)] /\
        ALL (nonoverlapping (p3,216))
            [(word pc,LENGTH p521_jadd_alt_cmc); (word_sub stackpointer (word 624),632)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) p521_jadd_alt_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [p3; p1; p2] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,216);
                      memory :> bytes(word_sub stackpointer (word 624),624)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P521_JADD_ALT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_p521_jadd_alt_cmc = define_from_elf "windows_p521_jadd_alt_cmc"
      "x86/p521/p521_jadd_alt.obj";;

let windows_p521_jadd_alt_mc = define_trimmed "windows_p521_jadd_alt_mc" windows_p521_jadd_alt_cmc;;

let WINDOWS_P521_JADD_ALT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 640),640))
            [(word pc,LENGTH windows_p521_jadd_alt_mc); (p1,216); (p2,216)] /\
        ALL (nonoverlapping (p3,216))
            [(word pc,LENGTH windows_p521_jadd_alt_mc); (word_sub stackpointer (word 640),648)]
        ==> ensures x86
...
let WINDOWS_P521_JADD_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 640),640))
            [(word pc,LENGTH windows_p521_jadd_alt_cmc); (p1,216); (p2,216)] /\
        ALL (nonoverlapping (p3,216))
            [(word pc,LENGTH windows_p521_jadd_alt_cmc); (word_sub stackpointer (word 640),648)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_p521_jadd_alt_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,216);
                      memory :> bytes(word_sub stackpointer (word 640),640)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_P521_JADD_ALT_SUBROUTINE_CORRECT));;
