...
let BIGNUM_SQR_P521_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer.
           ALL (nonoverlapping (stackpointer,64))
           [word pc,1247; z,8 * 9; x,8 * 9] /\
           nonoverlapping (z,8 * 9) (word pc,1247)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_sqr_p521_tmc) /\
                    read RIP s = word (pc + 13) /\
                    read RSP s = stackpointer /\
                    C_ARGUMENTS [z; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 1233) /\
                    (exists e2.
...
let BIGNUM_SQR_P521_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
           ALL (nonoverlapping (z,8 * 9))
           [word pc,LENGTH bignum_sqr_p521_tmc;
            word_sub stackpointer (word 104),112] /\
           ALL (nonoverlapping (word_sub stackpointer (word 104),104))
           [word pc,LENGTH bignum_sqr_p521_tmc; x,8 * 9]
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_sqr_p521_tmc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [z; x] s /\
               ...
               (\s.
                    ...
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events x z pc (word_sub stackpointer (word 104))
                                       returnaddress /\
                         memaccess_inbounds e2
                         [x,72; z,72; word_sub stackpointer (word 104),64;
                          word_sub stackpointer (word 40),48]
                         [z,72; word_sub stackpointer (word 104),64;
                          word_sub stackpointer (word 40),40]))
               (MAYCHANGE [RSP] ,,
                ...
                [memory :> bytes (z,8 * 9);
                 memory :> bytes (word_sub stackpointer (word 104),104)])`,
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_sqr_p521_tmc BIGNUM_SQR_P521_SAFE
   `[RBP; R12; R13; R14; R15]` 104 THEN
...
let BIGNUM_SQR_P521_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
           ...
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_sqr_p521_mc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [z; x] s /\
               ...
               (MAYCHANGE [RSP] ,,
                ...
                [memory :> bytes (z,8 * 9);
                 memory :> bytes (word_sub stackpointer (word 104),104)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_P521_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_SQR_P521_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
           ALL (nonoverlapping (z,8 * 9))
           [word pc,LENGTH bignum_sqr_p521_windows_tmc;
            word_sub stackpointer (word 120),128] /\
           ALL (nonoverlapping (word_sub stackpointer (word 120),120))
           [word pc,LENGTH bignum_sqr_p521_windows_tmc; x,8 * 9]
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_sqr_p521_windows_tmc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    WINDOWS_C_ARGUMENTS [z; x] s /\
               ...
               (\s.
                    ...
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events x z pc (word_sub stackpointer (word 120)) returnaddress /\
                         memaccess_inbounds e2
                         [x,72; z,72; (word_sub stackpointer (word 120)),64;
                          word_sub stackpointer (word 56),64]
                         [z,72; word_sub stackpointer (word 120),64;
                          word_sub stackpointer (word 56),56]))
               (MAYCHANGE [RSP] ,,
                ...
                [memory :> bytes (z,8 * 9);
                 memory :> bytes (word_sub stackpointer (word 120),120)])`,
  WINDOWS_X86_WRAP_STACK_TAC
   bignum_sqr_p521_windows_tmc bignum_sqr_p521_tmc BIGNUM_SQR_P521_SAFE
   `[RBP; R12; R13; R14; R15]` 104 THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_SQR_P521_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
           ALL (nonoverlapping (z,8 * 9))
           [word pc,LENGTH bignum_sqr_p521_windows_mc;
            word_sub stackpointer (word 120),128] /\
           ALL (nonoverlapping (word_sub stackpointer (word 120),120))
           [word pc,LENGTH bignum_sqr_p521_windows_mc; x,8 * 9]
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_sqr_p521_windows_mc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    WINDOWS_C_ARGUMENTS [z; x] s /\
               ...
               (MAYCHANGE [RSP] ,,
                ...
                [memory :> bytes (z,8 * 9);
                 memory :> bytes (word_sub stackpointer (word 120),120)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_P521_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
