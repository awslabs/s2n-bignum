  ...
  REMOVE_THEN assump_name (fun safety_th ->
    X86_SUBROUTINE_SIM_TAC ~is_safety_thm:true
       (curve25519_x25519base_tmc,
        ...
        `pc + 0x18b4`; `stackpointer:int64`] n
    THENL [
      EXISTS_E2_TAC(ref
...
let CURVE25519_X25519BASE_SAFE = time prove
 (`exists f_events.
       forall e res scalar pc stackpointer.
           ALL (nonoverlapping (stackpointer,488))
           [word pc,60471; res,32; scalar,32] /\
           nonoverlapping (res,32) (word pc,60471)
           ==> ensures x86
               (\s.
                    ...
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 11877) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events scalar res pc stackpointer /\
                         memaccess_inbounds e2
                         [scalar,32; res,32; stackpointer,488;
                          word (pc + LENGTH curve25519_x25519base_tmc),
                          LENGTH curve25519_x25519base_data]
  ...
  REWRITE_TAC[CONV_RULE (RAND_CONV LENGTH_CONV)
     (AP_TERM `LENGTH:byte list->num` curve25519_x25519base_data)] THEN
  REWRITE_TAC[BYTES_LOADED_DATA] THEN

  (*** Setup of the main loop ***)

  ENSURES_EVENTS_WHILE_UP2_TAC `63` `pc + 0x19a` `pc + 0x17b6`(*+2 insns *)
   `\i s.
      read (memory :> bytes(word(pc + 0x2e77),48576)) s =
          num_of_bytelist curve25519_x25519base_data /\
      read RSP s = stackpointer /\
  ...
  REPEAT CONJ_TAC THENL [
    ...
    X86_STEPS_TAC CURVE25519_X25519BASE_EXEC (948--989) THEN (* missed.. *)
...
let CURVE25519_X25519BASE_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 536),536))
        [word pc,LENGTH
            (APPEND curve25519_x25519base_mc curve25519_x25519base_data);
         scalar,32] /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      memaccess_inbounds e2
                          [scalar,32; res,32;
                           word_sub stackpointer (word 536),544;
                           word (pc + LENGTH curve25519_x25519base_mc),
                              LENGTH curve25519_x25519base_data]
                          [res,32; word_sub stackpointer (word 536),536]))
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             ...
             [memory :> bytes (res,32);
              memory :> bytes (word_sub stackpointer (word 536),536)])`,
  MP_TAC (ADD_IBT_RULE CURVE25519_X25519BASE_NOIBT_SUBROUTINE_SAFE) THEN
  REWRITE_TAC[LENGTH_APPEND] THEN
  REWRITE_TAC[CONV_RULE (RAND_CONV LENGTH_CONV)
     (AP_TERM `LENGTH:byte list->num` curve25519_x25519base_data)] THEN
  REWRITE_TAC[CONV_RULE (RAND_CONV LENGTH_CONV)
     (AP_TERM `LENGTH:byte list->num` curve25519_x25519base_mc)] THEN
  NUM_REDUCE_TAC);;
...
let CURVE25519_X25519BASE_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e res scalar pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 560),560))
        [word pc,60487; scalar,32] /\
        nonoverlapping (res,32) (word pc,60487) /\
        nonoverlapping (res,32) (word_sub stackpointer (word 560),568)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc)
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      [scalar,32; res,32;
                       ...
                       word (pc + LENGTH curve25519_x25519base_windows_tmc),
                              LENGTH curve25519_x25519base_windows_data]
  ...
  let subth =
    REWRITE_RULE[BYTES_LOADED_APPEND_CLAUSE; BYTES_LOADED_DATA;
      fst CURVE25519_X25519BASE_EXEC;
      CONV_RULE (RAND_CONV LENGTH_CONV)
         (AP_TERM `LENGTH:byte list->num` curve25519_x25519base_data)] baseth in
  ...
  GEN_REWRITE_TAC
   ...
   [bytes_loaded] THEN
  REWRITE_TAC[READ_BYTELIST_EQ_BYTES; CONV_RULE (RAND_CONV LENGTH_CONV)
     (AP_TERM `LENGTH:byte list->num` curve25519_x25519base_windows_data)] THEN
  REWRITE_TAC[curve25519_x25519base_windows_data] THEN
  REWRITE_TAC[GSYM curve25519_x25519base_data] THEN
  ENSURES_PRESERVED_TAC "rsi_init" `RSI` THEN
  ENSURES_PRESERVED_TAC "rdi_init" `RDI` THEN
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      memaccess_inbounds e2
                        [scalar,32; res,32;
                         word_sub stackpointer (word 560),568;
                         word (pc + LENGTH curve25519_x25519base_windows_mc),
                                LENGTH curve25519_x25519base_windows_data]
                        [res,32; word_sub stackpointer (word 560),560]))
            (MAYCHANGE [RSP] ,,
             WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             ...
             [memory :> bytes (res,32);
              memory :> bytes (word_sub stackpointer (word 560),560)])`,
  MP_TAC (ADD_IBT_RULE CURVE25519_X25519BASE_NOIBT_WINDOWS_SUBROUTINE_SAFE) THEN
  REWRITE_TAC[LENGTH_APPEND] THEN
  REWRITE_TAC[CONV_RULE (RAND_CONV LENGTH_CONV)
     (AP_TERM `LENGTH:byte list->num` curve25519_x25519base_windows_data)] THEN
  REWRITE_TAC[CONV_RULE (RAND_CONV LENGTH_CONV)
     (AP_TERM `LENGTH:byte list->num` curve25519_x25519base_windows_mc)] THEN
  NUM_REDUCE_TAC);;