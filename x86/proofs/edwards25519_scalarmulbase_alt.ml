...
(**** print_literal_relocs_from_elf "x86/curve25519/edwards25519_scalarmulbase_alt.o";;
 ****)

let edwards25519_scalarmulbase_alt_body : 'a. int list -> (int list -> 'a) ->
                                              (string * int -> 'a) -> 'a list =
...
let edwards25519_scalarmulbase_alt_mc,
    (edwards25519_scalarmulbase_alt_constant_data::_) =
    define_assert_relocs_from_elf "edwards25519_scalarmulbase_alt_mc"
    "x86/curve25519/edwards25519_scalarmulbase_alt.o"
    (fun b ADDR -> edwards25519_scalarmulbase_alt_body
       [0xf3; 0x0f; 0x1e; 0xfa]  (* ENDBR64 *)
       b ADDR);;
let edwards25519_scalarmulbase_alt_tmc =
  define_trimmed "edwards25519_scalarmulbase_alt_tmc" edwards25519_scalarmulbase_alt_mc;;

let EDWARDS25519_SCALARMULBASE_ALT_EXEC =
  X86_MK_EXEC_RULE edwards25519_scalarmulbase_alt_tmc;;

let edwards25519_scalarmulbase_alt_tmc' =
  SPECL [`pc:num`; `tables:num`] edwards25519_scalarmulbase_alt_tmc;;
...
let BYTES_LOADED_DATA = prove
 (`bytes_loaded s tab edwards25519_scalarmulbase_alt_constant_data <=>
  ...
  REWRITE_TAC[bytes_loaded; READ_BYTELIST_EQ_BYTES;
    CONV_RULE (RAND_CONV LENGTH_CONV)
     (AP_TERM `LENGTH:byte list->num` edwards25519_scalarmulbase_alt_constant_data)]);;
...
let LOCAL_MUL_P25519_TAC =
  X86_MACRO_SIM_ABBREV_TAC edwards25519_scalarmulbase_alt_tmc' 129 lvs
   `!(t:x86state) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      ==>
      nonoverlapping (word pc,0x3174) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (edwards25519_scalarmulbase_alt_tmc pc tables) /\
                read RIP s = pcin /\
...
let LOCAL_MUL_4_TAC =
  X86_MACRO_SIM_ABBREV_TAC edwards25519_scalarmulbase_alt_tmc' 120 lvs
   `!(t:x86state) pcin pcout q3 n3 q1 n1 q2 n2.
      ...
      ==>
      nonoverlapping (word pc,0x3174) (word_add (read q3 t) (word n3),8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (edwards25519_scalarmulbase_alt_tmc pc tables) /\
                read RIP s = pcin /\
                read RSP s = read RSP t /\
...
let EDWARDS25519_SCALARMULBASE_ALT_CORRECT = time prove
 (`!tables res scalar n pc stackpointer.
    ...
    ALL (nonoverlapping (stackpointer,488))
        [(word pc,0x3174); (word tables,48576); (res,64); (scalar,32)] /\
    ALL (nonoverlapping (res,64)) [(word pc,0x3174); (word tables,48576)]
    ==> ensures x86
         (\s. bytes_loaded s (word pc) (edwards25519_scalarmulbase_alt_tmc pc tables) /\
  ...
  ENSURES_WHILE_UP_TAC `63` `pc + 0x1f1` `pc + 0x18a7`
   `\i s.
...
let EDWARDS25519_SCALARMULBASE_ALT_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!tables res scalar n pc stackpointer returnaddress.
    ...
    ALL (nonoverlapping (word_sub stackpointer (word 536),536))
        [(word pc,LENGTH (edwards25519_scalarmulbase_alt_tmc pc tables));
    ...
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
                (edwards25519_scalarmulbase_alt_tmc pc tables) /\
              read RIP s = word pc /\
              bytes_loaded s (word tables)
                edwards25519_scalarmulbase_alt_constant_data /\
              read RSP s = stackpointer /\
...
let EDWARDS25519_SCALARMULBASE_ALT_MC_BRIDGE = prove
 (`!pc tables. edwards25519_scalarmulbase_alt_mc pc tables =
     APPEND [word 0xf3:byte; word 0x0f; word 0x1e; word 0xfa]
            (edwards25519_scalarmulbase_alt_tmc (pc + 4) tables)`,
  REPEAT GEN_TAC THEN
  REWRITE_TAC[edwards25519_scalarmulbase_alt_mc;
              edwards25519_scalarmulbase_alt_tmc; APPEND] THEN
...
let LENGTH_EDWARDS25519_SCALARMULBASE_ALT_MC = prove
 (`!pc tables. LENGTH (edwards25519_scalarmulbase_alt_mc pc tables) = 0x3178`,
  REWRITE_TAC[EDWARDS25519_SCALARMULBASE_ALT_MC_BRIDGE; LENGTH_APPEND; LENGTH;
              fst EDWARDS25519_SCALARMULBASE_ALT_EXEC] THEN ARITH_TAC);;

let EDWARDS25519_SCALARMULBASE_ALT_SUBROUTINE_CORRECT = time prove
 (`!tables res scalar n pc stackpointer returnaddress.
    riprel32_within_bounds tables (pc + 178) /\
    ALL (nonoverlapping (word_sub stackpointer (word 536),536))
        [(word pc,LENGTH (edwards25519_scalarmulbase_alt_mc pc tables));
         (word tables,48576); (scalar,32)] /\
    ALL (nonoverlapping (res,64))
        [(word pc,LENGTH (edwards25519_scalarmulbase_alt_mc pc tables));
         (word tables,48576);
    ...
    ==> ensures x86
         ...
         (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
          MAYCHANGE [memory :> bytes(res,64);
                     memory :> bytes(word_sub stackpointer (word 536),536)])`,
  REWRITE_TAC[LENGTH_EDWARDS25519_SCALARMULBASE_ALT_MC] THEN
  MATCH_ACCEPT_TAC(ADD_IBT_RULE
                     ~bridge:(Some EDWARDS25519_SCALARMULBASE_ALT_MC_BRIDGE)
                     (REWRITE_RULE[fst EDWARDS25519_SCALARMULBASE_ALT_EXEC]
                       EDWARDS25519_SCALARMULBASE_ALT_NOIBT_SUBROUTINE_CORRECT)));;
...
let edwards25519_scalarmulbase_alt_windows_mc,_ =
...
let edwards25519_scalarmulbase_alt_windows_tmc =
  define_trimmed "edwards25519_scalarmulbase_alt_windows_tmc"
                 edwards25519_scalarmulbase_alt_windows_mc;;

let WINDOWS_EDWARDS25519_SCALARMULBASE_ALT_EXEC =
  X86_MK_EXEC_RULE edwards25519_scalarmulbase_alt_windows_tmc;;

(* Bridge equation analogous to EDWARDS25519_SCALARMULBASE_ALT_MC_BRIDGE for the
   Windows mc/tmc pair. Used to apply ADD_IBT_RULE on rodata-aware
   parametrized mcs. *)
let EDWARDS25519_SCALARMULBASE_ALT_WINDOWS_MC_BRIDGE = prove
 (`!pc tables. edwards25519_scalarmulbase_alt_windows_mc pc tables =
     APPEND [word 0xf3:byte; word 0x0f; word 0x1e; word 0xfa]
            (edwards25519_scalarmulbase_alt_windows_tmc (pc + 4) tables)`,
  REPEAT GEN_TAC THEN
  REWRITE_TAC[edwards25519_scalarmulbase_alt_windows_mc;
              edwards25519_scalarmulbase_alt_windows_tmc; APPEND] THEN
  REWRITE_TAC[GSYM INT_OF_NUM_ADD] THEN
  AP_TERM_TAC THEN AP_TERM_TAC THEN AP_TERM_TAC THEN
...
(* Length of the Windows IBT mc, derived from the bridge and the Windows
   trimmed mc length. *)
let LENGTH_EDWARDS25519_SCALARMULBASE_ALT_WINDOWS_MC = prove
 (`!pc tables. LENGTH (edwards25519_scalarmulbase_alt_windows_mc pc tables) = 0x3188`,
  REWRITE_TAC[EDWARDS25519_SCALARMULBASE_ALT_WINDOWS_MC_BRIDGE;
              LENGTH_APPEND; LENGTH;
              fst WINDOWS_EDWARDS25519_SCALARMULBASE_ALT_EXEC] THEN ARITH_TAC);;
...
let EDWARDS25519_SCALARMULBASE_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!tables res scalar n pc stackpointer returnaddress.
    ...
    ALL (nonoverlapping (word_sub stackpointer (word 560),560))
        [(word pc,LENGTH (edwards25519_scalarmulbase_alt_windows_tmc pc tables));
         (word tables,48576); (scalar,32)] /\
    ALL (nonoverlapping (res,64))
        [(word pc,LENGTH (edwards25519_scalarmulbase_alt_windows_tmc pc tables));
         (word tables,48576);
    ...
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
                (edwards25519_scalarmulbase_alt_windows_tmc pc tables) /\
              read RIP s = word pc /\
              bytes_loaded s (word tables)
                edwards25519_scalarmulbase_alt_constant_data /\
              read RSP s = stackpointer /\
  ...
  REWRITE_TAC[BYTES_LOADED_DATA;
              fst WINDOWS_EDWARDS25519_SCALARMULBASE_ALT_EXEC] THEN
...
let EDWARDS25519_SCALARMULBASE_ALT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!tables res scalar n pc stackpointer returnaddress.
    ...
    ALL (nonoverlapping (word_sub stackpointer (word 560),560))
        [(word pc,LENGTH (edwards25519_scalarmulbase_alt_windows_mc pc tables));
         (word tables,48576); (scalar,32)] /\
    ALL (nonoverlapping (res,64))
        [(word pc,LENGTH (edwards25519_scalarmulbase_alt_windows_mc pc tables));
         (word tables,48576);
    ...
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
                (edwards25519_scalarmulbase_alt_windows_mc pc tables) /\
              read RIP s = word pc /\
              bytes_loaded s (word tables)
                edwards25519_scalarmulbase_alt_constant_data /\
              read RSP s = stackpointer /\
         ...
         (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
          MAYCHANGE [memory :> bytes(res,64);
                     memory :> bytes(word_sub stackpointer (word 560),560)])`,
  REWRITE_TAC[LENGTH_EDWARDS25519_SCALARMULBASE_ALT_WINDOWS_MC] THEN
  MATCH_ACCEPT_TAC(ADD_IBT_RULE
                     ~bridge:(Some EDWARDS25519_SCALARMULBASE_ALT_WINDOWS_MC_BRIDGE)
                     (REWRITE_RULE[fst WINDOWS_EDWARDS25519_SCALARMULBASE_ALT_EXEC]
                       EDWARDS25519_SCALARMULBASE_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT)));;
...
let LOCAL_MODINV_SAFETY_TAC (assump_name:string) (n:int) =
  REMOVE_THEN assump_name (fun safety_th ->
    X86_SUBROUTINE_SIM_TAC ~is_safety_thm:true
        ...
        `pc + 0x194b`; `stackpointer:int64`] n
    THENL [
      EXISTS_E2_TAC(ref
...
(* The hand-written precondition below uses the leaner
   `nonoverlapping (res,64) (word pc,0x3174)` form. This is intentionally
...
let EDWARDS25519_SCALARMULBASE_ALT_SAFE = time prove
 (`exists f_events.
       forall e tables res scalar pc stackpointer.
           ...
           ALL (nonoverlapping (stackpointer,488))
           [word pc,0x3174; word tables,48576; res,64; scalar,32] /\
           nonoverlapping (res,64) (word pc,0x3174)
           ==> ensures x86
               (\s.
  ...
  ENSURES_EVENTS_WHILE_UP2_TAC `63` `pc + 0x1f1` `pc + 0x18b9`(*+2 insn *)
   `\i s.
      read (memory :> bytes(word tables,48576)) s =
      num_of_bytelist edwards25519_scalarmulbase_alt_constant_data /\
      read RSP s = stackpointer /\
...
let EDWARDS25519_SCALARMULBASE_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
  ...
  MATCH_ACCEPT_TAC(ADD_IBT_RULE
                     ~bridge:(Some EDWARDS25519_SCALARMULBASE_ALT_MC_BRIDGE)
                     (REWRITE_RULE[fst EDWARDS25519_SCALARMULBASE_ALT_EXEC]
                       EDWARDS25519_SCALARMULBASE_ALT_NOIBT_SUBROUTINE_SAFE)));;
...
let EDWARDS25519_SCALARMULBASE_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
  ...
  MATCH_ACCEPT_TAC(ADD_IBT_RULE
                     ~bridge:(Some EDWARDS25519_SCALARMULBASE_ALT_WINDOWS_MC_BRIDGE)
                     (REWRITE_RULE[fst WINDOWS_EDWARDS25519_SCALARMULBASE_ALT_EXEC]
                       EDWARDS25519_SCALARMULBASE_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE)));;
