...
(**** print_literal_relocs_from_elf "x86/curve25519/edwards25519_scalarmulbase.o";;
 ****)

let edwards25519_scalarmulbase_body : 'a. int list -> (int list -> 'a) ->
                                          (string * int -> 'a) -> 'a list =
  ...
  0x4c; 0x8d; 0x15]); ADDR ("WHOLE_READONLY_data",(-4)); b[
                           (* LEA (% r10) (Riprel (word_sx (iword (&WHOLE_READONLY_data - (&pc + &178))))) *)
...
let edwards25519_scalarmulbase_mc,
    (edwards25519_scalarmulbase_constant_data::_) =
    define_assert_relocs_from_elf "edwards25519_scalarmulbase_mc"
    "x86/curve25519/edwards25519_scalarmulbase.o"
    (fun b ADDR -> edwards25519_scalarmulbase_body
       [0xf3; 0x0f; 0x1e; 0xfa]  (* ENDBR64 *)
       b ADDR);;
let edwards25519_scalarmulbase_tmc =
  define_trimmed "edwards25519_scalarmulbase_tmc" edwards25519_scalarmulbase_mc;;

let EDWARDS25519_SCALARMULBASE_EXEC =
  X86_MK_EXEC_RULE edwards25519_scalarmulbase_tmc;;

let edwards25519_scalarmulbase_tmc' =
  SPECL [`pc:num`; `tables:num`] edwards25519_scalarmulbase_tmc;;
...
let LOCAL_MUL_P25519_TAC =
  X86_MACRO_SIM_ABBREV_TAC edwards25519_scalarmulbase_tmc' 91 lvs
   `!(t:x86state) pcin pcout q3 n3 q1 n1 q2 n2.
      ...
      ==>
      nonoverlapping (word pc,0x3076) (word_add (read q3 t) (word n3),8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (edwards25519_scalarmulbase_tmc pc tables) /\
                read RIP s = pcin /\
...
let EDWARDS25519_SCALARMULBASE_CORRECT = time prove
 (`!tables res scalar n pc stackpointer.
    riprel32_within_bounds tables (pc + 174) /\
    ALL (nonoverlapping (stackpointer,488))
        [(word pc,0x3076); (word tables,48576); (res,64); (scalar,32)] /\
    ALL (nonoverlapping (res,64)) [(word pc,0x3076); (word tables,48576)]
    ==> ensures x86
         (\s. bytes_loaded s (word pc) (edwards25519_scalarmulbase_tmc pc tables) /\
  ...
  ENSURES_WHILE_UP_TAC `63` `pc + 0x1f1` `pc + 0x17fb`
   `\i s.
  ...
  REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    UNDISCH_TAC `read R10 s39 =
        word(val (word (pc + 174):(64)word) +
             val (word_sx (iword (&tables - (&pc + &174)):int32):(64)word):num)` THEN
    IMP_REWRITE_TAC[RIP_REL_ADDR_FOLD] THEN
    ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
    UNDISCH_TAC `read R11 s39 =
        word_add
        (word (val (word (pc + 174):(64)word) +
               val (word_sx (iword (&tables - (&pc + &174)):int32):(64)word):num))
        (word 96)` THEN
    IMP_REWRITE_TAC[RIP_REL_ADDR_FOLD] THEN
    ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
...
let EDWARDS25519_SCALARMULBASE_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!tables res scalar n pc stackpointer returnaddress.
    ...
    ALL (nonoverlapping (word_sub stackpointer (word 536),536))
        [(word pc,LENGTH (edwards25519_scalarmulbase_tmc pc tables));
    ...
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
                (edwards25519_scalarmulbase_tmc pc tables) /\
              read RIP s = word pc /\
              bytes_loaded s (word tables)
                edwards25519_scalarmulbase_constant_data /\
              read RSP s = stackpointer /\
...
let EDWARDS25519_SCALARMULBASE_MC_BRIDGE = prove
 (`!pc tables. edwards25519_scalarmulbase_mc pc tables =
     APPEND [word 0xf3:byte; word 0x0f; word 0x1e; word 0xfa]
            (edwards25519_scalarmulbase_tmc (pc + 4) tables)`,
  REPEAT GEN_TAC THEN
  REWRITE_TAC[edwards25519_scalarmulbase_mc;
              edwards25519_scalarmulbase_tmc; APPEND] THEN
  REWRITE_TAC[GSYM INT_OF_NUM_ADD] THEN
  AP_TERM_TAC THEN AP_TERM_TAC THEN AP_TERM_TAC THEN
  REWRITE_TAC[INT_ARITH `&pc + &178 = (&pc + &4) + &174:int`]);;

(* Length of the IBT mc, derived from the bridge and the trimmed mc length. *)
let LENGTH_EDWARDS25519_SCALARMULBASE_MC = prove
 (`!pc tables. LENGTH (edwards25519_scalarmulbase_mc pc tables) = 0x307a`,
  REWRITE_TAC[EDWARDS25519_SCALARMULBASE_MC_BRIDGE; LENGTH_APPEND; LENGTH;
              fst EDWARDS25519_SCALARMULBASE_EXEC] THEN ARITH_TAC);;

let EDWARDS25519_SCALARMULBASE_SUBROUTINE_CORRECT = time prove
 (`!tables res scalar n pc stackpointer returnaddress.
    riprel32_within_bounds tables (pc + 178) /\
    ALL (nonoverlapping (word_sub stackpointer (word 536),536))
        [(word pc,LENGTH (edwards25519_scalarmulbase_mc pc tables));
         (word tables,48576); (scalar,32)] /\
    ALL (nonoverlapping (res,64))
        [(word pc,LENGTH (edwards25519_scalarmulbase_mc pc tables));
         (word tables,48576);
    ...
    ==> ensures x86
         ...
         (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
          MAYCHANGE [memory :> bytes(res,64);
                     memory :> bytes(word_sub stackpointer (word 536),536)])`,
  REWRITE_TAC[LENGTH_EDWARDS25519_SCALARMULBASE_MC] THEN
  MATCH_ACCEPT_TAC(ADD_IBT_RULE
                     ~bridge:(Some EDWARDS25519_SCALARMULBASE_MC_BRIDGE)
                     (REWRITE_RULE[fst EDWARDS25519_SCALARMULBASE_EXEC]
                       EDWARDS25519_SCALARMULBASE_NOIBT_SUBROUTINE_CORRECT)));;
...
let edwards25519_scalarmulbase_windows_mc,_ =
...
let edwards25519_scalarmulbase_windows_tmc =
  define_trimmed "edwards25519_scalarmulbase_windows_tmc"
                 edwards25519_scalarmulbase_windows_mc;;

let WINDOWS_EDWARDS25519_SCALARMULBASE_EXEC =
  X86_MK_EXEC_RULE edwards25519_scalarmulbase_windows_tmc;;

(* Bridge equation analogous to EDWARDS25519_SCALARMULBASE_MC_BRIDGE for the
   Windows mc/tmc pair. Used to apply ADD_IBT_RULE on rodata-aware
   parametrized mcs. *)
let EDWARDS25519_SCALARMULBASE_WINDOWS_MC_BRIDGE = prove
 (`!pc tables. edwards25519_scalarmulbase_windows_mc pc tables =
     APPEND [word 0xf3:byte; word 0x0f; word 0x1e; word 0xfa]
            (edwards25519_scalarmulbase_windows_tmc (pc + 4) tables)`,
  REPEAT GEN_TAC THEN
  REWRITE_TAC[edwards25519_scalarmulbase_windows_mc;
              edwards25519_scalarmulbase_windows_tmc; APPEND] THEN
  REWRITE_TAC[GSYM INT_OF_NUM_ADD] THEN
  AP_TERM_TAC THEN AP_TERM_TAC THEN AP_TERM_TAC THEN
  REWRITE_TAC[INT_ARITH `&pc + &194 = (&pc + &4) + &190:int`]);;

(* Length of the Windows IBT mc, derived from the bridge and the Windows
   trimmed mc length. *)
let LENGTH_EDWARDS25519_SCALARMULBASE_WINDOWS_MC = prove
 (`!pc tables. LENGTH (edwards25519_scalarmulbase_windows_mc pc tables) = 0x308a`,
  REWRITE_TAC[EDWARDS25519_SCALARMULBASE_WINDOWS_MC_BRIDGE;
              LENGTH_APPEND; LENGTH;
              fst WINDOWS_EDWARDS25519_SCALARMULBASE_EXEC] THEN ARITH_TAC);;

let EDWARDS25519_SCALARMULBASE_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!tables res scalar n pc stackpointer returnaddress.
    riprel32_within_bounds tables (pc + 190) /\
    ALL (nonoverlapping (word_sub stackpointer (word 560),560))
        [(word pc,LENGTH (edwards25519_scalarmulbase_windows_tmc pc tables));
         (word tables,48576); (scalar,32)] /\
    ALL (nonoverlapping (res,64))
        [(word pc,LENGTH (edwards25519_scalarmulbase_windows_tmc pc tables));
         (word tables,48576);
    ...
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
                (edwards25519_scalarmulbase_windows_tmc pc tables) /\
              read RIP s = word pc /\
              bytes_loaded s (word tables)
                edwards25519_scalarmulbase_constant_data /\
              read RSP s = stackpointer /\
  ...
  REWRITE_TAC[BYTES_LOADED_DATA;
              fst WINDOWS_EDWARDS25519_SCALARMULBASE_EXEC] THEN
...
let EDWARDS25519_SCALARMULBASE_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!tables res scalar n pc stackpointer returnaddress.
    riprel32_within_bounds tables (pc + 194) /\
    ALL (nonoverlapping (word_sub stackpointer (word 560),560))
        [(word pc,LENGTH (edwards25519_scalarmulbase_windows_mc pc tables));
         (word tables,48576); (scalar,32)] /\
    ALL (nonoverlapping (res,64))
        [(word pc,LENGTH (edwards25519_scalarmulbase_windows_mc pc tables));
         (word tables,48576);
    ...
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
                (edwards25519_scalarmulbase_windows_mc pc tables) /\
              read RIP s = word pc /\
              bytes_loaded s (word tables)
                edwards25519_scalarmulbase_constant_data /\
              read RSP s = stackpointer /\
         ...
         (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
          MAYCHANGE [memory :> bytes(res,64);
                     memory :> bytes(word_sub stackpointer (word 560),560)])`,
  REWRITE_TAC[LENGTH_EDWARDS25519_SCALARMULBASE_WINDOWS_MC] THEN
  MATCH_ACCEPT_TAC(ADD_IBT_RULE
                     ~bridge:(Some EDWARDS25519_SCALARMULBASE_WINDOWS_MC_BRIDGE)
                     (REWRITE_RULE[fst WINDOWS_EDWARDS25519_SCALARMULBASE_EXEC]
                       EDWARDS25519_SCALARMULBASE_NOIBT_WINDOWS_SUBROUTINE_CORRECT)));;
...
let LOCAL_MODINV_SAFETY_TAC (assump_name:string) (n:int) =
  REMOVE_THEN assump_name (fun safety_th ->
    X86_SUBROUTINE_SIM_TAC ~is_safety_thm:true
        ...
        `pc + 0x189f`; `stackpointer:int64`] n
    THENL [
      EXISTS_E2_TAC(ref
        [`scalar:int64`;`res:int64`;`pc:num`;`stackpointer:int64`;`tables:num`;
         (* inside the loop... *)
         `i:num`;
         `f_ev_loop:int64->int64->num->num->int64->num->num->(uarch_event)list`]);
...
(* The hand-written precondition below uses the leaner
   `nonoverlapping (res,64) (word pc,0x3076)` form. This is intentionally
   weaker than what `mk_safety_spec`'s `~readonly_objects` would produce
   (which folds `word tables,48576` into the same `nonoverlapping (res,64)`
   ALL list). Strengthening to the full form to enable
   `ASSERT_CONCL_TAC full_spec` propagates an extra `nonoverlapping (res,64)
   (word tables,48576)` hypothesis through `STRIP_TAC`/`X86_STEPS_TAC` and
   trips later automated steps. So `ASSERT_CONCL_TAC full_spec` is omitted
   here. *)
let EDWARDS25519_SCALARMULBASE_SAFE = time prove
 (`exists f_events.
       forall e tables res scalar pc stackpointer.
           riprel32_within_bounds tables (pc + 174) /\
           ALL (nonoverlapping (stackpointer,488))
           [word pc,0x3076; word tables,48576; res,64; scalar,32] /\
           nonoverlapping (res,64) (word pc,0x3076)
           ==> ensures x86
               (\s.
  ...
  ENSURES_EVENTS_WHILE_UP2_TAC `63` `pc + 0x1f1` `pc + 0x180d`(*+2 insn *)
   `\i s.
      read (memory :> bytes(word tables,48576)) s =
      num_of_bytelist edwards25519_scalarmulbase_constant_data /\
      read RSP s = stackpointer /\
  ...
  REPEAT CONJ_TAC THENL [
    ...
    X86_STEPS_TAC EDWARDS25519_SCALARMULBASE_EXEC (1--39) THEN
    ...
    X86_STEPS_TAC EDWARDS25519_SCALARMULBASE_EXEC (1--281) THEN
    X86_STEPS_TAC EDWARDS25519_SCALARMULBASE_EXEC (282--
    ...
    X86_STEPS_TAC EDWARDS25519_SCALARMULBASE_EXEC (296--298) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    CONJ_TAC THENL [
  ...
  ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN
  X86_STEPS_TAC EDWARDS25519_SCALARMULBASE_EXEC (3--26) THEN
  LOCAL_MODINV_SAFETY_TAC "H_INV" 27 THEN
  X86_STEPS_TAC EDWARDS25519_SCALARMULBASE_EXEC (28--(27+1+91*2)) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN

  DISCHARGE_SAFETY_PROPERTY_TAC);;

let EDWARDS25519_SCALARMULBASE_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
  ...
  X86_ADD_RETURN_STACK_TAC EDWARDS25519_SCALARMULBASE_EXEC
   (REWRITE_RULE[BYTES_LOADED_DATA; fst EDWARDS25519_SCALARMULBASE_EXEC]
    EDWARDS25519_SCALARMULBASE_SAFE)
    `[RBX; RBP; R12; R13; R14; R15]` 536
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let EDWARDS25519_SCALARMULBASE_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e tables res scalar pc stackpointer returnaddress.
        riprel32_within_bounds tables (pc + 178) /\
        ALL (nonoverlapping (word_sub stackpointer (word 536),536))
  ...
  MATCH_ACCEPT_TAC(ADD_IBT_RULE
                     ~bridge:(Some EDWARDS25519_SCALARMULBASE_MC_BRIDGE)
                     (REWRITE_RULE[fst EDWARDS25519_SCALARMULBASE_EXEC]
                       EDWARDS25519_SCALARMULBASE_NOIBT_SUBROUTINE_SAFE)));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let EDWARDS25519_SCALARMULBASE_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e tables res scalar pc stackpointer returnaddress.
        riprel32_within_bounds tables (pc + 190) /\
        ALL (nonoverlapping (word_sub stackpointer (word 560),560))
  ...
  let subth =
    REWRITE_RULE[BYTES_LOADED_DATA; fst EDWARDS25519_SCALARMULBASE_EXEC]
    (X86_SIMD_SHARPEN_RULE EDWARDS25519_SCALARMULBASE_NOIBT_SUBROUTINE_SAFE
     (REWRITE_TAC[BYTES_LOADED_DATA; fst EDWARDS25519_SCALARMULBASE_EXEC] THEN
      X86_ADD_RETURN_STACK_TAC EDWARDS25519_SCALARMULBASE_EXEC
       (REWRITE_RULE[BYTES_LOADED_DATA; fst EDWARDS25519_SCALARMULBASE_EXEC]
        EDWARDS25519_SCALARMULBASE_SAFE)
...
let EDWARDS25519_SCALARMULBASE_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e tables res scalar pc stackpointer returnaddress.
        riprel32_within_bounds tables (pc + 194) /\
        ALL (nonoverlapping (word_sub stackpointer (word 560),560))
  ...
  MATCH_ACCEPT_TAC(ADD_IBT_RULE
                     ~bridge:(Some EDWARDS25519_SCALARMULBASE_WINDOWS_MC_BRIDGE)
                     (REWRITE_RULE[fst WINDOWS_EDWARDS25519_SCALARMULBASE_EXEC]
                       EDWARDS25519_SCALARMULBASE_NOIBT_WINDOWS_SUBROUTINE_SAFE)));;
