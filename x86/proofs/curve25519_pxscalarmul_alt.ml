...
let curve25519_pxscalarmul_alt_cmc = define_assert_from_elf
  "curve25519_pxscalarmul_alt_cmc" "x86/curve25519/curve25519_pxscalarmul_alt.o"
[
...
let curve25519_pxscalarmul_alt_mc = define_trimmed "curve25519_pxscalarmul_alt_mc" curve25519_pxscalarmul_alt_cmc;;
...
let CURVE25519_PXSCALARMUL_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!res scalar n point X pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 408),408))
        [(word pc,LENGTH curve25519_pxscalarmul_alt_cmc); (scalar,32); (point,32)] /\
    nonoverlapping (res,64) (word pc,LENGTH curve25519_pxscalarmul_alt_cmc) /\
    nonoverlapping (res,64) (word_sub stackpointer (word 408),416)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) curve25519_pxscalarmul_alt_cmc /\
              read RIP s = word pc /\
              read RSP s = stackpointer /\
              read (memory :> bytes64 stackpointer) s = returnaddress /\
              C_ARGUMENTS [res; scalar; point] s /\
         ...
         (\s. read RIP s = returnaddress /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(res,64);
                      memory :> bytes(word_sub stackpointer (word 408),408)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE CURVE25519_PXSCALARMUL_ALT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_curve25519_pxscalarmul_alt_cmc = define_from_elf
  "windows_curve25519_pxscalarmul_alt_cmc"
  "x86/curve25519/curve25519_pxscalarmul_alt.obj";;

let windows_curve25519_pxscalarmul_alt_mc = define_trimmed "windows_curve25519_pxscalarmul_alt_mc" windows_curve25519_pxscalarmul_alt_cmc;;
...
let WINDOWS_CURVE25519_PXSCALARMUL_ALT_SUBROUTINE_CORRECT = time prove
 (`!res scalar n point X pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 424),424))
        [(word pc,LENGTH windows_curve25519_pxscalarmul_alt_mc); (scalar,32); (point,32)] /\
    nonoverlapping (res,64) (word pc,LENGTH windows_curve25519_pxscalarmul_alt_mc) /\
...
let WINDOWS_CURVE25519_PXSCALARMUL_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!res scalar n point X pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 424),424))
        [(word pc,LENGTH windows_curve25519_pxscalarmul_alt_cmc); (scalar,32); (point,32)] /\
    nonoverlapping (res,64) (word pc,LENGTH windows_curve25519_pxscalarmul_alt_cmc) /\
    nonoverlapping (res,64) (word_sub stackpointer (word 424),432)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) windows_curve25519_pxscalarmul_alt_cmc /\
              read RIP s = word pc /\
              read RSP s = stackpointer /\
              read (memory :> bytes64 stackpointer) s = returnaddress /\
              WINDOWS_C_ARGUMENTS [res; scalar; point] s /\
         ...
         (\s. read RIP s = returnaddress /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(res,64);
                      memory :> bytes(word_sub stackpointer (word 424),424)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_CURVE25519_PXSCALARMUL_ALT_SUBROUTINE_CORRECT));;
