...
let p256_montjmixadd_alt_cmc = define_assert_from_elf
  "p256_montjmixadd_alt_cmc" "x86/p256/p256_montjmixadd_alt.o"
[
...
let p256_montjmixadd_alt_mc = define_trimmed "p256_montjmixadd_alt_mc" p256_montjmixadd_alt_cmc;;
...
let P256_MONTJMIXADD_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 240),240))
            [(word pc,LENGTH p256_montjmixadd_alt_cmc); (p1,96); (p2,64)] /\
        ALL (nonoverlapping (p3,96))
            [(word pc,LENGTH p256_montjmixadd_alt_cmc); (word_sub stackpointer (word 240),248)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) p256_montjmixadd_alt_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [p3; p1; p2] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,96);
                      memory :> bytes(word_sub stackpointer (word 240),240)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P256_MONTJMIXADD_ALT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_p256_montjmixadd_alt_cmc = define_from_elf "windows_p256_montjmixadd_alt_cmc"
      "x86/p256/p256_montjmixadd_alt.obj";;

let windows_p256_montjmixadd_alt_mc = define_trimmed "windows_p256_montjmixadd_alt_mc" windows_p256_montjmixadd_alt_cmc;;

let WINDOWS_P256_MONTJMIXADD_ALT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 256),256))
            [(word pc,LENGTH windows_p256_montjmixadd_alt_mc); (p1,96); (p2,64)] /\
        ALL (nonoverlapping (p3,96))
            [(word pc,LENGTH windows_p256_montjmixadd_alt_mc); (word_sub stackpointer (word 256),264)]
        ==> ensures x86
...
let WINDOWS_P256_MONTJMIXADD_ALT_IBT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 256),256))
            [(word pc,LENGTH windows_p256_montjmixadd_alt_cmc); (p1,96); (p2,64)] /\
        ALL (nonoverlapping (p3,96))
            [(word pc,LENGTH windows_p256_montjmixadd_alt_cmc); (word_sub stackpointer (word 256),264)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_p256_montjmixadd_alt_cmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,96);
                      memory :> bytes(word_sub stackpointer (word 256),256)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WINDOWS_P256_MONTJMIXADD_ALT_SUBROUTINE_CORRECT));;
