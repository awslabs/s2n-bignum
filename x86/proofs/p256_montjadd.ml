...
let P256_MONTJADD_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 p2 pc stackpointer.
           ALL (nonoverlapping (stackpointer,224))
           [word pc,10510; p1,96; p2,96; p3,96] /\
           nonoverlapping (p3,96) (word pc,10510)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST p256_montjadd_tmc) /\
                    read RIP s = word (pc + 17) /\
                    read RSP s = stackpointer /\
                    C_ARGUMENTS [p3; p1; p2] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 10492) /\
                    (exists e2.
...
let P256_MONTJADD_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 272),272))
        [word pc,LENGTH p256_montjadd_tmc; p1,96; p2,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH p256_montjadd_tmc;
         word_sub stackpointer (word 272),280]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjadd_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
...
let P256_MONTJADD_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ...
        [word pc,LENGTH p256_montjadd_mc; p1,96; p2,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH p256_montjadd_mc;
         word_sub stackpointer (word 272),280]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjadd_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 272),272)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P256_MONTJADD_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let P256_MONTJADD_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 288),288))
        [word pc,LENGTH p256_montjadd_windows_tmc; p1,96; p2,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH p256_montjadd_windows_tmc;
         word_sub stackpointer (word 288),296]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjadd_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
...
let P256_MONTJADD_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 288),288))
        [word pc,LENGTH p256_montjadd_windows_mc; p1,96; p2,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH p256_montjadd_windows_mc;
         word_sub stackpointer (word 288),296]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjadd_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      read events s = APPEND e2 e /\
                      e2 = f_events p1 p2 p3 pc (word_sub stackpointer (word 288))
                          returnaddress /\
                      memaccess_inbounds e2
                      [p1,96; p2,96; p3,96;
                       word_sub stackpointer (word 288),296]
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 288),288)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P256_MONTJADD_NOIBT_WINDOWS_SUBROUTINE_SAFE));;