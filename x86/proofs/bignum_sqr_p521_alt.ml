...
let full_spec,public_vars = mk_safety_spec
    ...
    BIGNUM_SQR_P521_ALT_CORRECT
    BIGNUM_SQR_P521_ALT_EXEC;;

let BIGNUM_SQR_P521_ALT_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer.
           ALL (nonoverlapping (stackpointer,72))
           [word pc,1280; z,8 * 9; x,8 * 9] /\
           nonoverlapping (z,8 * 9) (word pc,1280)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc)
                    (BUTLAST bignum_sqr_p521_alt_tmc) /\
                    read RIP s = word (pc + 13) /\
                    read RSP s = stackpointer /\
                    C_ARGUMENTS [z; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 1266) /\
                    (exists e2.
...
let BIGNUM_SQR_P521_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
           ALL (nonoverlapping (z,8 * 9))
           [word pc,LENGTH bignum_sqr_p521_alt_tmc;
            word_sub stackpointer (word 112),120] /\
           ALL (nonoverlapping (word_sub stackpointer (word 112),112))
           [word pc,LENGTH bignum_sqr_p521_alt_tmc; x,8 * 9]
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_sqr_p521_alt_tmc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [z; x] s /\
               ...
               (\s.
                    ...
                    (exists e2.
                         ...
                         [x,72; z,72; word_sub stackpointer (word 112),72;
                          word_sub stackpointer (word 40),48]
                         [z,72; word_sub stackpointer (word 112),72;
               ...
               (MAYCHANGE [RSP] ,,
                ...
                [memory :> bytes (z,8 * 9);
                 memory :> bytes (word_sub stackpointer (word 112),112)])`,
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_sqr_p521_alt_tmc BIGNUM_SQR_P521_ALT_SAFE
   `[RBX; R12; R13; R14; R15]` 112 THEN
...
let BIGNUM_SQR_P521_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
           ALL (nonoverlapping (z,8 * 9))
           [word pc,LENGTH bignum_sqr_p521_alt_mc;
            word_sub stackpointer (word 112),120] /\
           ALL (nonoverlapping (word_sub stackpointer (word 112),112))
           [word pc,LENGTH bignum_sqr_p521_alt_mc; x,8 * 9]
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_sqr_p521_alt_mc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [z; x] s /\
               ...
               (MAYCHANGE [RSP] ,,
                ...
                [memory :> bytes (z,8 * 9);
                 memory :> bytes (word_sub stackpointer (word 112),112)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_P521_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_SQR_P521_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
           ALL (nonoverlapping (z,8 * 9))
           [word pc,LENGTH bignum_sqr_p521_alt_windows_tmc;
            word_sub stackpointer (word 128),136] /\
           ALL (nonoverlapping (word_sub stackpointer (word 128),128))
           [word pc,LENGTH bignum_sqr_p521_alt_windows_tmc; x,8 * 9]
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_sqr_p521_alt_windows_tmc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    WINDOWS_C_ARGUMENTS [z; x] s /\
               ...
               (\s.
                    ...
                    (exists e2.
                         ...
                         returnaddress /\
                         memaccess_inbounds e2
                         [x,72; z,72; word_sub stackpointer (word 128),72;
                          word_sub stackpointer (word 56),64]
                         [z,72; word_sub stackpointer (word 128),72;
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   bignum_sqr_p521_alt_windows_tmc bignum_sqr_p521_alt_tmc
   BIGNUM_SQR_P521_ALT_SAFE `[RBX; R12; R13; R14; R15]` 112 THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_SQR_P521_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
           ALL (nonoverlapping (z,8 * 9))
           [word pc,LENGTH bignum_sqr_p521_alt_windows_mc;
            word_sub stackpointer (word 128),136] /\
           ALL (nonoverlapping (word_sub stackpointer (word 128),128))
           [word pc,LENGTH bignum_sqr_p521_alt_windows_mc; x,8 * 9]
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) bignum_sqr_p521_alt_windows_mc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    WINDOWS_C_ARGUMENTS [z; x] s /\
               ...
               (MAYCHANGE [RSP] ,,
                ...
                [memory :> bytes (z,8 * 9);
                 memory :> bytes (word_sub stackpointer (word 128),128)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_P521_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
