...
extern uint64_t mldsa_rej_uniform_eta2(int32_t r[S2N_BIGNUM_STATIC 256], const uint8_t *buf, unsigned buflen, const uint8_t table[S2N_BIGNUM_STATIC 4096]);

// Rejection sampling for ML-DSA secret key (eta = 4; parameter set 65)
// Inputs buf[buflen], buflen, table[4096] (uint8_t); output r[256] (signed 32-bit words)
extern uint64_t mldsa_rej_uniform_eta4(int32_t r[S2N_BIGNUM_STATIC 256], const uint8_t *buf, unsigned buflen, const uint8_t table[S2N_BIGNUM_STATIC 4096]);
