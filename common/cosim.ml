...
(*
 * Instruction cosimulation checks the HOL Light architecture model against
 * an external executor. The HOL client generates an encoded instruction and
 * an input machine state, sends both to the executor, and compares the
 * returned state with the model's result. This tests the formal semantics
 * against native hardware, an emulator, or another protocol-speaking
 * backend.
 *
 * This file implements persistent external instruction-executor sessions
 * for those cosimulation tests.
 *
 * Runtime configuration is controlled by the following environment
 * variables:
 *
...
let cosim_process_exit status = Unix.\_exit status;;
