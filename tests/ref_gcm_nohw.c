...
// NOTE (decrypt PR): aws-lc's `hw_gcm_encrypt` Path-B dispatch is omitted here.
// It is unused by this reference (the reference path drives CRYPTO_gcm128_encrypt
// with the pure-C block function) and it call-references the ENCRYPT kernels,
// which this change set does not add. The decrypt-side dispatch under test lives
// in test.c as `hw_gcm_decrypt_wb`, mirroring aws-lc's `hw_gcm_decrypt`.
