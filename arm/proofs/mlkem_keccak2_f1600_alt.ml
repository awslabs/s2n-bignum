...
(**** print_literal_from_elf "arm/mlkem/mlkem_keccak2_f1600_alt.o";;
 ****)

let mlkem_keccak2_f1600_alt_mc = define_assert_from_elf
  "mlkem_keccak2_f1600_alt_mc" "arm/mlkem/mlkem_keccak2_f1600_alt.o"
[
...
let MLKEM_KECCAK2_F1600_ALT_CORRECT = prove
 (`!a rc A A' pc.
      ALL (nonoverlapping (a,400)) [(word pc,0x340); (rc,192)]
      ==> ensures arm
           (\s. aligned_bytes_loaded s (word pc) mlkem_keccak2_f1600_alt_mc /\
  ...
  REWRITE_TAC[fst MLKEM_KECCAK2_F1600_ALT_EXEC] THEN
  ...
  REWRITE_TAC[condition_semantics] THEN REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    ARM_STEPS_TAC MLKEM_KECCAK2_F1600_ALT_EXEC (1--41) THEN
    ...
    ASSUME_TAC THENL
     [UNDISCH_TAC `i < 24` THEN SPEC_TAC(`i:num`,`i:num`) THEN
      ...
      ALL_TAC] THEN
    ARM_STEPS_TAC MLKEM_KECCAK2_F1600_ALT_EXEC (1--115) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    ARM_SIM_TAC MLKEM_KECCAK2_F1600_ALT_EXEC (1--41) THEN
    CONV_TAC(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV) THEN
...
let MLKEM_KECCAK2_F1600_ALT_SUBROUTINE_CORRECT = prove
