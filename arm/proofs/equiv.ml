...
let FIND_HOLE_TAC: tactic =
  fun (asl,g) ->
    ...
    (* MAP_EVERY (fun (v:term) ->
        ASM_CASES_TAC (mk_binary "<" (v,mk_numeral (Num.Int segsize))) THEN
        ASM_CASES_TAC (mk_binary "<" (v,mk_numeral (Num.Int (2*segsize)))))
        [`val (addr_in:int64)`;`val (addr_out:int64)`] *)
    let cases_tac = MAP_EVERY (fun (v:term) ->
        List.fold_left then_ ALL_TAC
          (List.map
            (fun i -> ASM_CASES_TAC (mk_binary "<" (v,mk_numeral (Num.Int (i * segsize)))))
    ...
    let try_holes = List.fold_left then_ ALL_TAC
        (List.map
          (fun i -> TRY_CONST_PC_TAC (mk_numeral (Num.Int (i * segsize + maxlen))))
