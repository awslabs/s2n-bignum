...
let LOCAL_MODINV_SAFETY_TAC (assump_name:string) (n:int) =
  REMOVE_THEN assump_name (fun safety_th ->
    ARM_SUBROUTINE_SIM_TAC ~is_safety_thm:true
        (edwards25519_scalarmulbase_mc',
          EDWARDS25519_SCALARMULBASE_EXEC,0x196c,
          (GEN_REWRITE_CONV RAND_CONV [bignum_inv_p25519_mc] THENC TRIM_LIST_CONV)
          `TRIM_LIST (12,16) bignum_inv_p25519_mc`,
          safety_th)
        [`e:(uarch_event)list`; `read X0 s`; `read X1 s`;
          `pc + 0x196c`; `stackpointer:int64`] n
    THENL [
      EXISTS_E2_TAC(ref
...
let EDWARDS25519_SCALARMULBASE_SAFE = time prove
 (`exists f_events.
       forall e tables res scalar pc stackpointer.
           ...
           ALL (nonoverlapping (stackpointer,448))
           [word pc,12072; word tables,48576; res,64; scalar,32] /\
           nonoverlapping (res,64) (word pc,12072)
           ==> ensures arm
               (\s.
  ...
  ENSURES_EVENTS_WHILE_UP2_TAC `63` `pc + 0x144` `pc + 0x1924`(*+2 insn *)
   `\i s.
      read (memory :> bytes (word tables,48576)) s =
        num_of_bytelist edwards25519_scalarmulbase_constant_data /\
      read SP s = stackpointer /\
  ...
  REPEAT CONJ_TAC THENL [
    ...
    ARM_STEPS_TAC EDWARDS25519_SCALARMULBASE_EXEC (1--41) THEN
    FIRST_ASSUM(SUBST_ALL_TAC o MATCH_MP ADRP_ADD_FOLD) THEN
    ARM_STEPS_TAC EDWARDS25519_SCALARMULBASE_EXEC (42--77) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    ARM_STEPS_TAC EDWARDS25519_SCALARMULBASE_EXEC (1--183) THEN
    ARM_STEPS_TAC EDWARDS25519_SCALARMULBASE_EXEC (184--211) THEN
    ARM_STEPS_TAC EDWARDS25519_SCALARMULBASE_EXEC (212--
    ...
    ARM_STEPS_TAC EDWARDS25519_SCALARMULBASE_EXEC (226--228) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    CONJ_TAC THENL [
  ...
  ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN
  ARM_STEPS_TAC EDWARDS25519_SCALARMULBASE_EXEC (3--20) THEN
  LOCAL_MODINV_SAFETY_TAC "H_INV" 21 THEN
  ARM_STEPS_TAC EDWARDS25519_SCALARMULBASE_EXEC (22--(21+180*2)) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
