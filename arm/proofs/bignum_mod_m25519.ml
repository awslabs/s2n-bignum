...
(**** print_literal_from_elf "arm/curve25519/bignum_mod_m25519.o";;
 ****)

let bignum_mod_m25519_mc =
  define_assert_from_elf "bignum_mod_m25519_mc" "arm/curve25519/bignum_mod_m25519.o"
[
  0xf100103f;       (* arm_CMP X1 (rvalue (word 4)) *)
  0x54000683;       (* arm_BCC (word 208) *)
  ...
  0xd293ed0b;       (* arm_MOV X11 (rvalue (word 40808)) *)
  0xf2bcf5cb;       (* arm_MOVK X11 (word 59310) 16 *)
  0xf2c31a4b;       (* arm_MOVK X11 (word 6354) 32 *)
  0xf2f8126b;       (* arm_MOVK X11 (word 49299) 48 *)
  0xd29cd64c;       (* arm_MOV X12 (rvalue (word 59058)) *)
  0xf2a2f78c;       (* arm_MOVK X12 (word 6076) 16 *)
  0xf2d9deac;       (* arm_MOVK X12 (word 52981) 32 *)
  0xf2f4deec;       (* arm_MOVK X12 (word 42743) 48 *)
  0xd2f0000a;       (* arm_MOVZ X10 (word 32768) 48 *)
  0xeb0b0067;       (* arm_SUBS X7 X3 X11 *)
  0xfa0c0088;       (* arm_SBCS X8 X4 X12 *)
  0xfa1f00a9;       (* arm_SBCS X9 X5 XZR *)
  0xfa0a00ca;       (* arm_SBCS X10 X6 X10 *)
  ...
  0x93c5fccd;       (* arm_EXTR X13 X6 X5 63 *)
  0x9240f8a5;       (* arm_AND X5 X5 (rvalue (word 9223372036854775807)) *)
  0xcb46fdad;       (* arm_SUB X13 X13 (Shiftedreg X6 LSR 63) *)
  0x924100c6;       (* arm_AND X6 X6 (rvalue (word 9223372036854775808)) *)
  ...
  0x92410086;       (* arm_AND X6 X4 (rvalue (word 9223372036854775808)) *)
...
let BIGNUM_MOD_M25519_EXEC = ARM_MK_EXEC_RULE bignum_mod_m25519_mc;;
...
let BIGNUM_MOD_M25519_CORRECT = time prove
 (`!z k x n pc.
      ...
      ==> ensures arm
           (\s. aligned_bytes_loaded s (word pc) bignum_mod_m25519_mc /\
                read PC s = word pc /\
                C_ARGUMENTS [z; k; x] s /\
  ...
  ASM_CASES_TAC `k < 4` THENL
   ...
   REWRITE_TAC[bignum_of_wordlist] THEN ASM_REWRITE_TAC[] THENL
    [ARM_STEPS_TAC BIGNUM_MOD_M25519_EXEC (1--9);
     ARM_STEPS_TAC BIGNUM_MOD_M25519_EXEC (1--12);
     ARM_STEPS_TAC BIGNUM_MOD_M25519_EXEC (1--15);
     ARM_STEPS_TAC BIGNUM_MOD_M25519_EXEC (1--17)] THEN
  ...
  ENSURES_SEQUENCE_TAC `pc + 0x60`
   `\s. bignum_from_memory(x,k) s = n /\
        ...
        read X2 s = x /\
        read X1 s = word(k - 4) /\
        read X11 s = word 0xc09318d2e7ae9f68 /\
        read X12 s = word 0xa6f7cef517bce6b2 /\
  ...
  CONJ_TAC THENL
   [ABBREV_TAC `j = k - 4` THEN VAL_INT64_TAC `j:num` THEN
    ...
    ARM_STEPS_TAC BIGNUM_MOD_M25519_EXEC (1--5) THEN
    RULE_ASSUM_TAC(REWRITE_RULE[WORD_RULE
     `word_add (word_shl (word j) 3) x = word_add x (word(8 * j))`]) THEN
    ARM_ACCSTEPS_TAC BIGNUM_MOD_M25519_EXEC (17--20) (6--24) THEN
    RULE_ASSUM_TAC(REWRITE_RULE
     [REAL_BITVAL_NOT; ADD_CLAUSES; VAL_WORD_BITVAL]) THEN
    SUBGOAL_THEN `carry_s20 <=> m < m_25519` SUBST_ALL_TAC THENL
    ...
    EXPAND_TAC "m" THEN
    REWRITE_TAC[REAL_BITVAL_NOT; m_25519; bignum_of_wordlist] THEN
  ...
  THENL
   [GHOST_INTRO_TAC `d0:int64` `read X3` THEN
    ...
    ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_MOD_M25519_EXEC (1--3) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  ENSURES_WHILE_DOWN_TAC `k - 4` `pc + 0x64` `pc + 0xc4`
   `\i s. bignum_from_memory(x,k) s = n /\
          ...
          read X12 s = word 0xa6f7cef517bce6b2 /\
          bignum_of_wordlist[read X3 s; read X4 s; read X5 s; read X6 s] =
          (highdigits n i) MOD m_25519` THEN
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [VAL_INT64_TAC `k - 4` THEN REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
    ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_MOD_M25519_EXEC [1] THEN
  ...
  ARM_ACCSTEPS_TAC BIGNUM_MOD_M25519_EXEC [6;7;9;11] (1--12) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[WORD_RULE
  ...
  ARM_ACCSTEPS_TAC BIGNUM_MOD_M25519_EXEC
  ...
  SUBGOAL_THEN
   `&(val(word_add (word_and m3 (word 9223372036854775807))
                   (word_and m4 (word 9223372036854775808)):int64)):real =
  ...
  CONJ_TAC THENL
   [SUBGOAL_THEN
     `m < val(q:int64) * m_25519 <=> carry_s17`
    SUBST1_TAC THENL
 ...
 (`!z k x n pc returnaddress.
      ...
      ==> ensures arm
          ...
          (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,4)])`,
  ARM_ADD_RETURN_NOSTACK_TAC BIGNUM_MOD_M25519_EXEC BIGNUM_MOD_M25519_CORRECT);;
...
let BIGNUM_MOD_M25519_SAFE = time prove
 (`exists f_events.
        forall e z k x pc.
            nonoverlapping (word pc,264) (z,32)
            ==> ensures arm
                (\s.
                     aligned_bytes_loaded s (word pc) bignum_mod_m25519_mc /\
                     read PC s = word pc /\
                     C_ARGUMENTS [z; k; x] s /\
                     read events s = e)
                (\s.
                     read PC s = word (pc + 208) /\
                     (exists e2.
  ...
  ASM_CASES_TAC `k' < 4` THENL [
    ...
    STRIP_TAC THENL [
      ...
      ARM_SIM_TAC BIGNUM_MOD_M25519_EXEC (1--9) THEN
      DISCHARGE_SAFETY_PROPERTY_TAC;

      (* k = 1 *)
      ASM_REWRITE_TAC[ARITH] THEN
      SUBST1_TAC (ISPEC `k:int64` (GSYM WORD_VAL)) THEN
      ARM_SIM_TAC BIGNUM_MOD_M25519_EXEC (1--12) THEN
      DISCHARGE_SAFETY_PROPERTY_TAC;

      (* k = 2 *)
      ASM_REWRITE_TAC[ARITH] THEN
      SUBST1_TAC (ISPEC `k:int64` (GSYM WORD_VAL)) THEN
      ARM_SIM_TAC BIGNUM_MOD_M25519_EXEC (1--15) THEN
      DISCHARGE_SAFETY_PROPERTY_TAC;

      (* k = 3 *)
      ASM_REWRITE_TAC[ARITH] THEN
      SUBST1_TAC (ISPEC `k:int64` (GSYM WORD_VAL)) THEN
      ARM_SIM_TAC BIGNUM_MOD_M25519_EXEC (1--17) THEN
      DISCHARGE_SAFETY_PROPERTY_TAC;
    ];
  ...
  ENSURES_EVENTS_SEQUENCE_TAC `pc + 0x60`
   `\s. read X0 s = z /\
    ...
    ARM_SIM_TAC ~canonicalize_pc_diff:false
      BIGNUM_MOD_M25519_EXEC (1--24) THEN
    CONJ_TAC THENL [
  ...
  THENL
   [ASM_REWRITE_TAC[SUB_REFL;MULT_0] THEN
    ...
    ARM_STEPS_TAC BIGNUM_MOD_M25519_EXEC (1--3) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    DISCHARGE_SAFETY_PROPERTY_TAC;
  ...
  ASM_REWRITE_TAC[] THEN
  ENSURES_EVENTS_WHILE_UP2_TAC `k' - 4` `pc + 0x64` `pc + 0xc8`
   `\i s. read X0 s = z /\
          read X2 s = x /\
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL [
    ...
    ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN
    ARM_STEPS_TAC BIGNUM_MOD_M25519_EXEC [1] THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    CONJ_TAC THENL [
    ...
    ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN
    ARM_STEPS_TAC BIGNUM_MOD_M25519_EXEC (1--2) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    DISCHARGE_SAFETY_PROPERTY_TAC
  ] THEN
  ...
  ASM_REWRITE_TAC[] THEN
  ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN
  ARM_STEPS_TAC BIGNUM_MOD_M25519_EXEC (1--25) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONJ_TAC THENL [
 ...
 (`exists f_events.
        forall e z k x pc returnaddress.
          ...
          ==> ensures arm
                ...
                (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
                 MAYCHANGE [memory :> bignum(z,4)])`,
  ARM_ADD_RETURN_NOSTACK_TAC BIGNUM_MOD_M25519_EXEC BIGNUM_MOD_M25519_SAFE
