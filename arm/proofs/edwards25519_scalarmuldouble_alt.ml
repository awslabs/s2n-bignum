 ...
 (`read (memory :> bytes(wpc,768)) s =
  ...
  let GE25519_POWERS =
    ...
    (funpow 7 (fun l -> GE25519_GROUPER GE25519_POW_1 (hd l)::l)
                          [GE25519_POW_1]) in
  REWRITE_TAC[GSYM BYTES_LOADED_DATA; edwards25519_scalarmuldouble_alt_constant_data] THEN
...
let edwards25519_scalarmuldouble_alt_mc' =
  SPECL [`pc:num`; `tables:num`] edwards25519_scalarmuldouble_alt_mc;;
...
let LOCAL_MUL_P25519_TAC =
  ARM_MACRO_SIM_ABBREV_TAC edwards25519_scalarmuldouble_alt_mc' 100 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      nonoverlapping (word pc,0x56a8) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures arm
           (\s. aligned_bytes_loaded s (word pc) (edwards25519_scalarmuldouble_alt_mc pc tables) /\
                read PC s = pcin /\
                read SP s = read SP t /\
...
let LOCAL_PDOUBLE_CORRECT = time prove
 (`!p3 p1 T1 pc stackpointer.
    aligned 16 stackpointer /\
    ALL (nonoverlapping (stackpointer,160))
        [(word pc,0x56a8); (p3,96); (p1,96)] /\
    nonoverlapping (p3,96) (word pc,0x56a8)
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc)
                 (edwards25519_scalarmuldouble_alt_mc pc tables) /\
...
let LOCAL_EPADD_CORRECT = time prove
 (`!p3 p1 Q1 p2 Q2 pc stackpointer.
    ...
    ALL (nonoverlapping (stackpointer,192))
        [(word pc,0x56a8); (p3,128); (p1,128); (p2,128)] /\
    nonoverlapping (p3,128) (word pc,0x56a8)
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc)
                 (edwards25519_scalarmuldouble_alt_mc pc tables) /\
...
let LOCAL_PEPADD_CORRECT = time prove
 (`!p3 p1 Q1 p2 T2 pc stackpointer.
    aligned 16 stackpointer /\
    ALL (nonoverlapping (stackpointer,192))
        [(word pc,0x56a8); (p3,128); (p1,128); (p2,96)] /\
    nonoverlapping (p3,128) (word pc,0x56a8)
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc)
                 (edwards25519_scalarmuldouble_alt_mc pc tables) /\
  ...
  REMOVE_THEN assump_name (fun safety_th ->
    ARM_SUBROUTINE_SIM_TAC ~is_safety_thm:true
        ...
        [`e:(uarch_event)list`; `read X0 s`; `read X1 s`;
          `pc + 0x1130`; `word_add stackpointer (word 192):int64`] n
    THENL [
      EXISTS_E2_TAC(ref
...
let LOCAL_EPDOUBLE_SAFETY_TAC (assump_name:string) (n:int) =
  REMOVE_THEN assump_name (fun safety_th ->
    ARM_SUBROUTINE_SIM_TAC ~is_safety_thm:true
        (edwards25519_scalarmuldouble_alt_mc',EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC,
          0x0,edwards25519_scalarmuldouble_alt_mc',safety_th)
...
let EDWARDS25519_SCALARMULDOUBLE_ALT_SAFE = time prove
 (`exists f_events.
  ...
  ENSURES_EVENTS_WHILE_UP2_TAC `63` `pc + 0x994` `pc + 0x1128`(*+1 insn *)
   `\i s.
      read (memory :> bytes (word tables,768)) s =
        num_of_bytelist edwards25519_scalarmuldouble_alt_constant_data /\
      read SP s = word_add stackpointer (word 192) /\
  ...
  REPEAT CONJ_TAC THENL [
    ...
    ARM_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (1--64) THEN
    ARM_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (65--(64+6+94(*MUL_4*))) THEN
    ...
    ARM_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (122--128) THEN
    REPEAT(FIRST_X_ASSUM(SUBST_ALL_TAC o MATCH_MP ADRP_ADD_FOLD)) THEN
    ARM_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (129--539) THEN
    ARM_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (540--544) THEN
    ...
    ARM_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (1--2) THEN
    LOCAL_MODINV_SAFETY_TAC "H_INV" 3 THEN
    ARM_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (4--(3+3+100(*MUL_P25519*))) THEN
    ARM_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (5--(4+3+100(*MUL_P25519*))) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  REPEAT(FIRST_X_ASSUM(SUBST_ALL_TAC o MATCH_MP ADRP_ADD_FOLD)) THEN
  ARM_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (19--189) THEN
  ARM_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (190--214) THEN
  ARM_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (215--222) THEN
  ARM_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (223--445) THEN
  ARM_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (446--471) THEN
  ARM_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (472--475) THEN
  ...
  ARM_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (507--508) THEN
  ARM_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_ALT_EXEC (509--509) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONJ_TAC THENL [
