...
let BIGNUM_COPY_ROW_FROM_TABLE_16_SUBROUTINE_SAFE = prove(
  `exists f_events.
       forall e z table height idx pc returnaddress.
           nonoverlapping (word pc,188) (z,8 * 16) /\
           nonoverlapping (word pc,188) (table,8 * val height * 16) /\
           nonoverlapping (z,8 * 16) (table,8 * val height * 16) /\
           val idx < val height
           ==> ensures arm
               (\s.
                    aligned_bytes_loaded s (word pc)
                    bignum_copy_row_from_table_16_mc /\
                    read PC s = word pc /\
                    read X30 s = returnaddress /\
                    C_ARGUMENTS [z; table; height; idx] s /\
                    read events s = e)
  ...
  REWRITE_TAC[C_ARGUMENTS; NONOVERLAPPING_CLAUSES;
              fst BIGNUM_COPY_ROW_FROM_TABLE_16_EXEC] THEN
  REPEAT STRIP_TAC THEN

  ENSURES_EVENTS_WHILE_UP2_TAC `h:num` `pc + 0x24` `pc + 0x80`
  ...
  THEN REPEAT CONJ_TAC THENL [
    ...
    ARM_SIM_TAC ~preprocess_tac:(TRY STRIP_EXISTS_ASSUM_TAC)
        ~canonicalize_pc_diff:false BIGNUM_COPY_ROW_FROM_TABLE_16_EXEC (1--9)
    THEN
  ...
  REWRITE_TAC[] THEN
  ARM_SIM_TAC ~preprocess_tac:(TRY STRIP_EXISTS_ASSUM_TAC)
      ~canonicalize_pc_diff:false BIGNUM_COPY_ROW_FROM_TABLE_16_EXEC (1--23)
  THEN
  CONJ_TAC THENL [
