       ...
       ensures arm
        ...
        (\s. read PC s = word(pc + 0xed0) /\
             (exists e2.
                ...
                // e2 is actually [], but let's keep it to make tactics happy
                e2 = f_events pc /\
...
let CORE_MONTINV_P256_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer.
          ...
          ==> ensures arm
               (\s.
                    ...
                    read events s = e)
               (\s.
                    read PC s = word (pc + 0x10c0) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events x z pc stackpointer /\
                         memaccess_inbounds e2
                          [x,32; z,32; stackpointer,160]
                          [z,32; stackpointer,160]))
               (MAYCHANGE [PC; X0; X1; X2; X3; X4; X5; X6; X7; X8; X9;
                            X10; X11; X12; X13; X14; X15; X16; X17;
                            X19; X20; X21; X22; X23; X24] ,,
                MAYCHANGE SOME_FLAGS ,, MAYCHANGE [events] ,,
  ...
  ENSURES_EVENTS_WHILE_UP2_TAC `9` `pc + 0x550` `pc + 0x550`
   `\i s.
      read SP s = stackpointer /\
      read X20 s = z /\
      read X21 s = word(10 - i)`
  THEN REPEAT CONJ_TAC THENL [
    ...
    ARM_SIM_TAC ~preprocess_tac:(TRY STRIP_EXISTS_ASSUM_TAC)
        ~canonicalize_pc_diff:false CORE_MONTINV_P256_EXEC (1--36) THEN
    ...
    USE_THEN "H_DIVSTEP59_SAFE"
      (MP_TAC o SPECL [`e:(uarch_event)list`;`pc:num`]) THEN
    ...
    REWRITE_TAC[SOME_FLAGS] THEN
    ARM_BIGSTEP_TAC CORE_MONTINV_P256_EXEC "s4" THENL [
    ...
    ARM_STEPS_TAC CORE_MONTINV_P256_EXEC (8--311) THEN
  ...
  ARM_STEPS_TAC CORE_MONTINV_P256_EXEC
   (5--128) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  (* since automation of DISCHARGE_SAFETY_PROPERTY_TAC is insufficient,
      manually write the proof here *)
...
let BIGNUM_MONTINV_P256_SAFE = time prove
 (`exists f_events. forall e z x pc stackpointer.
        ...
        ==> ensures arm
             ...
             (\s. read PC s = word (pc + 0x10d0) /\
                  (exists e2.
                      read events s = APPEND e2 e /\
                      e2 = f_events x z pc stackpointer /\
                      memaccess_inbounds e2 [x,32; z,32; stackpointer,160]
                                            [z,32; stackpointer,160]))
          (MAYCHANGE [PC; X0; X1; X2; X3; X4; X5; X6; X7; X8; X9;
                      X10; X11; X12; X13; X14; X15; X16; X17;
                      X19; X20; X21; X22; X23; X24] ,,
  ...
  DISCH_THEN(REPEAT_TCL CONJUNCTS_THEN ASSUME_TAC) THEN
  REWRITE_TAC(!simulation_precanon_thms) THEN ENSURES_INIT_TAC "s0" THEN
  REMOVE_THEN "H_CORE_SAFE" (fun safe_th -> ARM_SUBROUTINE_SIM_TAC
      (bignum_montinv_p256_mc,BIGNUM_MONTINV_P256_EXEC,0x10,
       (GEN_REWRITE_CONV RAND_CONV [bignum_montinv_p256_mc] THENC
        TRIM_LIST_CONV)
       `TRIM_LIST (16,20) bignum_montinv_p256_mc`,
...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_montinv_p256" subroutine_signatures)
    BIGNUM_MONTINV_P256_SUBROUTINE_CORRECT
...
let BIGNUM_MONTINV_P256_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
           aligned 16 stackpointer /\
           ALL (nonoverlapping (word_sub stackpointer (word 208),208))
           [word pc,4324; z,8 * 4; x,8 * 4] /\
           nonoverlapping (word pc,4324) (z,8 * 4)
           ==> ensures arm
               (\s.
                    aligned_bytes_loaded s (word pc) bignum_montinv_p256_mc /\
                    read PC s = word pc /\
                    read SP s = stackpointer /\
                    read X30 s = returnaddress /\
                    C_ARGUMENTS [z; x] s /\
               ...
               (\s.
                    ...
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 =
                         f_events x z pc (word_sub stackpointer (word 208))
                         returnaddress /\
                         memaccess_inbounds e2
                         [x,32; z,32;
  ...
  ARM_ADD_RETURN_STACK_TAC
   BIGNUM_MONTINV_P256_EXEC BIGNUM_MONTINV_P256_SAFE
   `[X19;X20;X21;X22;X23;X24]` 208 THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;
