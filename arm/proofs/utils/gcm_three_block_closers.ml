...
(* June-2026 base AC-closure helpers (shared by both 3-block closers).         *)
(* bubble_sort_conv only does n passes; iterate to a fixpoint for a canonical  *)
(* XOR normal form.  The final per-half goal reintroduces the mid-pmuls        *)
(* (w?md), qS and qB in assorted XOR-associations; fold them back to their     *)
(* abbreviations -- mids first, then qS, then qB (qB's argument embeds qS) --  *)
(* so the closing fixpoint sort sees only atoms.                               *)
let rec bubble_fix tm =
  let th = bubble_sort_conv tm in
  let r = rhs(concl th) in
  if r = tm then th else TRANS th (bubble_fix r);;

let GCM_3BLOCK_FOLD_MIDS_TAC : tactic =
  fun (asl,gg) ->
...
(* Fold each `word_pmul <arg> (word 13979173243358019584)` whose <arg> has the
   given atom count back to `tgt` (qS=3 atoms, qB=13 atoms), matching up to AC. *)
let GCM_3BLOCK_FOLD_TO tgt natoms : tactic =
...
(* Close one 64-bit half: re-normalise the simulation-side byte shuffles, fold
   the mid-pmuls, then qS, then qB, then AC-sort to a fixpoint. *)
let GCM_3BLOCK_HALF_CLOSE : tactic =
  ...
  GCM_3BLOCK_FOLD_MIDS_TAC THEN ASM_REWRITE_TAC[] THEN
  GCM_3BLOCK_FOLD_TO `qS:(128)word` 3 THEN ASM_REWRITE_TAC[] THEN
  GCM_3BLOCK_FOLD_TO `qB:(128)word` 13 THEN ASM_REWRITE_TAC[] THEN
  CONV_TAC(BINOP_CONV bubble_fix) THEN REFL_TAC;;

(* GCM_3BLOCK_GHASH_STEP_TAC moved to gcm_aesgcm_standalone_blocks_helper.ml (unused by AES256_GCM_ENCRYPT_CORRECT). *)

(* ========================================================================= *)
(*  PARTIAL-FINAL-BLOCK GHASH CLOSER (masked ct3) + helper lemmas.            *)
(*  total bytes = 32 + byte_len (blocks 1,2 full, block 3 = byte_len bytes).  *)
(* ========================================================================= *)

let GCM_3BLOCK_GHASH_STEP_MASKED_TAC =
  ...
  (* Partial last block: abbreviate the mask and the masked block ctm3, and
     bridge the simulator's word_and mask ct3 to ctm3 = word_and ct3 mask. *)
  ...
  MP_TAC(SPECL
    [`word_reversefields 8 (word_xor xi ct1):int128`;
     ...
     `word_reversefields 8 ctm3:int128`;
     `h:int128`;
     `h1k:int128`;
...
let THREEBLOCK_USHR = prove
 (`!byte_len. byte_len <= 16 ==>
  ...
  SUBGOAL_THEN `256 + 8 * byte_len = 8 * (32 + byte_len)` SUBST1_TAC THENL
   [ARITH_TAC; ALL_TAC] THEN
  MATCH_MP_TAC NBLOCK_USHR_BYTELEN THEN ASM_ARITH_TAC);;

let THREEBLOCK_MASK_REG = prove
 (`!byte_len (b0:int128). 1 <= byte_len /\ byte_len <= 16 ==>
    (word_insert
     ((word_insert (b0:int128)
        (0,64)
        (if ~(ival (word_sub (word_and (word_sub (word 0) (word_sub (word_and (word (256 + 8*byte_len):int64) (word 127)) (word 128))) (word 127)) (word 64)) < &0 <=>
              ~(ival (word_and (word_sub (word 0) (word_sub (word_and (word (256 + 8*byte_len):int64) (word 127)) (word 128))) (word 127)) - &64 =
                ival (word_sub (word_and (word_sub (word 0) (word_sub (word_and (word (256 + 8*byte_len):int64) (word 127)) (word 128))) (word 127)) (word 64))))
         then word 18446744073709551615:int64
         else word_jushr (word 18446744073709551615:int64) (word_and (word_sub (word 0) (word_sub (word_and (word (256 + 8*byte_len):int64) (word 127)) (word 128))) (word 127)))):int128)
     (64,64)
     (if ~(ival (word_sub (word_and (word_sub (word 0) (word_sub (word_and (word (256 + 8*byte_len):int64) (word 127)) (word 128))) (word 127)) (word 64)) < &0 <=>
           ~(ival (word_and (word_sub (word 0) (word_sub (word_and (word (256 + 8*byte_len):int64) (word 127)) (word 128))) (word 127)) - &64 =
             ival (word_sub (word_and (word_sub (word 0) (word_sub (word_and (word (256 + 8*byte_len):int64) (word 127)) (word 128))) (word 127)) (word 64))))
        then word_jushr (word 18446744073709551615:int64) (word_and (word_sub (word 0) (word_sub (word_and (word (256 + 8*byte_len):int64) (word 127)) (word 128))) (word 127))
        else word 0:int64)
    : int128)
...
(* total length is X5 = word_ushr (word (256 + 8*byte_len)) 3 = 32 + byte_len *)
(* (two leading full blocks + the partial last block).  Each cmp/b.gt in the  *)
(* cascade (thresholds 96,80,64,48,32,16) leaves the PC as an if-then-else on  *)
(* the signed-greater-than condition; THREEBLOCK_GT_COND collapses that to the *)
(* numeric test t < 32 + byte_len, which the byte_len bounds then decide.      *)
(* ------------------------------------------------------------------------- *)
...
let THREEBLOCK_GT_COND = prove
 (`!byte_len t. byte_len <= 16 /\ t <= 96 ==>
    ((~(val (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word t)) = 0) /\
      (ival (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word t)) < &0 <=>
       ~(ival (word_ushr (word (256 + 8 * byte_len):int64) 3) - &t =
         ival (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word t)))))
     <=> t < 32 + byte_len)`,
  REPEAT STRIP_TAC THEN
  ...
  ASM_SIMP_TAC[IVAL_WORD_SUB_SMALL; NBLOCK_IVAL_WORD_SMALL;
               ARITH_RULE `byte_len <= 16 ==> 32 + byte_len < 2 EXP 63`;
               ARITH_RULE `t <= 96 ==> t < 2 EXP 63`] THEN
  REWRITE_TAC[GSYM INT_OF_NUM_ADD; GSYM INT_OF_NUM_LT] THEN
  ASM_INT_ARITH_TAC);;

let THREEBLOCK_GT_COND_FALSE = prove
 (`!byte_len t. byte_len <= 16 /\ 48 <= t /\ t <= 96 ==>
    ((~(val (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word t)) = 0) /\
      (ival (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word t)) < &0 <=>
       ~(ival (word_ushr (word (256 + 8 * byte_len):int64) 3) - &t =
         ival (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word t)))))
     <=> F)`,
  REPEAT STRIP_TAC THEN
  ASM_SIMP_TAC[THREEBLOCK_GT_COND; ARITH_RULE `48 <= t /\ t <= 96 ==> t <= 96`] THEN
  ASM_ARITH_TAC);;

let THREEBLOCK_GT_COND_TRUE = prove
 (`!byte_len. 1 <= byte_len /\ byte_len <= 16 ==>
    ((~(val (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word 32)) = 0) /\
      (ival (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word 32)) < &0 <=>
       ~(ival (word_ushr (word (256 + 8 * byte_len):int64) 3) - &32 =
         ival (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word 32)))))
     <=> T)`,
  REPEAT STRIP_TAC THEN
  ASM_SIMP_TAC[THREEBLOCK_GT_COND; ARITH_RULE `32 <= 96`] THEN
  ASM_ARITH_TAC);;

let THREEBLOCK_CASC96 = prove
 (`!byte_len. byte_len <= 16 ==>
    ((~(val (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word 96)) = 0) /\
      (ival (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word 96)) < &0 <=>
       ~(ival (word_ushr (word (256 + 8 * byte_len):int64) 3) - &96 =
         ival (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word 96))))) <=> F)`,
  REPEAT STRIP_TAC THEN MATCH_MP_TAC THREEBLOCK_GT_COND_FALSE THEN ASM_ARITH_TAC);;
let THREEBLOCK_CASC80 = prove
 (`!byte_len. byte_len <= 16 ==>
    ((~(val (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word 80)) = 0) /\
      (ival (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word 80)) < &0 <=>
       ~(ival (word_ushr (word (256 + 8 * byte_len):int64) 3) - &80 =
         ival (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word 80))))) <=> F)`,
  REPEAT STRIP_TAC THEN MATCH_MP_TAC THREEBLOCK_GT_COND_FALSE THEN ASM_ARITH_TAC);;
let THREEBLOCK_CASC64 = prove
 (`!byte_len. byte_len <= 16 ==>
    ((~(val (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word 64)) = 0) /\
      (ival (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word 64)) < &0 <=>
       ~(ival (word_ushr (word (256 + 8 * byte_len):int64) 3) - &64 =
         ival (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word 64))))) <=> F)`,
  REPEAT STRIP_TAC THEN MATCH_MP_TAC THREEBLOCK_GT_COND_FALSE THEN ASM_ARITH_TAC);;
let THREEBLOCK_CASC48 = prove
 (`!byte_len. byte_len <= 16 ==>
    ((~(val (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word 48)) = 0) /\
      (ival (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word 48)) < &0 <=>
       ~(ival (word_ushr (word (256 + 8 * byte_len):int64) 3) - &48 =
         ival (word_sub (word_ushr (word (256 + 8 * byte_len):int64) 3) (word 48))))) <=> F)`,
  REPEAT STRIP_TAC THEN MATCH_MP_TAC THREEBLOCK_GT_COND_FALSE THEN ASM_ARITH_TAC);;
...
(* THREEBLOCK_CASCADE_TAC moved to gcm_aesgcm_standalone_blocks_helper.ml (unused by AES256_GCM_ENCRYPT_CORRECT). *)
