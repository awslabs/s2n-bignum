...
let CORE_MONTINV_P384_SAFE = time prove
 (`exists f_events.
       forall e z x pc stackpointer.
          ...
          ==> ensures arm
               (\s.
                    ...
                    read events s = e)
               (\s.
                    read PC s = word (pc + 0x13bc) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events x z pc stackpointer /\
                         memaccess_inbounds e2
                          [x,48; z,48; stackpointer,256]
                          [z,48; stackpointer,256]))
               (MAYCHANGE [PC; X0; X1; X2; X3; X4; X5; X6; X7; X8; X9;
                            X10; X11; X12; X13; X14; X15; X16; X17;
                            X19; X20; X21; X22; X23; X24] ,,
                MAYCHANGE SOME_FLAGS ,, MAYCHANGE [events] ,,
  ...
  ENSURES_EVENTS_WHILE_UP2_TAC `14` `pc + 0x788` `pc + 0x788`
   `\i s.
      read SP s = stackpointer /\
      read X20 s = z /\
      read X21 s = word(15 - i)`
  THEN REPEAT CONJ_TAC THENL [
    ...
    ARM_SIM_TAC ~preprocess_tac:(TRY STRIP_EXISTS_ASSUM_TAC)
        ~canonicalize_pc_diff:false CORE_INV_P384_EXEC (1--42) THEN
    ...
    REWRITE_TAC[SOME_FLAGS] THEN
    ARM_BIGSTEP_TAC CORE_INV_P384_EXEC "s4" THENL [
    ...
    ARM_STEPS_TAC CORE_INV_P384_EXEC (8--447) THEN
  ...
  ARM_STEPS_TAC CORE_INV_P384_EXEC
   (5--177) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  (* since automation of DISCHARGE_SAFETY_PROPERTY_TAC is insufficient,
      manually write the proof here *)
        ...
        ==> ensures arm
             ...
             (\s. read PC s = word (pc + 0x13cc) /\
                  (exists e2.
                      read events s = APPEND e2 e /\
                      e2 = f_events x z pc stackpointer /\
                      memaccess_inbounds e2 [x,48; z,48; stackpointer,256]
                                            [z,48; stackpointer,256]))
          (MAYCHANGE [PC; X0; X1; X2; X3; X4; X5; X6; X7; X8; X9;
                      X10; X11; X12; X13; X14; X15; X16; X17;
                      X19; X20; X21; X22; X23; X24] ,,
           ...
           MAYCHANGE [memory :> bytes(z,8 * 6);
                      memory :> bytes(stackpointer,256)])`,
  ASSUME_CALLEE_SAFETY_TAC CORE_MONTINV_P384_SAFE "H_CORE_SAFE" THEN
  META_EXISTS_TAC THEN
  MAP_EVERY X_GEN_TAC
 ...
 (`exists f_events.
       forall e z x pc stackpointer returnaddress.
          ...
          nonoverlapping (word pc,0x13e0) (z,8 * 6)
           ==> ensures arm
               (\s.
                    aligned_bytes_loaded s (word pc) bignum_montinv_p384_mc /\
                    read PC s = word pc /\
                    read SP s = stackpointer /\
                    read X30 s = returnaddress /\
                    C_ARGUMENTS [z; x] s /\
               ...
               (\s.
                    ...
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 =
                         f_events x z pc (word_sub stackpointer (word 304))
                         returnaddress /\
                         memaccess_inbounds e2
                         [x,48; z,48;
  ...
  ARM_ADD_RETURN_STACK_TAC
   BIGNUM_MONTINV_P384_EXEC BIGNUM_MONTINV_P384_SAFE
   `[X19;X20;X21;X22;X23;X24]` 304 THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;
