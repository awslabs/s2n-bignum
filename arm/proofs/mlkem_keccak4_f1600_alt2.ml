...
(**** print_literal_from_elf "arm/mlkem/mlkem_keccak4_f1600_alt2.o";;
 ****)

let mlkem_keccak4_f1600_alt2_mc = define_assert_from_elf
  "mlkem_keccak4_f1600_alt2_mc" "arm/mlkem/mlkem_keccak4_f1600_alt2.o"
[
...
let MLKEM_KECCAK4_F1600_ALT2_EXEC = ARM_MK_EXEC_RULE mlkem_keccak4_f1600_alt2_mc;;
...
let MLKEM_KECCAK4_F1600_ALT2_CORRECT = prove
 (`!a rc A1 A2 A3 A4 pc stackpointer.
      ...
      ==> ensures arm
           (\s. aligned_bytes_loaded s (word pc) mlkem_keccak4_f1600_alt2_mc /\
  ...
  REWRITE_TAC[fst MLKEM_KECCAK4_F1600_ALT2_EXEC] THEN
  ...
  REWRITE_TAC[condition_semantics] THEN REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    ARM_STEPS_TAC MLKEM_KECCAK4_F1600_ALT2_EXEC (1--395) THEN
    ...
    ARM_STEPS_TAC MLKEM_KECCAK4_F1600_ALT2_EXEC (1--339) THEN
    ...
    CONV_TAC(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV) THEN
    CONV_TAC(ONCE_DEPTH_CONV NORMALIZE_RELATIVE_ADDRESS_CONV) THEN
    ARM_SIM_TAC MLKEM_KECCAK4_F1600_ALT2_EXEC [1] THEN
  ...
  REWRITE_TAC[GSYM round_constants] THEN
  ENSURES_INIT_TAC "s0" THEN
  ARM_STEPS_TAC MLKEM_KECCAK4_F1600_ALT2_EXEC (1--83) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONV_TAC(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV) THEN
  CONV_TAC(ONCE_DEPTH_CONV NORMALIZE_RELATIVE_ADDRESS_CONV) THEN
  ASM_REWRITE_TAC[] THEN
...
let MLKEM_KECCAK4_F1600_ALT2_SUBROUTINE_CORRECT = prove
 (`!a rc A1 A2 A3 A4 pc stackpointer returnaddress.
  ...
  ARM_ADD_RETURN_STACK_TAC ~pre_post_nsteps:(11,11) MLKEM_KECCAK4_F1600_ALT2_EXEC
   (CONV_RULE TWEAK_CONV MLKEM_KECCAK4_F1600_ALT2_CORRECT)
