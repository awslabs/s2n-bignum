...
let LOCAL_MUL_P25519_TAC =
  ARM_MACRO_SIM_ABBREV_TAC curve25519_pxscalarmul_mc 180 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      nonoverlapping (word pc,0x197c) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures arm
...
let CURVE25519_PXSCALARMUL_CORRECT = time prove
 (`!res scalar n point X pc stackpointer.
    ...
    ALLPAIRS nonoverlapping
      ...
      [(word pc,0x197c); (scalar,32); (point,32)] /\
  ...
  ENSURES_WHILE_PDOWN_TAC `256` `pc + 0x4c` `pc + 0x18e0`
   `\i s.
     (read SP s = stackpointer /\
      read X17 s = res /\
      read X19 s = point /\
  ...
  REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    ARM_STEPS_TAC CURVE25519_PXSCALARMUL_EXEC (1--16) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    CONV_TAC(ONCE_DEPTH_CONV BIGNUM_LEXPAND_CONV) THEN
