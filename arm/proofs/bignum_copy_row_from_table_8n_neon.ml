...
[
  0xb40005c2;       (* arm_CBZ X2 (word 184) *)
  0xb40005a3;       (* arm_CBZ X3 (word 180) *)
  ...
  0x4e301e31;       (* arm_AND_VEC Q17 Q17 Q16 128 *)
  0x4eb11e52;       (* arm_ORR_VEC Q18 Q18 Q17 128 *)
  ...
  0x54fffc21;       (* arm_BNE (word 2097028) *)
...
let ADD_SUB_SWAP = prove(`!x y z. y >= z /\ x >= z ==> x + (y - z) = y + (x - z)`,
  ARITH_TAC);;
...
let ABBREV_Z_READ_128BITS_TAC name st ofs =
  ...
  let rhs = subst [(st,`st0:armstate`)] templ in
...
g `!z table height width idx pc n m returnaddress.
    nonoverlapping (word pc, 0xbc) (z, 8 * val width) /\
    nonoverlapping (word pc, 0xbc) (table, 8 * val height * val width) /\
...
e(ENSURES_SEQUENCE_TAC `pc + 0x14`
...
e(SUBGOAL_THEN `word (8 * val width DIV 8) = width:int64` ASSUME_TAC THENL [
...
e(ENSURES_WHILE_PDOWN_TAC `val (width:64 word) DIV 8:num` `pc + 0x14` `pc + 0x2c`
...
e(ENSURES_WHILE_PUP_TAC `val (height:64 word):num` `pc + 0x38` `pc + 0xb4`
  `\i s.  (read X30 s = returnaddress /\ read X0 s = z /\ read X1 s = table /\
          read X2 s = height /\ read X3 s = width /\ read X4 s = idx /\
...
e(REPEAT STRIP_TAC);;
e(SUBGOAL_THEN `i < 2 EXP 64` ASSUME_TAC THENL [ASM_MESON_TAC[LT_WORD_64]; ALL_TAC]);;
e(SUBGOAL_THEN `i + 1 < 2 EXP 64` ASSUME_TAC THENL [
...
e(ENSURES_WHILE_PDOWN_TAC `(val (width:64 word) DIV 8):num` `pc + 0x4c` `pc + 0xa8`
  ...
  ASM_CASES_TAC `i < val (idx:int64)` THENL [
    ...
    ASM_CASES_TAC `i = val (idx:int64)` THENL [
      ...
      REWRITE_ASSUMES_TAC `~(i = val (idx:int64))` THEN
      REWRITE_ASSUMES_TAC `~(i < val (idx:int64))` THEN
      SPLIT_FIRST_CONJ_TAC THEN (* get `i > val idx` *)
      DISJ2_TAC THEN
...
e(MAP_EVERY ASSERT_USING_ASM_ARITH_TAC [
  `val (width:int64) - 8 * (i' + 1) + 2 <= val width`;
  `(val (width:int64) - 8 * (i' + 1) + 2) + 2 <= val width`;
  `(val (width:int64) - 8 * (i' + 1) + 4) + 2 <= val width`;
  `(val (width:int64) - 8 * (i' + 1) + 6) + 2 <= val width`]);;
e(MAP_EVERY (fun t -> ASSERT_USING_UNDISCH_AND_ARITH_TAC t `8 * val (width:int64) < 2 EXP 64`) [
  ...
  `8 * (val (width:int64) - 8 * (i' + 1)) + 16 <= 18446744073709551616`]);;
e(MAP_EVERY (fun t -> SUBGOAL_THEN t ASSUME_TAC THENL [
  ...
  `(i * val (width:int64) + val width - 8 * (i' + 1)) + 2 <= val (height:int64) * val width`]);;
e(MAP_EVERY (fun i ->
    let si = string_of_int i in
    ABBREV_TABLE_READ_128BITS_TAC ("p" ^ si) `s0:armstate` (i*16) THEN
    ABBREV_Z_READ_128BITS_TAC ("q" ^ si) `s0:armstate` (i*16)) (0--3));;

e(ASM_CASES_TAC `i = val (idx:int64)` THENL [
  (* case 1 *)
  ...
  ARM_STEPS_TAC BIGNUM_COPY_ROW_FROM_TABLE_8N_NEON_EXEC (1--23) THEN
  ...
  (* case 2 *)
e(REWRITE_ASSUMES_TAC `~(i = val (idx:int64))` THEN
  REWRITE_TAC[ASSUME `~(i = val (idx:int64))`] THEN
  ASM_CASES_TAC `i < val (idx:int64)` THENL [
    ASSERT_USING_UNDISCH_AND_ARITH_TAC `~(i > val (idx:int64))` `i < val (idx:int64)` THEN
    MAP_EVERY REWRITE_ASSUMES_TAC [`~(i > val (idx:int64))`; `i < val (idx:int64)`] THEN
...
e(ASSERT_USING_ASM_ARITH_TAC `i > val (idx:int64)` THEN
  MAP_EVERY REWRITE_ASSUMES_TAC [`~(i < val (idx:int64))`; `i > val (idx:int64)`] THEN
  MAP_EVERY (fun t -> REWRITE_TAC[ASSUME t]) [`~(i < val (idx:int64))`; `i > val (idx:int64)`] THEN
  SPLIT_FIRST_CONJ_TAC THEN
  ...
  REPEAT CONJ_TAC THENL [
    ...
    REPEAT CONJ_TAC THENL [
      ...
      ASM_SIMP_TAC [WORD_AND_0;WORD_OR_0;VAL_WORD_EQ;DIMINDEX_128;ARITH_RULE`128=64*2`;LOWDIGITS_BOUND] THEN
      REPEAT CONJ_TAC THEN TRY (REWRITE_TAC [WORD_ADD_ASSOC_CONSTS] THEN CONV_TAC WORD_ARITH) THEN
      EXPAND_TAC "m" THEN
