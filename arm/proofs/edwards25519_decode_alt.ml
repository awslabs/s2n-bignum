...
let LOCAL_MUL_P25519_SAFE = time prove
 (`exists f_events.
        forall e z x y pc.
            nonoverlapping (word pc,1968) (z,8 * 4)
            ==> ensures arm
                (\s.
                     aligned_bytes_loaded s (word pc)
                       edwards25519_decode_alt_mc /\
                     read PC s = word (pc + 1224) /\
                     C_ARGUMENTS [z; x; y] s /\
                     read events s = e)
                (\s.
                     read PC s = word (pc + 1624) /\
                     (exists e2.
...
let LOCAL_MUL_SAFETY_TAC (assump_name:string) (n:int) =
  REMOVE_THEN assump_name (fun safety_th ->
    ARM_SUBROUTINE_SIM_TAC ~is_safety_thm:true
        (edwards25519_decode_alt_mc,EDWARDS25519_DECODE_ALT_EXEC,
          0x0,edwards25519_decode_alt_mc,safety_th)
...
let LOCAL_NSQR_P25519_SAFE = time prove
 (`exists f_events.
  ...
  CONCRETIZE_F_EVENTS_TAC
    `\(z:int64) (k:int64) (x:int64) (pc:num).
      APPEND
        (f_ev_loop_post z k x pc)
        (APPEND
          (ENUMERATEL (val k) (f_ev_loop z k x pc))
            (f_ev_loop_pre z k x pc))
  ...
  ENSURES_EVENTS_WHILE_UP2_TAC `val (k:int64)` `pc + 0x668` `pc + 0x780`(*+1*)
   `\i s. read X0 s = z /\
          read X1 s = word((val (k:int64)) - i)` THEN REPEAT CONJ_TAC THENL [
    ASM_ARITH_TAC;
    ...
    ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (1--11) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    DISCHARGE_SAFETY_PROPERTY_TAC
  ] THEN
  ...
  ENSURES_INIT_TAC "s0" THEN
  STRIP_EXISTS_ASSUM_TAC THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (1--70) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONJ_TAC THENL [
...
let EDWARDS25519_DECODE_ALT_SAFE = time prove
 (`exists f_events.
       forall e z c pc stackpointer.
           aligned 16 stackpointer /\
           ALL (nonoverlapping (stackpointer,192))
           [word pc,1968; z,8 * 8; c,8 * 4] /\
           nonoverlapping (word pc,1968) (z,8 * 8)
           ==> ensures arm
               (\s.
                    aligned_bytes_loaded s (word pc)
                    edwards25519_decode_alt_mc /\
               ...
               (\s.
                    ...
                    (exists e2.
                         ...
                         memaccess_inbounds e2 [c,32; z,64; stackpointer,192]
                         [z,64; stackpointer,192]))
               (MAYCHANGE
                [PC; X0; X1; X2; X3; X4; X5; X6; X7; X8; X9; X10; X11; X12;
                 X13; X14; X15; X16; X17; X19; X20; X21; X30] ,,
                MAYCHANGE SOME_FLAGS ,,
                MAYCHANGE [events] ,,
                MAYCHANGE
                [memory :> bytes (z,8 * 8);
  ...
  REWRITE_TAC[ALL; C_ARGUMENTS; SOME_FLAGS] THEN
  REPEAT STRIP_TAC THEN

  ENSURES_INIT_TAC "s0" THEN
  (* simply use symbolic sim to cover the first part of ENSURES_SEQUENCE_TAC *)
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (1--74) THEN
  (* now the second part of ENSURES_SEQUENCE_TAC. *)
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (75--78) THEN
  LOCAL_NSQR_SAFETY_TAC "H_NSQR" 79 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (80--111) THEN
  LOCAL_MUL_SAFETY_TAC "H_MUL" 112 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (113--121) THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (122--125) THEN
  LOCAL_MUL_SAFETY_TAC "H_MUL" 126 THEN

  (*** The power tower. Simply use the original step numbers in fn correctness
       proof! ***)
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (53--57) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR" 58 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (59--63) THEN LOCAL_MUL_SAFETY_TAC "H_MUL" 64 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (65--69) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR" 70 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (71--75) THEN LOCAL_MUL_SAFETY_TAC "H_MUL" 76 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (77--81) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR" 82 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (83--87) THEN LOCAL_MUL_SAFETY_TAC "H_MUL" 88 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (89--93) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR" 94 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (95--99) THEN LOCAL_MUL_SAFETY_TAC "H_MUL" 100 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (101--105) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR" 106 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (107--111) THEN LOCAL_MUL_SAFETY_TAC "H_MUL" 112 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (113--117) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR" 118 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (119--123) THEN LOCAL_MUL_SAFETY_TAC "H_MUL" 124 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (125--129) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR" 130 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (131--135) THEN LOCAL_MUL_SAFETY_TAC "H_MUL" 136 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (137--141) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR" 142 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (143--147) THEN LOCAL_MUL_SAFETY_TAC "H_MUL" 148 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (149--153) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR" 154 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (155--159) THEN LOCAL_MUL_SAFETY_TAC "H_MUL" 160 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (161--165) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR" 166 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (167--171) THEN LOCAL_MUL_SAFETY_TAC "H_MUL" 172 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (173--177) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR" 178 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (179--183) THEN LOCAL_MUL_SAFETY_TAC "H_MUL" 184 THEN

  (*** e^2 * w computation ***)
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (185--189) THEN LOCAL_NSQR_SAFETY_TAC "H_NSQR" 190 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (191--195) THEN LOCAL_MUL_SAFETY_TAC "H_MUL" 196 THEN

  (*** s = u * e ***)
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (197--201) THEN LOCAL_MUL_SAFETY_TAC "H_MUL" 202 THEN
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (203--225) THEN
  LOCAL_MUL_SAFETY_TAC "H_MUL" 226 THEN

  (*** The remaining parts ***)
  ARM_STEPS_TAC EDWARDS25519_DECODE_ALT_EXEC (227--283) THEN
