...
let P384_MONTJADD_ALT_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 p2 pc stackpointer.
           aligned 16 stackpointer /\
           ALL (nonoverlapping (stackpointer,336))
           [word pc,17536; p1,144; p2,144; p3,144] /\
           nonoverlapping (p3,144) (word pc,17536)
           ==> ensures arm
               (\s.
                    aligned_bytes_loaded s (word pc) p384_montjadd_alt_mc /\
                    read PC s = word (pc + 20) /\
                    read SP s = stackpointer /\
                    C_ARGUMENTS [p3; p1; p2] s /\
                    read events s = e)
               (\s.
                    read PC s = word (pc + 17512) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events p1 p2 p3 pc stackpointer /\
                         memaccess_inbounds e2
                         [p1,144; p2,144; p3,144; stackpointer,336]
 ...
 (`exists f_events.
  ...
  ARM_ADD_RETURN_STACK_TAC P384_MONTJADD_ALT_EXEC
   P384_MONTJADD_ALT_SAFE
    `[X19; X20; X21; X22; X23; X24; X25; X26]` 400 THEN
