...
let BIGNUM_MUL_8_16_EQUIV = prove(
  ...
  REWRITE_TAC[SOME_FLAGS;ALL;NONOVERLAPPING_CLAUSES;BIGNUM_MUL_8_16_EXEC;
              BIGNUM_MUL_8_16_NEON_EXEC;BIGNUM_MUL_8_16_CORE_EXEC;
  ...
  ASSERT_NONOVERLAPPING_MODULO_TAC
    `nonoverlapping_modulo (2 EXP 64)
        (val (z:int64),8 * 16) (pc2,LENGTH bignum_mul_8_16_neon_core_mc)`
    BIGNUM_MUL_8_16_NEON_CORE_EXEC THEN
...
let BIGNUM_MUL_8_16_NEON_CORE_CORRECT = prove(
  ...
  VCGEN_EQUIV_TAC BIGNUM_MUL_8_16_EQUIV BIGNUM_MUL_8_16_CORE_CORRECT_N
    [BIGNUM_MUL_8_16_EXEC; BIGNUM_MUL_8_16_NEON_CORE_EXEC] THEN

  (* unravel definitions that may block reasonings *)
  RULE_ASSUM_TAC (REWRITE_RULE([ALL;NONOVERLAPPING_CLAUSES;BIGNUM_MUL_8_16_EXEC])) THEN
  ...
  REPEAT CONJ_TAC THENL [
    ...
    (** SUBGOAL 2. Postcond **)
    MESON_TAC[BIGNUM_MUL_8_16_NEON_CORE_EXEC;bignum_mul_8_16_equiv_output_states;BIGNUM_FROM_MEMORY_BYTES];
...
let BIGNUM_MUL_8_16_NEON_CORRECT = prove(
  ...
  REPEAT GEN_TAC THEN
  DISCH_THEN (ASSUME_TAC o (REWRITE_RULE [BIGNUM_MUL_8_16_NEON_EXEC])) THEN
  MP_TAC (SPECL [`z:int64`;`x:int64`;`y:int64`;`a:num`;`b:num`;`pc2+12`]
    (REWRITE_RULE[BIGNUM_MUL_8_16_NEON_CORE_EXEC]
  ...
  REPEAT CONJ_TAC THENL [
    REPEAT STRIP_TAC THEN ASM_REWRITE_TAC[bignum_mul_8_16_neon_core_mc_def] THEN
    REWRITE_TAC[WORD_RULE`word (pc2+12):int64 = word_add (word pc2) (word 12)`] THEN
