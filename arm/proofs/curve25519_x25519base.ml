...
let curve25519_x25519base_mc' =
  SPECL [`pc:num`; `tables:num`] curve25519_x25519base_mc;;
...
let LOCAL_MUL_P25519_TAC =
  ARM_MACRO_SIM_ABBREV_TAC curve25519_x25519base_mc' 180 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      nonoverlapping (word pc,0x2c1c) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures arm
           (\s. aligned_bytes_loaded s (word pc) (curve25519_x25519base_mc pc tables) /\
                read PC s = pcin /\
                read SP s = read SP t /\
...
let CURVE25519_X25519BASE_CORRECT = time prove
 (`!tables res scalar n pc stackpointer.
    ...
    ALL (nonoverlapping (stackpointer,448))
        [(word pc,0x2c1c); (word tables,48576); (res,32); (scalar,32)] /\
    ALL (nonoverlapping (res,32)) [(word pc,0x2c1c); (word tables,48576)]
    ==> ensures arm
