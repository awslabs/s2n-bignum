...
(**** print_literal_from_elf "arm/mlkem/mlkem_keccak4_f1600.o";;
 ****)

let mlkem_keccak4_f1600_mc = define_assert_from_elf
  "mlkem_keccak4_f1600_mc" "arm/mlkem/mlkem_keccak4_f1600.o"
[
...
let MLKEM_KECCAK4_F1600_EXEC = ARM_MK_EXEC_RULE mlkem_keccak4_f1600_mc;;
...
(* ------------------------------------------------------------------------- *)
(* Some custom simplification to avoid overkill of SAT for  bit-blasting.    *)
(* ------------------------------------------------------------------------- *)
...
let IFF_NOT_CONV =
  let pth = TAUT
   `((~p <=> q) <=> ~(p <=> q)) /\
    ((p <=> ~q) <=> ~(p <=> q))` in
  GEN_REWRITE_CONV REDEPTH_CONV [pth; NOT_CLAUSES; EQ_CLAUSES];;
...
and IFF_RAND_CONV tm =
  ...
  CONV_RULE(RAND_CONV(TRY_CONV conv_t)) th;;
...
let IFF_CONV = IFF_NOT_CONV THENC IFF_NORM_CONV;;

let IFF_TAC =
  CONV_TAC(BINOP_CONV(IFF_NOT_CONV THENC IFF_NORM_CONV)) THEN REFL_TAC;;

let SIMPLE_BITBLAST_TAC =
...
let MLKEM_KECCAK4_F1600_CORRECT = prove
 (`!a rc A1 A2 A3 A4 pc stackpointer.
      ...
      ==> ensures arm
           (\s. aligned_bytes_loaded s (word pc) mlkem_keccak4_f1600_mc /\
  ...
  REWRITE_TAC[fst MLKEM_KECCAK4_F1600_EXEC] THEN
  ...
  REWRITE_TAC[condition_semantics] THEN REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    ARM_STEPS_TAC MLKEM_KECCAK4_F1600_EXEC (1--349) THEN
    ...
    CONJ_TAC THENL
     [GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV)
      ...
      POP_ASSUM_LIST(K ALL_TAC) THEN SIMPLE_BITBLAST_TAC;
    ...
    ARM_STEPS_TAC MLKEM_KECCAK4_F1600_EXEC (1--293) THEN
    ...
    CONV_TAC(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV) THEN
    CONV_TAC(ONCE_DEPTH_CONV NORMALIZE_RELATIVE_ADDRESS_CONV) THEN
    ARM_SIM_TAC MLKEM_KECCAK4_F1600_EXEC [1] THEN
  ...
  REWRITE_TAC[GSYM round_constants] THEN
  ENSURES_INIT_TAC "s0" THEN
  ARM_STEPS_TAC MLKEM_KECCAK4_F1600_EXEC (1--83) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONV_TAC(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV) THEN
  CONV_TAC(ONCE_DEPTH_CONV NORMALIZE_RELATIVE_ADDRESS_CONV) THEN
  ASM_REWRITE_TAC[] THEN
...
let MLKEM_KECCAK4_F1600_SUBROUTINE_CORRECT = prove
 (`!a rc A1 A2 A3 A4 pc stackpointer returnaddress.
  ...
  ARM_ADD_RETURN_STACK_TAC ~pre_post_nsteps:(11,11) MLKEM_KECCAK4_F1600_EXEC
