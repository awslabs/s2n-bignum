...
let GT_REFL = prove(`!x. ~(x > x)`, ARITH_TAC);;

(* `val idx * val width + val width <= val height * val width` *)
let LE_MULT_ADD = prove(`!x x2 y. x < x2 ==> x * y + y <= x2 * y`,
    REPEAT STRIP_TAC THEN REWRITE_TAC[ARITH_RULE `x * y + y= (x + 1)*y`] THEN
    REWRITE_TAC[LE_MULT_RCANCEL] THEN
    ASM_ARITH_TAC);;

let SPLIT_FIRST_CONJ_TAC = FIRST_X_ASSUM (fun thm -> let t1,t2 = CONJ_PAIR thm in
                              MAP_EVERY ASSUME_TAC [t1;t2]);;

let CASES_FIRST_DISJ_TAC = FIRST_X_ASSUM (fun thm ->
...
e(REPEAT GEN_TAC THEN REWRITE_TAC[C_ARGUMENTS; NONOVERLAPPING_CLAUSES;
              MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI] THEN
  REPEAT STRIP_TAC);;

e(ASM_CASES_TAC `val (height:(64)word) = 0` THENL [
...
e(ENSURES_SEQUENCE_TAC `pc + 0x10`
...
e(SUBGOAL_THEN `val (idx:int64) * val (width:int64) + val width <= val (height:int64) * val width` ASSUME_TAC
    THENL [IMP_REWRITE_TAC[LE_MULT_ADD]; ALL_TAC]);;

(* bignum_copy_row_from_table_initzero *)
e(ENSURES_WHILE_PDOWN_TAC `val (width:64 word):num` `pc + 0x10` `pc + 0x1c`
...
e(ENSURES_WHILE_PUP_TAC `val (height:64 word):num` `pc + 0x28` `pc + 0x64`
  `\i s.  (read X30 s = returnaddress /\ read X0 s = z /\ read X1 s = table /\
          read X2 s = height /\ read X3 s = width /\ read X4 s = idx /\
  ...
  CASES_FIRST_DISJ_TAC THEN SPLIT_FIRST_CONJ_TAC THENL [
    UNDISCH_TAC `val (idx:int64) < val (height:int64)` THEN
...
e(ENSURES_WHILE_PDOWN_TAC `val (width:64 word):num` `pc + 0x38` `pc + 0x58`
  ...
  ASM_CASES_TAC `i < val (idx:int64)` THENL [
    ...
    ASM_CASES_TAC `i = val (idx:int64)` THENL [
      ...
      SPLIT_FIRST_CONJ_TAC THEN DISJ2_TAC THEN
      ...
      REWRITE_ASSUMES_TAC `~(i = val (idx:int64))` THEN
      REWRITE_ASSUMES_TAC `~(i < val (idx:int64))` THEN
      REPEAT SPLIT_FIRST_CONJ_TAC THEN (* get `i > val idx` *)
...
e(REPEAT STRIP_TAC THEN REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES]);;

e(MATCH_MP_TAC ENSURES_FRAME_SUBSUMED THEN
  EXISTS_TAC `MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
       MAYCHANGE [memory :> bytes64 (word (val (z:int64) + 8 * (val (width:int64) - (i' + 1))))]` THEN
  REWRITE_TAC[MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI] THEN
  CONJ_TAC THENL [
...
e(ASM_CASES_TAC `i = val (idx:int64)`);;
  (* case 1 *)
  e(REWRITE_TAC[ASSUME `i = val (idx:int64)`; LT_REFL; GT_REFL] THEN
    REWRITE_ASSUMES_TAC `i = val (idx:int64)` THEN
    RULE_ASSUM_TAC (REWRITE_RULE [LT_REFL; GT_REFL]) THEN
    SPLIT_FIRST_CONJ_TAC THEN
    SUBGOAL_THEN `
    q:int64 = word 0 /\
    read (memory :>
      bytes (word_add z (word (8 * (val (width:int64) - i'))),8 * i')) s4 = 0`
    (fun thm -> MAP_EVERY ASSUME_TAC (let x,y=CONJ_PAIR thm in [x;y])));;
    e(CONJ_TAC);;
      e(EXPAND_TAC "q" THEN REWRITE_TAC[GSYM VAL_EQ] THEN
  ...
  e(SUBST_ALL_TAC (ASSUME `q=word 0:int64`));;
  e(SUBGOAL_THEN `val (width:int64) - (i' + 1) < val (width:int64) - i'` ASSUME_TAC THENL
    [ASM_ARITH_TAC; ALL_TAC]);;
  e(SUBGOAL_THEN `8 * (val (width:int64) - i') + 8 * i' <= 18446744073709551616` ASSUME_TAC THENL [
    ASM_ARITH_TAC; ALL_TAC]);;
  e(SUBGOAL_THEN `8 * (val (width:int64) - (i' + 1)) + 8 <= 18446744073709551616` ASSUME_TAC THENL [
    ASM_ARITH_TAC; ALL_TAC]);;
  e(ARM_STEPS_TAC BIGNUM_COPY_ROW_FROM_TABLE_EXEC [5]);;
  e(ARM_STEPS_TAC BIGNUM_COPY_ROW_FROM_TABLE_EXEC [6;7;8]);;
  e(ENSURES_FINAL_STATE_TAC);;
  e(ASM_REWRITE_TAC[WORD_SUB_ADD; WORD_ADD_ASSOC_CONSTS]);;
  e(REPEAT CONJ_TAC);;
    e(REPEAT AP_TERM_TAC);;
    e(UNDISCH_TAC `i' < val (width:int64)` THEN ARITH_TAC);;

    e(REPEAT AP_TERM_TAC);;
    e(UNDISCH_TAC `i' < val (width:int64)` THEN ARITH_TAC);;

    e(REWRITE_TAC[GSYM BIGNUM_FROM_MEMORY_BYTES]);;
    e(SUBGOAL_THEN `val (width:int64) - i' = val width - (i'+1) + 1` (fun thm -> REWRITE_TAC[thm]) THENL [
      UNDISCH_TAC `i' < val (width:int64)` THEN ARITH_TAC; ALL_TAC]);;
    e(ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_EQ_LOWDIGITS; BIGNUM_FROM_MEMORY_BYTES]);;
    e(REWRITE_TAC[WORD_BLAST `word_and p (word 18446744073709551615) = p:int64`]);;
    e(MAP_EVERY EXPAND_TAC ["p";"m"]);;
    e(ONCE_REWRITE_TAC[GSYM VAL_EQ]);;
    e(REWRITE_TAC[GSYM BIGNUM_FROM_MEMORY_BYTES]);;
    e(REWRITE_TAC[BIGDIGIT_BIGNUM_FROM_MEMORY]);;
    e(SUBGOAL_THEN `val (width:int64) - (i' + 1) < val width` (fun thm->REWRITE_TAC[thm]) THENL [
      UNDISCH_TAC `i'<val (width:int64)` THEN ARITH_TAC; ALL_TAC]);;
    e(REWRITE_TAC[WORD_VAL; WORD_ADD_ASSOC_CONSTS; LEFT_ADD_DISTRIB]);;

    e(IMP_REWRITE_TAC[VAL_WORD; MOD_LT; DIMINDEX_64; LT_WORD_64]);;

  (* case 2 *)
  e(ASM_CASES_TAC `i < val (idx:int64)`);;
    e(REWRITE_TAC [ASSUME `i < val (idx:int64)`]);;
    e(REWRITE_ASSUMES_TAC `i < val (idx:int64)`);;
    e(REWRITE_TAC [ASSUME `~(i = val (idx:int64))`]);;
    e(REWRITE_ASSUMES_TAC `~(i = val (idx:int64))`);;
    e(SUBGOAL_THEN `~(i > val (idx:int64))` ASSUME_TAC THENL
      [UNDISCH_TAC `i < val (idx:int64)` THEN ARITH_TAC; ALL_TAC]);;
    e(REWRITE_TAC [ASSUME `~(i > val (idx:int64))`]);;
    e(REWRITE_ASSUMES_TAC `~(i > val (idx:int64))`);;
    e(SUBGOAL_THEN `q = word 0:int64` SUBST_ALL_TAC THENL [
      ...
      ALL_TAC]);;
    e(SPLIT_FIRST_CONJ_TAC);;
    e(ASSERT_USING_ASM_ARITH_TAC `8 * (val (width:int64) - (i' + 1)) + 8 <= 18446744073709551616`);;
    e(SUBGOAL_THEN `read (memory :> bytes
      ...
      [IMP_REWRITE_TAC[READ_MEMORY_BYTES_SLICE; HIGHDIGITS_TRIVIAL; LOWDIGITS_TRIVIAL] THEN ARITH_TAC;
       ALL_TAC]);;
    e(SUBGOAL_THEN `8 * (val (width:int64) - (i' + 1)) + 8 * 1 = 8 * (val (width:int64) - i')`
      (fun thm -> RULE_ASSUM_TAC (REWRITE_RULE [thm])) THENL [
        UNDISCH_TAC `i' < val (width:int64)` THEN ARITH_TAC; ALL_TAC]);;
    e(ASSERT_USING_UNDISCH_AND_ARITH_TAC `val (width:int64) - (i' + 1) < val (width:int64) - i'` `i' < val (width:int64)`);;
    e(ASSERT_USING_ASM_ARITH_TAC `8 * (val (width:int64) - i') + 8 * i' <= 18446744073709551616`);;
    e(ARM_STEPS_TAC BIGNUM_COPY_ROW_FROM_TABLE_EXEC [5]);;
    e(ARM_STEPS_TAC BIGNUM_COPY_ROW_FROM_TABLE_EXEC [6;7;8]);;
    e(ENSURES_FINAL_STATE_TAC);;
    e(ASM_REWRITE_TAC[WORD_SUB_ADD; WORD_ADD_ASSOC_CONSTS]);;
    e(REPEAT CONJ_TAC);;
      ...
      e(MATCH_MP_TAC (SPECL [`z:int64`; `(val (width:int64) - (i' + 1)):num`; `1:num`] READ_MEMORY_BYTES_MERGE));;
      e(ASM_REWRITE_TAC[LOWDIGITS_TRIVIAL; HIGHDIGITS_TRIVIAL; ARITH_RULE `8*1=8`; READ_MEMORY_BYTES_BYTES64;
          WORD_AND_0; VAL_WORD_0] THEN UNDISCH_TAC `i' < val (width:int64)` THEN ARITH_TAC);;

      e(IMP_REWRITE_TAC[VAL_WORD; MOD_LT; DIMINDEX_64; LT_WORD_64]);;

    (* last case *)
    e(ASSERT_USING_ASM_ARITH_TAC `i > val (idx:int64)`);;
    e(MAP_EVERY (fun t -> REWRITE_TAC [ASSUME t] THEN REWRITE_ASSUMES_TAC t)
      [`i > val (idx:int64)`; `~(i = val (idx:int64))`; `~(i < val (idx:int64))`]);;
    e(SPLIT_FIRST_CONJ_TAC);;
    e(ASSERT_USING_ASM_ARITH_TAC `8 * (val (width:int64) - (i' + 1)) + 8 <= 18446744073709551616`);;
    e(SUBGOAL_THEN `
      ...
      e(CONJ_TAC);;
        ...
        e(SUBGOAL_THEN `!x. (lowdigits x 1) MOD 2 EXP 64 = lowdigits x 1` (fun thm -> REWRITE_TAC [thm]) THENL [
          STRIP_TAC THEN IMP_REWRITE_TAC[MOD_LT] THEN ONCE_REWRITE_TAC[ARITH_RULE `64=64*1`] THEN MESON_TAC[LOWDIGITS_BOUND];
          ALL_TAC
        ]);;
        e(MATCH_MP_TAC READ_MEMORY_BYTES_FIRSTELEM);;
        e(EXISTS_TAC `i'+1` THEN CONJ_TAC THENL [ARITH_TAC; ASM_REWRITE_TAC[]]);;
        ...
        e(SUBGOAL_THEN
          ...
          ALL_TAC]);;
        e(IMP_REWRITE_TAC[READ_MEMORY_BYTES_SLICE_HIGH]);;
        e(REWRITE_TAC[HIGHDIGITS_HIGHDIGITS] THEN CONJ_TAC THENL [REPEAT AP_TERM_TAC THEN ASM_ARITH_TAC; ASM_ARITH_TAC]);;

      e(ALL_TAC);;

    e(ASSERT_USING_ASM_ARITH_TAC `val (width:int64) - (i' + 1) < val width - i'`);;
    e(ASSERT_USING_ASM_ARITH_TAC `8 * (val (width:int64) - i') + 8 * i' <= 18446744073709551616`);;
    e(ARM_STEPS_TAC BIGNUM_COPY_ROW_FROM_TABLE_EXEC [5]);;
    e(ARM_STEPS_TAC BIGNUM_COPY_ROW_FROM_TABLE_EXEC [6;7;8]);;
    e(ENSURES_FINAL_STATE_TAC);;
    ...
    e(REPEAT CONJ_TAC);;
      ...
      e(MATCH_MP_TAC (SPECL [`z:int64`; `(val (width:int64) - (i' + 1)):num`; `1:num`] READ_MEMORY_BYTES_MERGE));;
      e(ASM_REWRITE_TAC[ARITH_RULE `8*1=8`; READ_MEMORY_BYTES_BYTES64; LOWDIGITS_LOWDIGITS;
          WORD_OR_0; WORD_AND_0; VAL_WORD]);;
      e(REPEAT CONJ_TAC);;
        e(AP_TERM_TAC THEN ARITH_TAC);;

        e(SUBGOAL_THEN `!x. (lowdigits x 1) MOD 2 EXP 64 = lowdigits x 1` (fun thm -> REWRITE_TAC [DIMINDEX_64; thm]) THENL [
          STRIP_TAC THEN IMP_REWRITE_TAC[MOD_LT] THEN ONCE_REWRITE_TAC[ARITH_RULE `64=64*1`] THEN MESON_TAC[LOWDIGITS_BOUND];
          ALL_TAC
        ]);;
        e(REWRITE_TAC[lowdigits; highdigits]);;
        e(REWRITE_TAC[DIV_MOD]);
        e(AP_THM_TAC THEN REPEAT AP_TERM_TAC);;
        e(REWRITE_TAC[GSYM EXP_ADD]);;
        e(AP_TERM_TAC);;
        e(UNDISCH_TAC `i' < val (width:int64)` THEN ARITH_TAC);;
        ...
        e(REWRITE_TAC[VAL_WORD; DIMINDEX_64]);;
        e(ASM_MESON_TAC[MOD_LT; LT_WORD_64]);;