...
let full_spec,public_vars = mk_safety_spec
    ...
    BIGNUM_MADD_N25519_SUBROUTINE_CORRECT
...
let BIGNUM_MADD_N25519_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e z x y c pc stackpointer returnaddress.
           aligned 16 stackpointer /\
           ALL (nonoverlapping (word_sub stackpointer (word 16),16))
           [word pc,1052; x,8 * 4; y,8 * 4; c,8 * 4; z,8 * 4] /\
           nonoverlapping (word pc,1052) (z,32)
           ==> ensures arm
               (\s.
                    aligned_bytes_loaded s (word pc) bignum_madd_n25519_mc /\
                    read PC s = word pc /\
                    read SP s = stackpointer /\
                    read X30 s = returnaddress /\
                    C_ARGUMENTS [z; x; y; c] s /\
