...
(**** print_literal_from_elf "arm/mlkem/mlkem_keccak2_f1600.o";;
 ****)

let mlkem_keccak2_f1600_mc = define_assert_from_elf
  "mlkem_keccak2_f1600_mc" "arm/mlkem/mlkem_keccak2_f1600.o"
[
...
let MLKEM_KECCAK2_F1600_EXEC = ARM_MK_EXEC_RULE mlkem_keccak2_f1600_mc;;
...
let MLKEM_KECCAK2_F1600_CORRECT = prove
 (`!a rc A A' pc.
      ALL (nonoverlapping (a,400)) [(word pc,0x284); (rc,192)]
      ==> ensures arm
           (\s. aligned_bytes_loaded s (word pc) mlkem_keccak2_f1600_mc /\
  ...
  REWRITE_TAC[fst MLKEM_KECCAK2_F1600_EXEC] THEN
  ...
  REWRITE_TAC[condition_semantics] THEN REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    ARM_STEPS_TAC MLKEM_KECCAK2_F1600_EXEC (1--41) THEN
    ...
    ASSUME_TAC THENL
     [UNDISCH_TAC `i < 24` THEN SPEC_TAC(`i:num`,`i:num`) THEN
      ...
      ALL_TAC] THEN
    ARM_STEPS_TAC MLKEM_KECCAK2_F1600_EXEC (1--68) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    REWRITE_TAC[round_constants; CONS_11; GSYM CONJ_ASSOC;
    WORDLIST_FROM_MEMORY_CONV `wordlist_from_memory(rc,24) s`] THEN
    ARM_SIM_TAC MLKEM_KECCAK2_F1600_EXEC [1] THEN
    ...
    ARM_SIM_TAC MLKEM_KECCAK2_F1600_EXEC (1--41) THEN
    CONV_TAC(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV) THEN
...
let MLKEM_KECCAK2_F1600_SUBROUTINE_CORRECT = prove
(`!a rc A A' pc stackpointer returnaddress.
  ...
  ARM_ADD_RETURN_STACK_TAC ~pre_post_nsteps:(5,5) MLKEM_KECCAK2_F1600_EXEC
   (CONV_RULE TWEAK_CONV MLKEM_KECCAK2_F1600_CORRECT)
