# AES-256-GCM whole-blocks DECRYPT — KAT gate

Mechanical validation of the `aesv8_gcm_8x_dec_256_wb.o` binary on whole-block
inputs **longer than 8 blocks**, exercising the main loop
(`.L256_dec_main_loop`, 0x4a0..0x9ec) and prepretail (0x9f0..0xec0). It catches
the guard-shifted-PC / ABI-clobber / byteswap bug class cheaply, independently
of the HOL Light proof.
...
Each program prints a per-case summary and a final `PASS` line; both exit
